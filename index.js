// ═══════════════════════════════════════════════════════════════
// STORE — Cloudflare Worker — v11.0 — White-Label Edition
// KV Binding : env.DATABASE
// ═══════════════════════════════════════════════════════════════

// ── Password hashing ──
// Old behavior: SHA-256 (single pass, unsalted) — vulnerable to GPU brute force.
// New behavior: PBKDF2-SHA256, 100k iterations, 16-byte random salt.
// Stored format: "pbkdf2$100000$<salt-b64>$<hash-b64>"
// Constant-time comparison is used via subtle.timingSafeEqual fallback.
...
async function hashPass(str){
  ...
  return `pbkdf2$${PBKDF2_ITERATIONS}$${btoa(String.fromCharCode(...salt))}$${btoa(String.fromCharCode(...hashBytes))}`;
}

// Verify a candidate password against a stored PBKDF2 hash. Returns true on match.
async function verifyPass(stored, candidate){
  try{
...
// Admin password hash comes ONLY from env (Cloudflare Secret).
// Format: "pbkdf2$100000$<salt-b64>$<hash-b64>" — set via `wrangler secret put ADMIN_PASS_HASH`.
// To generate one for a new password, run: node scripts/gen-admin-hash.js "<password>"
// If env secret is missing the /api/auth endpoint returns 500 with a clear error.
const BLOCK_MS = 8*3600000;
const MAX_ATT  = 5;
const VALID_CATS=["shirts","pants","shorts","hats","accessories","other"];

// Allowed push notification endpoint hostnames. Anything else is rejected.
const PUSH_ALLOWED_HOSTS = new Set([
  "fcm.googleapis.com",
  "updates.push.services.mozilla.com",
  "push.services.mozilla.com",
  "push.apple.com",
  "wns2-ln.notify.windows.com",
  "wns2-ppe.notify.windows.com",
]);

// ── Localization strings (KV-backed, with built-in defaults) ──
// STRINGS stores UI text per language key. Server first reads "strings:<lang>"
// from KV, falling back to the in-code DEFAULT_STRINGS baked below. Admins can
// edit any text without redeploying via the /api/strings endpoint.
const DEFAULT_STRINGS = {
  ar: {
    "site.title": "STORE_NAME",
    "aria.logo": "متجر",
    "aria.prev": "السابق",
    "aria.next": "التالي",
    "aria.lang_title": "اللغة / Language",
    "lang.ar": "العربية",
    "lang.fr": "Français",
    "lang.en": "English",
    "nav.home": "الرئيسية",
    ...
    "footer.track": "تتبع طلبيتك",
    "footer.faq": "الاسئلة الشائعة",
    "footer.policy": "سياسة الاستبدال",
    ...
    "cat.other": "أخرى",
    "cat.new": "الجديد",
    "cat.bestseller": "الأكثر مبيعا",
    "sort.default": "ترتيب افتراضي",
    "sort.price_asc": "السعر: الأقل أولا",
    "sort.price_desc": "السعر: الأعلى أولا",
    "product.add_to_cart": "إضافة للسلة",
    "product.out_of_stock": "نفذ المخزون",
    "product.low_stock": "تبقى {n} قطعة فقط",
    "product.sold": "تم بيع {n}",
    "product.discount_badge": "خصم {p}٪",
    "checkout.name": "الاسم",
    "checkout.phone1": "الهاتف 1",
    "checkout.phone2": "الهاتف 2",
    "checkout.email": "البريد",
    "checkout.wilaya": "الولاية",
    "checkout.commune": "البلدية",
    "checkout.delivery": "طريقة التوصيل",
    "checkout.delivery_home": "منزل",
    "checkout.delivery_desk": "Stop Desk",
    "checkout.pay": "الدفع",
    "checkout.pay_cod": "عند الاستلام",
    "checkout.pay_ccp": "CCP مسبق",
    "checkout.note": "ملاحظة",
    "checkout.submit": "تأكيد الطلب",
    "checkout.coupon": "كوبون خصم",
    "checkout.coupon_apply": "تطبيق",
    "checkout.error_missing": "حقول ناقصة",
    "checkout.error_phone": "هاتف غير صالح",
    "checkout.error_phone_match": "الهاتفان يجب أن يختلفا",
    "checkout.error_stock": "الكمية غير متوفرة",
    "checkout.ok": "تم إرسال طلبك",
    "track.title": "تتبع طلبية",
    "track.order_id": "رقم الطلبية",
    "track.phone_last4": "آخر 4 أرقام من الهاتف",
    "track.submit": "تتبع",
    "track.not_found": "لم يتم العثور على هذه الطلبية",
    "admin.login_title": "دخول الأدمن",
    "admin.password": "كلمة المرور",
    "admin.remember": "تذكرني (30 يوم)",
    "admin.login_btn": "دخول",
    "admin.logout": "خروج",
    "admin.close": "إغلاق",
    "admin.tabs.analytics": "Analytics",
    "admin.tabs.products": "Products",
    "admin.tabs.add_product": "Add",
    "admin.tabs.orders": "Orders",
    "admin.tabs.coupons": "Coupons",
    "admin.tabs.archive": "Archive",
    "admin.tabs.stock": "Stock",
    "admin.tabs.visitors": "Visitors",
    "admin.tabs.activity": "Activity",
    "admin.tabs.flash": "Flash",
    "admin.tabs.bundles": "Bundles",
    "admin.tabs.waitlist": "Waitlist",
    "admin.tabs.loyalty": "Loyalty",
    ...
    "admin.confirm.title": "تأكيد العملية",
    "admin.confirm.warn": "عملية حساسة",
    "admin.confirm.delete_text": "اكتب حذف",
    "admin.confirm.cancel": "إلغاء",
    "admin.confirm.ok": "نفذ",
    "admin.confirm.hold": "اضغط مطولا",
    "common.save": "حفظ",
    "common.cancel": "إلغاء",
    "common.edit": "تعديل",
    "common.delete": "حذف",
    ...
    "toast.failed": "فشلت العملية",
    "invoice.title": "فاتورة",
    "invoice.print": "طباعة",
    "invoice.close": "إغلاق",
    "invoice.hide_shipping": "إخفاء الشحن",
    "invoice.extra_note": "ملاحظة",
    "invoice.apply": "تطبيق",
    "invoice.save_pdf_hint": "للحفظ PDF: في نافذة الطباعة اختر Save as PDF كطابعة",
    "label.title": "بوليصة شحن",
    "label.print": "طباعة",
    "label.close": "إغلاق",
  },
  fr: {
    ...
    "aria.prev": "Précédent",
    "aria.next": "Suivant",
    "aria.lang_title": "Langue / Language",
    "lang.ar": "العربية",
    "lang.fr": "Français",
    "lang.en": "English",
    "nav.home": "Accueil",
    ...
    "footer.faq": "FAQ",
    "footer.policy": "Politique de retour",
    ...
    "cat.other": "Autres",
    "cat.new": "Nouveau",
    "cat.bestseller": "Best-seller",
    "sort.default": "Tri par defaut",
    "sort.price_asc": "Prix croissant",
    "sort.price_desc": "Prix decroissant",
    "product.add_to_cart": "Ajouter",
    "product.out_of_stock": "En rupture",
    "product.low_stock": "Plus que {n}",
    "product.sold": "Vendus {n}",
    "product.discount_badge": "-{p}%",
    "checkout.name": "Nom",
    "checkout.phone1": "Telephone 1",
    "checkout.phone2": "Telephone 2",
    "checkout.email": "Email",
    "checkout.wilaya": "Wilaya",
    "checkout.commune": "Commune",
    "checkout.delivery": "Livraison",
    "checkout.delivery_home": "Domicile",
    "checkout.delivery_desk": "Stop Desk",
    "checkout.pay": "Paiement",
    "checkout.pay_cod": "A la livraison",
    "checkout.pay_ccp": "CCP avance",
    "checkout.note": "Note",
    "checkout.submit": "Valider",
    "checkout.coupon": "Code promo",
    "checkout.coupon_apply": "Appliquer",
    "checkout.error_missing": "Champs manquants",
    "checkout.error_phone": "Telephone invalide",
    "checkout.error_phone_match": "Telephones differents",
    "checkout.error_stock": "Quantite indisponible",
    "checkout.ok": "Commande envoyee",
    "track.title": "Suivi commande",
    "track.order_id": "N commande",
    "track.phone_last4": "4 derniers chiffres",
    "track.submit": "Suivre",
    "track.not_found": "Commande introuvable",
    "admin.login_title": "Connexion admin",
    "admin.password": "Mot de passe",
    "admin.remember": "Se souvenir (30j)",
    "admin.login_btn": "Entrer",
    "admin.logout": "Sortie",
    "admin.close": "Fermer",
    "admin.tabs.analytics": "Analytics",
    "admin.tabs.products": "Produits",
    "admin.tabs.add_product": "Ajouter",
    "admin.tabs.orders": "Commandes",
    "admin.tabs.coupons": "Coupons",
    "admin.tabs.archive": "Archives",
    "admin.tabs.stock": "Stock",
    "admin.tabs.visitors": "Visiteurs",
    "admin.tabs.activity": "Activite",
    "admin.tabs.flash": "Flash",
    "admin.tabs.bundles": "Bundles",
    "admin.tabs.waitlist": "Liste",
    "admin.tabs.loyalty": "Fidelite",
    "admin.tabs.referrals": "Parrainage",
    "admin.tabs.reviews": "Avis",
    "admin.tabs.testimonials": "Temoin",
    "admin.tabs.stories": "Histoires",
    "admin.tabs.settings": "Reglages",
    "admin.confirm.title": "Confirmation",
    "admin.confirm.warn": "Action sensible",
    "admin.confirm.delete_text": "Tapez supprimer",
    "admin.confirm.cancel": "Annuler",
    "admin.confirm.ok": "OK",
    "admin.confirm.hold": "Maintenir",
    "common.save": "Sauver",
    ...
    "common.no": "Non",
    "toast.copied": "Copie",
    "toast.saved": "Sauvegarde",
    "toast.failed": "Echec",
    "invoice.title": "Facture",
    "invoice.print": "Imprimer",
    "invoice.close": "Fermer",
    "invoice.hide_shipping": "Cacher livraison",
    "invoice.extra_note": "Note",
    "invoice.apply": "Appliquer",
    "invoice.save_pdf_hint": "Pour PDF: choisissez Enregistrer en PDF dans la fenetre d impression",
    "label.title": "Bordereau",
  ...
  en: {
    ...
    "product.low_stock": "{n} left",
    "product.sold": "Sold {n}",
    "product.discount_badge": "-{p}%",
    "checkout.name": "Name",
    "checkout.phone1": "Phone 1",
    "checkout.phone2": "Phone 2",
    "checkout.email": "Email",
    "checkout.wilaya": "Wilaya",
    "checkout.commune": "City",
    "checkout.delivery": "Delivery",
    "checkout.delivery_home": "Home",
    "checkout.delivery_desk": "Stop Desk",
    "checkout.pay": "Payment",
    "checkout.pay_cod": "COD",
    "checkout.pay_ccp": "Prepaid",
    "checkout.note": "Note",
    "checkout.submit": "Submit",
    "checkout.coupon": "Promo code",
    "checkout.coupon_apply": "Apply",
    "checkout.error_missing": "Missing fields",
    "checkout.error_phone": "Invalid phone",
    "checkout.error_phone_match": "Phones must differ",
    "checkout.error_stock": "Out of stock",
    "checkout.ok": "Order sent",
    "track.title": "Track order",
    "track.order_id": "Order id",
    "track.phone_last4": "Last 4 digits",
    "track.submit": "Track",
    "track.not_found": "Not found",
    "admin.login_title": "Admin login",
    "admin.password": "Password",
    "admin.remember": "Remember 30d",
    "admin.login_btn": "Enter",
    "admin.logout": "Logout",
    "admin.close": "Close",
    "admin.tabs.analytics": "Analytics",
    "admin.tabs.products": "Products",
    ...
    "admin.confirm.warn": "Sensitive action",
    "admin.confirm.delete_text": "Type delete",
    "admin.confirm.cancel": "Cancel",
    "admin.confirm.ok": "OK",
    "admin.confirm.hold": "Hold",
    "common.save": "Save",
    ...
    "invoice.print": "Print",
    "invoice.close": "Close",
    "invoice.hide_shipping": "Hide shipping",
    "invoice.extra_note": "Note",
    "invoice.apply": "Apply",
    "invoice.save_pdf_hint": "For PDF: choose Save as PDF in the print dialog",
...
// In-memory merged strings cache, lazily hydrated from KV in main().
let STRINGS = null; // { ar:{...}, fr:{...}, en:{...} }

async function loadStrings(env){
  // Deep-merge KV-overridable strings on top of the baked defaults.
  const out = JSON.parse(JSON.stringify(DEFAULT_STRINGS));
  for (const lang of Object.keys(out)) {
    try {
      const raw = await env.DATABASE.get("strings:" + lang);
      if (raw) {
        const override = JSON.parse(raw);
        for (const k of Object.keys(override)) out[lang][k] = override[k];
      }
    } catch {}
  }
  STRINGS = out;
...
function T(lang, key, vars){
  // Look up a UI string. Falls back through ar->en if key missing.
  if (!STRINGS) return DEFAULT_STRINGS.en[key] || key;
  const s = STRINGS[lang] || STRINGS.ar || DEFAULT_STRINGS.ar;
  let v = s[key] || (STRINGS.en && STRINGS.en[key]) || DEFAULT_STRINGS.en[key] || DEFAULT_STRINGS.ar[key] || key;
...
// ── ADV: server-side helpers ──
// All new endpoints + helpers added below. Keep these at the top so
// the fetch handler below can reach them.

// Sanitize a free-text field: trim, collapse whitespace, cap length, strip control chars.
...
function _isUrl(s){ try { const u = new URL(s); return u.protocol === "https:"; } catch { return false; } }

// Deterministic per-request fingerprint: ip + ua + accept-language hash. Bounded.
async function _fp(req){
  ...
  const lang = (req.headers.get("Accept-Language") || "").substring(0, 30);
  const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(ip + "|" + ua + "|" + lang));
  return "fp:" + Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, "0")).join("").substring(0, 32);
}

// Sliding-window rate limiter (per key). Returns ok/blocked.
async function _rlCheck(env, key, limit, windowMs){
  try {
    const now = Date.now();
    const raw = await env.DATABASE.get(key);
    const arr = raw ? JSON.parse(raw) : [];
    const cutoff = now - windowMs;
    const recent = (Array.isArray(arr) ? arr : []).filter(t => t > cutoff);
    if (recent.length >= limit) return { ok: false, retryMs: (recent[0] + windowMs) - now, count: recent.length };
    recent.push(now);
    await env.DATABASE.put(key, JSON.stringify(recent), { expirationTtl: Math.ceil(windowMs / 1000) * 2 });
    return { ok: true, count: recent.length, remaining: limit - recent.length };
  } catch { return { ok: true, count: 0, remaining: limit }; }
}

// CSV serializer with UTF-8 BOM (Excel-friendly) and Arabic-safe escaping.
...
// Country-tier classifier for visitor quality scoring.
function _tierOf(ua){
  const r = String(ua || "").toLowerCase();
  if (/iphone|ipad|macintosh/.test(r)) return "high";
  if (/android|windows/.test(r)) return "mid";
  if (/linux/.test(r)) return "mid";
  return "low";
}

// Lock-step validator for env-shaped mutations. Sets minimum schema; extra fields preserved.
function _validateProduct(body, partial){
  const out = partial ? {} : { id: Date.now() };
  out.name = _sanitize(body.name, 120);
  if (!out.name && !partial) throw new Error("name_required");
  out.price = Math.max(0, Math.min(9999999, Math.round(Number(body.price) || 0)));
  out.discount = Math.max(0, Math.min(90, Math.round(Number(body.discount) || 0)));
  out.cat = VALID_CATS.includes(body.cat) ? body.cat : "other";
  out.desc = _sanitize(body.desc, 600);
  out.images = Array.isArray(body.images)
    ? body.images.slice(0, 4).filter(u => typeof u === "string" && u.length < 500000).map(u => String(u).trim())
    : [];
  out.stock = body.stock !== false;
  out.quantity = (body.quantity === null || body.quantity === undefined || body.quantity === "")
    ? null
    : Math.max(0, Math.floor(Number(body.quantity) || 0));
  out.sizes = Array.isArray(body.sizes) ? body.sizes.slice(0, 20).map(s => _sanitize(s, 20)) : [];
  out.colors = Array.isArray(body.colors) ? body.colors.slice(0, 20).map(c => _sanitize(c, 30)) : [];
  out.alertQty = Math.max(0, parseInt(body.alertQty) || 0);
  out.showAt = body.showAt ? (function(d){ try { return new Date(d).toISOString(); } catch { return null; } })(body.showAt) : null;
  // Variants: { sizeColor: qty }. Allows independent per-variant stock.
  if (body.variants && typeof body.variants === "object" && !Array.isArray(body.variants)) {
    const v = {};
    for (const k of Object.keys(body.variants).slice(0, 200)) {
      const n = parseInt(body.variants[k]);
      if (Number.isFinite(n) && n >= 0) v[String(k).substring(0, 40)] = Math.min(9999, n);
    }
    out.variants = v;
  }
  // Cost price for margin accounting.
  out.costPrice = body.costPrice !== undefined
    ? Math.max(0, Math.min(9999999, Math.round(Number(body.costPrice) || 0)))
    : 0;
  out.salesCount = Math.max(0, parseInt(body.salesCount) || 0);
  out.createdAt = partial ? out.createdAt : Date.now();
  return out;
}

// Variant-specific check: returns remaining stock for a chosen variant or whole product.
function _variantQty(prod, variantKey){
  if (!prod) return 0;
  if (prod.variants && variantKey && Object.prototype.hasOwnProperty.call(prod.variants, variantKey)) {
    return prod.variants[variantKey];
  }
  if (prod.variants && Object.keys(prod.variants).length) {
    return Object.values(prod.variants).reduce((a, b) => a + (Number(b) || 0), 0);
  }
  return prod.quantity == null ? Infinity : prod.quantity;
}
function _decVariant(prod, variantKey, n){
  if (prod.variants && variantKey && Object.prototype.hasOwnProperty.call(prod.variants, variantKey)) {
    prod.variants[variantKey] = Math.max(0, (prod.variants[variantKey] || 0) - n);
  } else if (prod.quantity != null) {
    prod.quantity = Math.max(0, prod.quantity - n);
  }
}

// Deep-merge saved price/discount in a products array (used by sales update).
function _applyFlash(prods, flash){
  if (!flash || !Array.isArray(prods)) return prods;
  return prods.map(p => {
    const f = flash.find(x => String(x.productId) === String(p.id) && x.active);
    if (!f) return p;
    return Object.assign({}, p, { flashDisc: Math.min(90, Math.max(0, Number(f.discVal) || 0)), flashEndAt: f.endAt });
  });
}

// Show product iff showAt is null or past.
function _isLive(p){
  if (!p || p.showAt == null) return true;
  return new Date(p.showAt).getTime() <= Date.now();
}

// Tiny in-house RFC4122-like id (browser-safe substitute for crypto.randomUUID in older envs).
function _uuid(){
  try { return crypto.randomUUID(); } catch { return "x-" + Date.now().toString(36) + "-" + Math.random().toString(36).slice(2, 10); }
}
...
async function isAdmin(req,env){
  /* الترويسة هي الطريقة الأساسية؛ ?key= كحل بديل فقط لصفحات تُفتح بـ window.open ولا يمكنها إرسال ترويسات (مثل /invoice و /shipping-label) */
  let k=req.headers.get("X-Admin-Key")||"";
  if(!k){try{k=new URL(req.url).searchParams.get("key")||"";}catch{k="";}}
  if(!k)return false;
  try{const val=await env.DATABASE.get("admin_token:"+k);return val!==null;}catch{return false;}
}
async function isUrlToken(req,env){
  /* توكن مؤقت أحادي الاستخدام (60 ثانية) لصفحات window.open فقط */
  let k="";
  try{k=new URL(req.url).searchParams.get("key")||"";}catch{k="";}
  if(!k)return false;
  try{
    const val=await env.DATABASE.get("url_token:"+k);
    if(val===null)return false;
    await env.DATABASE.delete("url_token:"+k);// single-use: يُحذف فور الاستخدام
    return true;
  }catch{return false;}
}
function getFP(req){
...
function isValidPhone(s){return /^0[567]\d{8}$/.test((s||"").replace(/\s/g,""));}
...
async function sendPush(env,title,body){
  try{
    ...
    // ── Validate endpoint is a known push service over HTTPS ──
    // Before: only checked `startsWith("https://")` — allowed any HTTPS endpoint,
    // including attacker-controlled ones. Now: also verify the hostname is in
    // the allow-list (FCM, Mozilla, Apple, WNS).
    let url;
    try { url = new URL(sub.endpoint); } catch { return; }
    if(url.protocol !== "https:")return;
    if(!PUSH_ALLOWED_HOSTS.has(url.hostname))return;
...
async function logError(env,info){
  try{const log=await kvGet(env,"error_log",[]);log.unshift(info);await kvSet(env,"error_log",log.slice(0,200));}catch{}
}

// ── ترجمة المنتج: استدعاء واحد لـ Claude Haiku يُعيد EN+FR معاً ──
async function translateProduct(env,name,desc){
  const key=env.ANTHROPIC_API_KEY;
  if(!key||(!name&&!desc))return {};
  try{
    const prompt="Translate the following Arabic product details to English AND French.\n"
      +"Reply with ONLY a JSON object, no preamble, no markdown:\n"
      +"{\n  \"nameEn\": \"English name\",\n  \"nameFr\": \"French name\",\n  \"descEn\": \"English description\",\n  \"descFr\": \"French description\"\n}\n\n"
      +"Product name (Arabic): "+name+"\n"
      +"Product description (Arabic): "+(desc||"(none)");
    ...
    const json=JSON.parse(raw.replace(/\x60{3}[a-z]*\n?|\n?\x60{3}/g,"").trim());
    return {nameEn:json.nameEn||"",nameFr:json.nameFr||"",descEn:json.descEn||"",descFr:json.descFr||""};
  }catch{return {};}
}

// ── ترجمة جميع المنتجات دفعةً واحدة عبر استدعاء واحد لـ Claude Haiku ──
async function translateAllProducts(env){
  const key=env.ANTHROPIC_API_KEY;
  if(!key)return {ok:false,error:"ANTHROPIC_API_KEY غير مضبوط في Secrets"};
  const prods=await kvGet(env,"products",[]);
  const toTranslate=prods.filter(p=>p.name&&(!p.nameEn||!p.nameFr));
  if(!toTranslate.length)return {ok:true,count:0};
  try{
    const list=toTranslate.map((p,i)=>`${i+1}. id:${p.id} | name:${p.name} | desc:${(p.desc||"").substring(0,200)}`).join("\n");
    const prompt="Translate these Arabic product names and descriptions to English AND French.\n"
      +"Reply with ONLY a JSON array (no markdown, no preamble), one object per product:\n"
      +"[{\"id\":123,\"nameEn\":\"...\",\"nameFr\":\"...\",\"descEn\":\"...\",\"descFr\":\"...\"},...]\n\n"
      +"Products:\n"+list;
    const r=await fetch("https://api.anthropic.com/v1/messages",{
      method:"POST",
      headers:{"Content-Type":"application/json","x-api-key":key,"anthropic-version":"2023-06-01"},
    ...
    const d=await r.json();
    const raw=(d.content&&d.content[0]&&d.content[0].text)||"";
    const translations=JSON.parse(raw.replace(/\x60{3}[a-z]*\n?|\n?\x60{3}/g,"").trim());
    translations.forEach(t=>{
      const pi=prods.findIndex(p=>p.id===t.id);
      if(pi>=0)Object.assign(prods[pi],{nameEn:t.nameEn||"",nameFr:t.nameFr||"",descEn:t.descEn||"",descFr:t.descFr||""});
    });
...
function _escSrv(s){return String(s||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;");}
...
function buildInvoiceHTML(o,s,opts){
  ...
  opts=opts||{};
  const sn=_escSrv(s.storeName||"STORE_NAME"),wa=_escSrv(s.whatsapp||"");
  ...
  const stTxt=ST[o.status]||o.status||"";const stC=SC[o.status]||"#1a1a1a";
...
function buildShippingLabel(o,s,fmt){
  ...
  const bc=encodeCode128((o.id||"ORD").substring(0,20));
...
export default {
  async fetch(request,env){
   try{
    ...
    if(path==="/sw.js"&&method==="GET")return RR(`self.addEventListener("push",function(e){var d={title:"STORE_NAME",body:""};try{d=e.data?e.data.json():d;}catch(_){}e.waitUntil(self.registration.showNotification(d.title||"STORE_NAME",{body:d.body||""}));});self.addEventListener("notificationclick",function(e){e.notification.close();e.waitUntil(clients.openWindow("/"));});`,200,{"Content-Type":"application/javascript;charset=utf-8","Cache-Control":"public,max-age=3600"});

    if(path==="/api/auth"&&method==="POST"){
      const fp=await _fp(request);
      // Hardening: lockout persisted across fingerprint, not just volatile.
      const rl=await _rlCheck(env, "auth:" + fp, MAX_ATT, BLOCK_MS);
      if(!rl.ok){
        return new Response(JSON.stringify({ok:false,stall:true}),{status:429,headers:{...(_getCorsHeaders(request)),"Content-Type":"application/json"}});
      }
      ...
      if(!stored||typeof stored!=="string"||!stored.startsWith("pbkdf2$")){
        // Misconfigured: no admin secret set. Do NOT attempt a default comparison.
        return new Response(JSON.stringify({ok:false,error:"admin_not_configured"}),{status:500,headers:{...(_getCorsHeaders(request)),"Content-Type":"application/json"}});
      }
      const ok=await verifyPass(stored,body.password||"");
      if(ok){
        // Successful auth: clear lockout.
        try { await env.DATABASE.delete("auth:" + fp); } catch {}
        const token=_uuid();
        const ttl=body.remember?2592000:3600;
        await env.DATABASE.put("admin_token:"+token,"1",{expirationTtl:ttl});
        await logActivity(env,"admin_login", "ip:" + fp);
        return RR({ok:true,token});
      }
      // Soft notification only (do not leak auth attempts to public).
      await sendPush(env,"login_failed","attempt " + (MAX_ATT - rl.remaining + 1));
      await logError(env,{type:"auth_fail",fp,t:Date.now()});
      return RR({ok:false,remaining:rl.remaining},401);
    }

    // ADV: strings editor (admin only). GET returns merged map; POST saves overrides.
    if(path==="/api/strings"&&method==="GET"){
      await loadStrings(env);
      return RR({ok:true,strings:STRINGS,defaults:DEFAULT_STRINGS});
    }
    if(path==="/api/strings"&&method==="POST"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      let body;try{body=await request.json();}catch{return RR({error:"bad_json"},400);}
      const lang=SUPPORTED_LANGS.includes(body.lang) ? body.lang : null;
      if(!lang) return RR({error:"bad_lang"},400);
      if(typeof body.overrides!=="object"||body.overrides===null) return RR({error:"bad_overrides"},400);
      // Cap: max 1000 keys, max 4000 chars per value (UI text should be brief).
      const keys = Object.keys(body.overrides).slice(0, 1000);
      const out = {};
      for (const k of keys){
        const v = String(body.overrides[k] == null ? "" : body.overrides[k]).substring(0, 4000);
        out[String(k).substring(0, 200)] = v;
      }
      await env.DATABASE.put("strings:" + lang, JSON.stringify(out));
      await logActivity(env, "strings_update", "lang:" + lang + " keys:" + keys.length);
      await loadStrings(env);
      return RR({ok:true,count:Object.keys(out).length});
    }

    // ADV: heatmap collector. Client batches every 10 clicks.
    if(path==="/api/heatmap"&&method==="POST"){
      let body;try{body=await request.json();}catch{return RR({error:"bad_json"},400);}
      const events = Array.isArray(body.events) ? body.events.slice(0, 50) : [];
      const valid = [];
      for (const e of events){
        if(!e||typeof e.x!=="number"||typeof e.y!=="number") continue;
        if(e.x<0||e.x>4000||e.y<0||e.y>4000) continue;
        if(typeof e.s!=="string"||e.s.length>50) continue;
        valid.push({x:Math.round(e.x),y:Math.round(e.y),s:String(e.s).substring(0,50),t:Date.now()});
      }
      if(!valid.length) return RR({ok:true,stored:0});
      const cur = await kvGet(env, "heatmap", []);
      cur.push(...valid);
      // cap dataset
      const trimmed = cur.length > 5000 ? cur.slice(-5000) : cur;
      await env.DATABASE.put("heatmap", JSON.stringify(trimmed));
      return RR({ok:true,stored:valid.length,total:trimmed.length});
    }
    if(path==="/api/heatmap"&&method==="GET"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      const data = await kvGet(env, "heatmap", []);
      return RR({ok:true,events:data});
    }

    // ADV: per-product analytics ingestion. Lightweight, single-shot.
    if(path==="/api/analytics/product"&&method==="POST"){
      let body;try{body=await request.json();}catch{return RR({error:"bad_json"},400);}
      const pid = parseInt(body.id);
      if(!Number.isFinite(pid)) return RR({error:"bad_id"},400);
      const event = String(body.event||"").substring(0, 20);
      if(!["view","add","remove","buy"].includes(event)) return RR({error:"bad_event"},400);
      const arr = await kvGet(env, "product_stats:" + pid, {view:0,add:0,remove:0,buy:0});
      arr[event] = (arr[event]||0) + 1;
      await env.DATABASE.put("product_stats:" + pid, JSON.stringify(arr));
      return RR({ok:true});
    }

    // ADV: bulk export helpers — orders (CSV) and bulk print (HTML).
    if(path==="/api/orders/export"&&method==="GET"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      const orders = await kvGet(env, "orders", []);
      const cols = ["id","date","status","confirmed","name","phone1","phone2","wilaya","commune","total","items"];
      const rows = [cols];
      for (const o of orders) {
        rows.push([
          o.id, o.date || "", o.status || "", o.confirmed ? "1" : "0",
          o.name || "", o.phone1 || "", o.phone2 || "", o.wilaya || "", o.commune || "",
          o.total || 0,
          (o.items || []).map(i => (i.name || "") + " x" + (i.qty || 1)).join(" | ")
        ]);
      }
      const csv = _toCsv(rows);
      return new Response(csv, {headers: Object.assign(_getCorsHeaders(request), {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition": "attachment; filename=orders.csv"
      })});
    }

    // ADV: bulk print. Returns a self-contained HTML page that opens in a new window
    // (the client uses window.open). Auth: admin OR a single-use url_token.
    if(path==="/api/bulk-print"&&method==="GET"){
      const ok = await isAdmin(request,env) || await isUrlToken(request,env);
      if(!ok) return RR("Unauthorized",401);
      const ids = (url.searchParams.get("ids") || "").split(",").filter(Boolean).slice(0, 100);
      const store = await kvGet(env,"settings",{});
      const fmt = url.searchParams.get("fmt") || "yalidine";
      const all = await kvGet(env,"orders",[]);
      const orders = ids.map(id => all.find(o => o.id === id)).filter(Boolean);
      // Build combined HTML doc.
      let html = "<!DOCTYPE html><html><head><meta charset='utf-8'><title>Bulk</title><style>" +
        "body{font-family:sans-serif;color:#111;background:#f5f5f5;margin:0;padding:20px}" +
        ".doc{background:#fff;margin:0 auto 20px;padding:20px;max-width:760px;box-shadow:0 2px 8px rgba(0,0,0,.1);page-break-after:always}" +
        "h2{margin:0 0 12px}h4{margin:0 0 4px;font-size:11px;color:#666;letter-spacing:1px;text-transform:uppercase}" +
        "table{width:100%;border-collapse:collapse}td{padding:6px;border-bottom:1px solid #eee;font-size:12px}" +
        "@media print{.doc{box-shadow:none;margin:0;page-break-after:always}.toolbar{display:none}body{background:#fff;padding:0}}</style></head><body>";
      html += "<div class='toolbar' style='text-align:center;margin-bottom:20px'><button onclick='window.print()' style='background:#111;color:#fff;border:none;padding:10px 24px;border-radius:6px;cursor:pointer;font-size:13px'>Print All</button></div>";
      for (const o of orders) {
        if (url.searchParams.get("type") === "shipping") {
          html += buildShippingLabel(o, store, fmt);
        } else {
          html += buildInvoiceHTML(o, store, {hideShipping: url.searchParams.get("hideShip")==="true", extraNote: url.searchParams.get("note")||"", scale: 1, printTracked: false});
        }
      }
      html += "</body></html>";
      return new Response(html, {headers: Object.assign(_getCorsHeaders(request), {
        "Content-Type": "text/html; charset=utf-8",
        "X-Frame-Options": "SAMEORIGIN"
      })});
    }

    // ADV: drag-drop reorder for products/bundles.
    if(path==="/api/reorder"&&method==="POST"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      let body;try{body=await request.json();}catch{return RR({error:"bad_json"},400);}
      const which = String(body.type||"products");
      const ids = Array.isArray(body.ids) ? body.ids.slice(0,500).map(x=>String(x).substring(0,40)) : null;
      if(!ids) return RR({error:"bad_ids"},400);
      const key = which === "bundles" ? "bundles" : "products";
      const arr = await kvGet(env, key, []);
      const sorted = ids.map(id => arr.find(x => String(x.id) === id)).filter(Boolean);
      const rest = arr.filter(x => !ids.find(id => String(id) === String(x.id)));
      const next = [...sorted, ...rest];
      await env.DATABASE.put(key, JSON.stringify(next));
      await logActivity(env, "reorder", key + ":" + sorted.length);
      return RR({ok:true});
    }

    // ADV: end-of-route pass: ensure all KV-pending writes were scheduled.
    if(path==="/__nope"&&method==="GET") return RR({ok:false},404);
    ...
    if(path==="/api/url-token"&&method==="POST"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      const tok=crypto.randomUUID();
      await env.DATABASE.put("url_token:"+tok,"1",{expirationTtl:60});
      return RR({ok:true,token:tok});
    }

    if(path==="/api/push-subscribe"&&method==="POST"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      const sub=await request.json().catch(()=>({}));
      // Validate the subscription endpoint belongs to a known push service.
      // Otherwise an attacker who gains admin could register an arbitrary HTTPS URL.
      try{
        const u=new URL(String(sub.endpoint||""));
        if(u.protocol!=="https:"||!PUSH_ALLOWED_HOSTS.has(u.hostname)){
          return RR({error:"endpoint_not_allowed"},400);
        }
      }catch{ return RR({error:"endpoint_invalid"},400); }
      await kvSet(env,"push_subscription",sub);
      return RR({ok:true});
    }
    ...
    if(path==="/api/products"){
      if(method==="GET"){
        ...
        const visibleProds=prods.filter(p=>_isLive(p));
        const prodsWithFlash=_applyFlash(visibleProds,activeFlash);
        // Strip costPrice (sensitive) from public listing.
        const safe = prodsWithFlash.map(p => { const {costPrice, ...rest} = p; return rest; });
        return RR(safe,200,{"Cache-Control":"public,max-age=15"});
      }
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      if(method==="POST"){
        ...
        const prods=await kvGet(env,"products",[]);
        let p;
        try { p = _validateProduct(body, false); }
        catch(e){ return RR({error:e.message||"validation_failed"},400); }
        prods.push(p);await kvSet(env,"products",prods);
        const tr=await translateProduct(env,p.name,p.desc);
        if(tr.nameEn){Object.assign(p,tr);prods[prods.length-1]=p;await kvSet(env,"products",prods);}
        await logActivity(env, "product_create", p.name);
        if(p.alertQty>0 && Number(p.quantity||0)<=p.alertQty){
          await sendPush(env,"low_stock:"+p.name,"qty:"+(p.quantity||0));
        }
        return RR(p);
      }
      if(method==="PUT"){
        const body=await request.json().catch(()=>({})),prods=await kvGet(env,"products",[]);
        const i=prods.findIndex(p=>p.id===body.id);
        if(i<0)return RR({error:"Not found"},404);
        const _allowed=["name","nameEn","nameFr","desc","descEn","descFr","price","discount","images","stock","quantity","cat","sizes","colors","alertQty","showAt","variants","costPrice"];
        const _upd={};
        for(const f of _allowed){if(body[f]!==undefined)_upd[f]=body[f];}
        if(_upd.price!==undefined)_upd.price=Math.max(0,Math.min(9999999,Math.round(Number(_upd.price)||0)));
        if(_upd.discount!==undefined)_upd.discount=Math.max(0,Math.min(90,Math.round(Number(_upd.discount)||0)));
        if(_upd.costPrice!==undefined)_upd.costPrice=Math.max(0,Math.min(9999999,Math.round(Number(_upd.costPrice)||0)));
        if(_upd.quantity!==undefined&&_upd.quantity!==null){
          _upd.quantity=Math.max(0,Math.floor(Number(_upd.quantity)||0));
          const alertQty=prods[i].alertQty||5;
          const prevQty=prods[i].quantity||999;
          const newQty=_upd.quantity;
          if(alertQty>0 && newQty<=alertQty && newQty<prevQty){
            const pname=prods[i].name||"";
            await sendPush(env,"low_stock:"+pname,"qty:"+newQty);
            try{
            ...
            await logActivity(env,"stock_alert",pname+":"+newQty);
          }
        }
        if(_upd.images!==undefined)_upd.images=Array.isArray(_upd.images)?_upd.images.slice(0,4).filter(u=>typeof u==="string"&&u.length<500000).map(u=>String(u).trim()):[];
        if(_upd.cat!==undefined&&!VALID_CATS.includes(_upd.cat))_upd.cat="other";
        if(_upd.showAt!==undefined){ try { _upd.showAt = _upd.showAt ? new Date(_upd.showAt).toISOString() : null; } catch { _upd.showAt = null; } }
        if(_upd.variants!==undefined && (typeof _upd.variants!=="object" || Array.isArray(_upd.variants))) _upd.variants = {};
        prods[i]=Object.assign({},prods[i],_upd);
        if(_upd.name!==undefined||_upd.desc!==undefined){
          const _tr=await translateProduct(env,prods[i].name,prods[i].desc);
          if(_tr && _tr.nameEn) Object.assign(prods[i], _tr);
        }
        await kvSet(env,"products",prods);
        await logActivity(env, "product_update", prods[i].name);
        return RR(prods[i]);
      }
      if(method==="DELETE"){
    ...
    if(path==="/api/orders"){
      if(method==="POST"){
        // Per-fingerprint rate-limit: max 5 orders per 10 minutes.
        const orderFp = "orders:" + (await _fp(request));
        const _rl = await _rlCheck(env, orderFp, 5, 10*60*1000);
        if (!_rl.ok) return RR({error:"rate_limited", retryMs: _rl.retryMs}, 429);

        const body=await request.json().catch(()=>({}));
        if(!body.name||!body.phone1||!body.phone2||!body.wilaya||!body.commune||!body.items?.length)
          return RR({error:"missing_fields"},400);
        if(body.phone1===body.phone2)return RR({error:"phones_match"},400);
        if(!isValidPhone(body.phone1)||!isValidPhone(body.phone2))
          return RR({error:"invalid_phone"},400);
        if(body.items.length>20)return RR({error:"too_many_items"},400);
        // Variant key, optional.
        for (const it of (body.items||[])) {
          if (it.variant && typeof it.variant !== "string") { it.variant = null; }
          if (it.variant) it.variant = String(it.variant).substring(0, 60);
        }

        // SF — معرّف على مستوى الوحدة أعلاه (مصدر واحد للخادم والعميل)
        ...
        /* ── خصم Mystery vs Admin ──
           IMPORTANT: previously this read `body.globalDiscount` and `body.mysteryExp`
           from the request body, allowing any client to self-apply up to a 10% discount
           by sending a fake future timestamp. Fixed: discounts now come ONLY from
           server-side `settings.admin_discount` (admin-controlled) or from a
           per-phone `mystery:{phone1}` KV entry that admin sets explicitly. ── */
        ...
        // Server-side mystery discount check: only honored if an admin-set KV entry exists.
        try{
          const mKey = "mystery:" + (body.phone1 || "").replace(/\s/g, "");
          const mRaw = await env.DATABASE.get(mKey);
          if (mRaw) {
            const m = JSON.parse(mRaw);
            if (m && typeof m.discVal === "number" && m.discVal >= 1 && m.discVal <= 10
                && typeof m.expiresAt === "number" && m.expiresAt > Date.now()) {
              appliedGlobalDisc = m.discVal;
              appliedDiscountMethod = "mystery";
              // one-shot: delete after use
              await env.DATABASE.delete(mKey);
            }
          }
        }catch{}
        ...
        const o={
          id:(settings.storeName||"ORD").substring(0,4).toUpperCase().replace(/\s/g,'')+"-"+Date.now().toString().slice(-7),date:new Date().toISOString(),
          ...
          wilaya:(body.wilaya||"").trim().substring(0,60),commune:(body.commune||"").trim().substring(0,80),dlbl:(body.dlbl||"").trim(),
        ...
        for(const item of body.items){
          ...
          if(pi>=0){
            ...
            // Check stock (whole product OR variant).
            const have=_variantQty(prodsRefresh[pi], item.variant);
            if(have!==Infinity && have<needed){
              return RR({error:"out_of_stock",product:prodsRefresh[pi].name||item.id,variant:item.variant||null},409);
            }
            _decVariant(prodsRefresh[pi], item.variant, needed);
            changed=true;
          }
        }
        if(changed){
          ...
          for(const item of body.items){
            ...
            if(p2){
              stockHist.unshift({t:new Date().toISOString(),productId:p2.id,productName:p2.name||"",type:"sale",qty:-(parseInt(item.qty)||1),balanceAfter:_variantQty(p2,item.variant),variant:item.variant||null,orderId:o.id});
            }
          }
        ...
        allOrders.unshift(o);await kvSet(env,"orders",allOrders.slice(0,2000));
      ...
      if(method==="PATCH"){
        const body=await request.json().catch(()=>({})),orders=await kvGet(env,"orders",[]);
    ...
    if(path==="/api/track"&&method==="POST"){
      const{orderId,phone,phoneLast4}=await request.json().catch(()=>({}));
      const trFp="track:"+getFP(request);
      const trRl=await chkRL(env,trFp);
      if(trRl.blocked)return RR({ok:false,msg:"الرجاء المحاولة بعد قليل"},429);
      // Require BOTH an orderId AND either a full phone OR last-4-digits verification.
      // Order IDs alone (e.g. "ORD-1234567") are 7-digit and guessable — without the phone
      // confirmation anyone could enumerate and read customer names/wilayas.
      if(!orderId||(!phone&&!phoneLast4)){
        return RR({ok:false,msg:"orderId ورقم الهاتف مطلوبان"},400);
      }
      const orders=await kvGet(env,"orders",[]);
      const norm=(s)=>String(s||"").replace(/\s/g,"");
      const phoneN=norm(phone);
      const last4=norm(phoneLast4).slice(-4);
      const match=(x)=>{
        if(x.id!==orderId)return false;
        if(last4){
          const p1=norm(x.phone1).slice(-4);
          const p2=norm(x.phone2).slice(-4);
          if(p1===last4||p2===last4)return true;
          return false;
        }
        if(phoneN){
          return norm(x.phone1)===phoneN||norm(x.phone2)===phoneN;
        }
        return false;
      };
      const o=orders.find(match);
      if(!o){await incRL(env,trFp);return RR({ok:false,msg:"لم يتم العثور على هذه الطلبية"});}
    ...
    if(path==="/api/settings"){
      if(method==="GET")return RR(await kvGet(env,"settings",{storeName:"STORE_NAME",whatsapp:"",email:"",instagram:""}));
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      const rawS=await request.json();
      const safeS={
        storeName:(rawS.storeName||"").trim().substring(0,80)||"STORE_NAME",
        whatsapp:(rawS.whatsapp||"").replace(/[^0-9+]/g,"").substring(0,20),
        email:(rawS.email||"").trim().substring(0,100),
        instagram:(rawS.instagram||"").trim().substring(0,60),
        ...
        about:(rawS.about||"").trim().substring(0,2000),
        faq:(rawS.faq||"").trim().substring(0,5000),
        refundPolicy:(rawS.refundPolicy||"").trim().substring(0,2000),
        lang:(rawS.lang&&["ar","fr","en"].includes(rawS.lang))?rawS.lang:"ar",
        trustItems:Array.isArray(rawS.trustItems)?rawS.trustItems.slice(0,8).map(function(x){return(x||"").trim().substring(0,100);}): [],
    ...
    if(path==="/api/waitlist"){
      if(method==="POST"){
        ...
        if(!isValidPhone(phone))return RR({error:"هاتف غير صالح"},400);
    ...
    if(path==="/api/ref"&&method==="GET"){
      ...
      return RR(ref?{ok:true,hash:ref.hash,uses:ref.uses,discount:ref.discount}:{ok:false});
    }
    if(path==="/refer"){
      const phone=url.searchParams.get("p")||"";
      if(!phone)return RR("رقم الهاتف مطلوب",400);
      const referFp="refer:"+getFP(request);
      const referRl=await chkRL(env,referFp);
      if(referRl.blocked)return RR("الرجاء المحاولة بعد قليل",429);
      const orders=await kvGet(env,"orders",[]);
      if(!orders.some(o=>o.phone1===phone||o.phone2===phone)){
        await incRL(env,referFp);
        return RR("لا يوجد طلب مسجل بهذا الرقم",403);
      }
      const refs=await kvGet(env,"referrals",[]);
      ...
      return RR(`<html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>رابط الإحالة</title></head><body style="font-family:sans-serif;text-align:center;padding:40px;background:#f8f8f8;color:#111"><h2 style="color:#111">رابط الإحالة الخاص بك</h2><p style="color:#888;margin-bottom:20px">أرسل هذا الرابط لأصدقائك وأحصل على خصم 5%</p><div id="rl" style="background:#f0f0f0;border:1px solid #ccc;border-radius:10px;padding:16px;font-size:14px;letter-spacing:1px;color:#111;word-break:break-all">${_refFullLink}</div><button onclick="var l=document.getElementById('rl').textContent;navigator.clipboard.writeText(l).then(function(){alert('تم النسخ!')})" style="margin-top:20px;background:#111;color:#fff;border:none;border-radius:8px;padding:10px 24px;cursor:pointer;font-size:14px">نسخ الرابط</button></body></html>`,200,{"Content-Type":"text/html;charset=utf-8"});
    }
    ...
    if(path==="/api/reviews"){
      if(method==="GET"){
        ...
        const filtered=pid?reviews.filter(r=>String(r.productId)===String(pid)):reviews;
        if(await isAdmin(request,env))return RR(filtered);
        return RR(filtered.filter(r=>r.approved).map(r=>({id:r.id,productId:r.productId,name:r.name,rating:r.rating,body:r.body,t:r.t,approved:r.approved})));
      }
      if(method==="POST"){
        const b=await request.json().catch(()=>({}));
    ...
    // ══ ABOUT PAGE ══════════════════════════════════════════════════
    if(path==="/about"){
      const sets=await kvGet(env,"settings",{storeName:"STORE_NAME"});
      const about=sets.about||"";
      return RR(`<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>عن ${_escSrv(sets.storeName||"STORE_NAME")}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f8f8f8;color:#111;padding:20px;min-height:100vh}.wrap{max-width:680px;margin:0 auto;padding:30px 0}.brand{font-size:28px;font-weight:700;letter-spacing:3px;color:#111;text-align:center;margin-bottom:8px}.sub{text-align:center;font-size:11px;color:rgba(0,0,0,.35);letter-spacing:3px;margin-bottom:40px}.body{font-size:14px;line-height:2;color:rgba(0,0,0,.65)}.back{display:inline-block;margin-bottom:24px;color:rgba(0,0,0,.5);font-size:12px;cursor:pointer;text-decoration:none;border:1px solid rgba(0,0,0,.15);padding:6px 14px;border-radius:7px}
/* ══ FLASH SALE BADGE ══ */
...
/* ══ BUNDLE CARD ══ */
.bundle-card{background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.2);border-radius:14px;padding:16px;cursor:pointer;transition:border-color .2s}
.bundle-card:hover{border-color:rgba(0,0,0,.4)}
.bundle-imgs{display:flex;gap:5px;margin-bottom:10px}
.bundle-img{width:52px;height:64px;object-fit:cover;border-radius:7px;border:1px solid rgba(0,0,0,.1)}
.bundle-name{font-size:13px;font-weight:600;color:rgba(0,0,0,.9);margin-bottom:4px}
...
/* ══ TESTIMONIAL CARD ══ */
.tcard{background:rgba(0,0,0,.03);border:1px solid rgba(0,0,0,.1);border-radius:12px;padding:16px;min-width:220px;max-width:260px;flex-shrink:0}
.tcard-stars{color:#f59e0b;font-size:13px;margin-bottom:7px}
.tcard-body{font-size:11px;color:rgba(0,0,0,.56);line-height:1.7;margin-bottom:10px}
.tcard-name{font-size:10px;color:rgba(0,0,0,.7);font-weight:600}

/* ══ WAITLIST BTN ══ */
.waitlist-btn{display:block;width:100%;padding:9px;background:rgba(0,0,0,.05);border:1px dashed rgba(0,0,0,.2);border-radius:10px;color:rgba(0,0,0,.6);font-size:11px;cursor:pointer;text-align:center;transition:.2s}
.waitlist-btn:hover{background:rgba(0,0,0,.06);border-color:rgba(0,0,0,.3)}
...
.bundle-row{display:grid;grid-template-columns:1fr auto auto auto;gap:10px;align-items:center;padding:9px 12px;border:1px solid rgba(0,0,0,.15);border-radius:9px;margin-bottom:6px;background:rgba(0,0,0,.04);font-size:11px}

/* ══ REVIEW CARD ══ */
.rv-card{padding:11px 13px;border:1px solid var(--b1);border-radius:9px;margin-bottom:7px;background:rgba(0,0,0,.025)}
...
.aprd-row.drag-over{background:rgba(0,0,0,.08);border-color:rgba(0,0,0,.3);outline:2px dashed rgba(0,0,0,.35)}
.aprd-row.dragging{opacity:.4}

/* ══ STORY CARD ══ */
.story-card{background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:10px;overflow:hidden;margin-bottom:10px}
.story-card img{width:100%;height:120px;object-fit:cover}
.story-card-body{padding:10px}
.story-title{font-size:13px;font-weight:600;color:rgba(0,0,0,.9);margin-bottom:5px}
.story-excerpt{font-size:11px;color:var(--mu);line-height:1.5}

/* ══ TRUST BADGES BAR ══ */
.trust-badges{display:flex;flex-wrap:wrap;justify-content:center;gap:12px;padding:10px 14px;border-top:1px solid rgba(0,0,0,.06)}
.trust-badge{display:flex;align-items:center;gap:5px;font-size:10px;color:rgba(0,0,0,.35);letter-spacing:.5px}

/* ══ FAQ ACCORDION ══ */
.faq-item{border-bottom:1px solid rgba(0,0,0,.08)}
.faq-q{padding:11px 14px;cursor:pointer;font-size:12px;color:rgba(0,0,0,.8);font-weight:600;display:flex;justify-content:space-between;align-items:center;list-style:none}
.faq-a{padding:0 14px 11px;font-size:11px;color:rgba(0,0,0,.48);line-height:1.7}
...
.lang-tab-btn.on{background:rgba(0,0,0,.12);color:rgba(0,0,0,.9)}

/* ══ SCHEDULED BADGE ══ */
.scheduled-badge{display:inline-flex;align-items:center;gap:4px;background:rgba(0,0,0,.1);border:1px solid rgba(0,0,0,.25);color:rgba(0,0,0,.8);font-size:9px;padding:2px 7px;border-radius:4px}

/* ══ VARIANTS GRID ══ */
.variants-grid{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}
.var-chip{padding:5px 12px;background:rgba(0,0,0,.05);border:1px solid rgba(0,0,0,.15);border-radius:20px;font-size:11px;color:var(--dim);cursor:pointer;transition:.15s}
.var-chip:hover,.var-chip.on{background:rgba(0,0,0,.12);border-color:rgba(0,0,0,.4);color:rgba(0,0,0,.9)}

</style></head><body><div class="wrap"><a class="back" href="/">&#8594; العودة للمتجر</a><div class="brand">${_escSrv(sets.storeName||"STORE_NAME")}</div><div class="sub">ABOUT US</div><div class="body">${about.replace(/\n/g,"<br>")}</div></div></body></html>`,200,{"Content-Type":"text/html;charset=utf-8"});
    }
    ...
    if(path.startsWith("/p/")){
      ...
      const sn=_escSrv(sets.storeName||"STORE_NAME");
      const pname=p?_escSrv(p.name||""):"منتج";
      const pdesc=p?_escSrv(p.desc||""):"تسوق الآن";
      const pprice=p?Number(p.price||0):0;
      const productUrl = url.origin + "/p/" + pid;
      // QR code: free public api.qrserver.com — no auth, no rate limits for low traffic.
      // We also embed the URL as a fallback for crawlers / print.
      const qrSrc = "https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=10&data=" + encodeURIComponent(productUrl);
...
<meta property="product:price:amount" content="${pprice}">
...
</head><body style="background:#f8f8f8;color:#333;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;text-align:center;padding:40px 20px;margin:0">
<div style="max-width:380px;margin:0 auto;background:#fff;border-radius:14px;padding:24px 20px;box-shadow:0 4px 24px rgba(0,0,0,.06)">
<div style="font-size:18px;font-weight:700;margin-bottom:6px;color:#1a1a1a">${pname}</div>
<div style="font-size:11px;color:#888;letter-spacing:1px;margin-bottom:14px">${sn}</div>
${img?`<img src="${_escSrv(img)}" alt="${pname}" style="max-width:100%;max-height:160px;object-fit:contain;border-radius:8px;margin-bottom:14px">`:``}
<div style="font-size:22px;font-weight:800;color:#1a1a1a;margin-bottom:14px">${pprice?pprice.toLocaleString()+" \u062f\u062c":""}</div>
<div style="background:#fff;padding:12px;border-radius:10px;display:inline-block;border:1px solid #eee">
<img src="${qrSrc}" alt="QR code" width="200" height="200" style="display:block;width:200px;height:200px">
</div>
<div style="font-size:11px;color:#888;margin-top:10px;letter-spacing:.5px">امسح الرمز للمشاركة</div>
<div style="font-size:10px;color:#aaa;margin-top:6px;word-break:break-all;font-family:monospace;direction:ltr">${_escSrv(productUrl)}</div>
<a href="/?openProd=${pid}" style="display:inline-block;margin-top:16px;padding:10px 22px;background:#1a1a1a;color:#fff;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600">فتح في المتجر</a>
</div>
<p style="margin-top:20px;font-size:11px;color:#aaa">جارٍ التحويل…</p>
    ...
    if(path==="/api/coupon-check"&&method==="POST"){
      ...
      const cpFp="cpcheck:"+getFP(request);
      const cpRl=await chkRL(env,cpFp);
      if(cpRl.blocked)return RR({ok:false,msg:"الرجاء المحاولة بعد قليل"},429);
      const coupons=await kvGet(env,"coupons",[]);
      const c=coupons.find(x=>x.code===(code||"").toUpperCase()&&x.active);
      if(!c){await incRL(env,cpFp);return RR({ok:false,msg:"الكود غير صالح"});}
    ...
    // ══ TRANSLATE ALL PRODUCTS (batch) ══════════════════════════════
    if(path==="/api/translate-products"&&method==="POST"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      const result=await translateAllProducts(env);
      return RR(result,result.ok?200:500);
    }

    // ══ ERROR LOG ═══════════════════════════════════════════════════
    if(path==="/api/error-log"){
      if(!await isAdmin(request,env))return RR({error:"Unauthorized"},401);
      if(method==="GET")return RR(await kvGet(env,"error_log",[]));
      if(method==="DELETE"){await kvSet(env,"error_log",[]);return RR({ok:true});}
    }

    // ══ INVOICE PAGE ════════════════════════════════════════════════
    if(path==="/invoice"&&method==="GET"){
      if(!await isAdmin(request,env)&&!await isUrlToken(request,env))return RR("Unauthorized",401);
      ...
      const sets=await kvGet(env,"settings",{storeName:"STORE_NAME",whatsapp:""});
      const opts={
      ...
      return new Response(buildInvoiceHTML(o,sets,opts),{headers:{"Content-Type":"text/html;charset=utf-8","Cache-Control":"no-store, no-cache","X-Content-Type-Options":"nosniff","X-Frame-Options":"DENY","Referrer-Policy":"no-referrer"}});
    }
    ...
    if(path==="/shipping-label"&&method==="GET"){
      ...
      if(!o)return RR("Not found",404);
      const sets=await kvGet(env,"settings",{storeName:"STORE_NAME",whatsapp:""});
      return new Response(buildShippingLabel(o,sets,fmt),{headers:{"Content-Type":"text/html;charset=utf-8","Cache-Control":"no-store, no-cache","X-Content-Type-Options":"nosniff","X-Frame-Options":"DENY","Referrer-Policy":"no-referrer"}});
    }
    ...
    if(path==="/stories"){
      ...
      const cards=stories.filter(s=>s.active).map(s=>
        `<div style="background:rgba(0,0,0,.03);border:1px solid rgba(0,0,0,.12);border-radius:14px;overflow:hidden;margin-bottom:20px">
          ${s.img?`<img src="${_escSrv(s.img)}" style="width:100%;height:200px;object-fit:cover">`:``}
          <div style="padding:18px">
            <h2 style="font-family:Georgia,serif;font-size:18px;color:rgba(0,0,0,.95);margin-bottom:10px">${_escSrv(s.title||"")}</h2>
            <p style="font-size:13px;color:rgba(0,0,0,.5);line-height:1.8">${_escSrv(s.body||"").replace(/\n/g,"<br>")}</p>
          </div>
        </div>`).join("");
...
<title>قصص النجاح — ${sn}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f8f8f8;color:#111;padding:20px;min-height:100vh}.wrap{max-width:700px;margin:0 auto}.brand{font-family:Georgia,serif;font-size:36px;font-weight:900;letter-spacing:6px;color:#555555;text-align:center;margin-bottom:6px}.sub{text-align:center;font-size:10px;color:rgba(0,0,0,.28);letter-spacing:4px;margin-bottom:36px}a.back{display:inline-block;margin-bottom:20px;color:rgba(0,0,0,.7);font-size:12px;border:1px solid rgba(0,0,0,.2);padding:6px 14px;border-radius:7px;text-decoration:none}</style></head>
<body><div class="wrap"><a class="back" href="/">&#8594; العودة للمتجر</a><div class="brand">${sn}</div><div class="sub">SUCCESS STORIES</div>${cards||"<p style='text-align:center;color:rgba(0,0,0,.32);padding:40px'>لا توجد قصص نجاح بعد</p>"}</div></body></html>`,200,{"Content-Type":"text/html;charset=utf-8"});
    }
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',monospace;background:#f5f5f5;color:#e0d0ff;padding:20px}h1{font-family:Georgia,serif;font-size:28px;color:#555555;margin-bottom:4px}.sub{font-size:11px;color:rgba(0,0,0,.32);letter-spacing:2px;margin-bottom:30px}.ep{background:rgba(0,0,0,.025);border:1px solid rgba(0,0,0,.12);border-radius:10px;padding:14px;margin-bottom:12px}.method{display:inline-block;padding:2px 9px;border-radius:4px;font-size:10px;font-weight:700;margin-right:8px;letter-spacing:1px}.get{background:rgba(34,197,94,.12);color:rgba(74,222,128,.9)}.post{background:rgba(0,0,0,.12);color:rgba(0,0,0,.9)}.patch{background:rgba(251,191,36,.12);color:rgba(253,224,71,.9)}.del{background:rgba(239,68,68,.12);color:rgba(252,165,165,.9)}.path{font-family:monospace;font-size:13px;color:rgba(0,0,0,.9)}.desc{font-size:11px;color:rgba(0,0,0,.44);margin-top:5px}.auth{font-size:9px;color:rgba(251,191,36,.7);margin-top:3px}</style></head>
...
${[
  ...
  ["GET","/api/error-log","سجل الأخطاء (للإنتاج)","admin"],
  ...
  ["GET","/api-docs","هذه الصفحة","admin"],
    ...
    const settings=await kvGet(env,"settings",{storeName:"STORE_NAME",whatsapp:"",email:"",instagram:""});
    // Load i18n strings (KV-overridable) so the SSR HTML reflects overrides.
    await loadStrings(env);
    // Pick the user's language: ?lang=ar|fr|en > cookie > Accept-Language > ar
    const _u2 = new URL(request.url);
    let _lang = _u2.searchParams.get("lang") || "";
    if (!SUPPORTED_LANGS.includes(_lang)) {
      const _ck = (request.headers.get("Cookie") || "").match(/(?:^|;\s*)lang=(ar|fr|en)/);
      _lang = _ck ? _ck[1] : "";
    }
    if (!SUPPORTED_LANGS.includes(_lang)) {
      const _al = (request.headers.get("Accept-Language") || "").toLowerCase();
      if (_al.startsWith("fr")) _lang = "fr";
      else if (_al.startsWith("en")) _lang = "en";
    }
    if (!SUPPORTED_LANGS.includes(_lang)) _lang = "ar";
    return RR(buildHTML(settings, _lang),200,{"Cache-Control":"public,max-age=60","X-Content-Type-Options":"nosniff","X-Frame-Options":"DENY","Content-Security-Policy":"default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: *; connect-src 'self'; frame-ancestors 'none'"});
   }catch(err){
    try{
      const errPath=new URL(request.url).pathname;
      console.error("[WORKER ERROR]",request.method,errPath,err&&err.stack||err);
      await logError(env,{t:new Date().toISOString(),method:request.method,path:errPath,
        msg:String(err&&err.message||err),stack:String(err&&err.stack||"").substring(0,1000)});
    }catch{}
    return new Response(JSON.stringify({error:"خطأ داخلي في الخادم"}),{status:500,headers:{..._getCorsHeaders(request),"Content-Type":"application/json"}});
   }
  }
};

/* ══ جدول رسوم الشحن — مصدر واحد للخادم والعميل معاً ══
   h=للمنزل | d=Stop Desk | r=الإرجاع
   عدّل هنا فقط — buildHTML تحقنه تلقائياً في العميل ══ */
const SF={
  "أدرار":{h:1700,d:900,r:400},"الشلف":{h:1100,d:700,r:400},"الأغواط":{h:1300,d:800,r:400},
  "أم البواقي":{h:1100,d:700,r:400},"باتنة":{h:1100,d:700,r:400},"بجاية":{h:1100,d:700,r:400},
  "بسكرة":{h:1300,d:800,r:400},"بشار":{h:1400,d:900,r:400},"بليدة":{h:1100,d:700,r:400},
  ...
  "الجزائر العاصمة":{h:1100,d:700,r:400},"الجلفة":{h:1300,d:800,r:400},"جيجل":{h:1100,d:700,r:400},
  ...
  "مسيلة":{h:1100,d:700,r:400},"معسكر":{h:1100,d:700,r:400},"ورقلة":{h:1300,d:800,r:400},
  "وهران":{h:1100,d:700,r:400},"البيض":{h:1400,d:900,r:400},"إليزي":{h:2200,d:1300,r:400},
  "برج بوعريريج":{h:1100,d:700,r:400},"بومرداس":{h:1100,d:700,r:400},"الطارف":{h:1100,d:700,r:400},
  "تندوف":{h:1900,d:1200,r:400},"تيسمسيلت":{h:1100,d:700,r:400},"الواد":{h:1300,d:800,r:400},
  "خنشلة":{h:1100,d:700,r:400},"سوق أهراس":{h:1100,d:700,r:400},"تيبازة":{h:1100,d:700,r:400},
  ...
  "تيميمون":{h:1700,d:1100,r:400},"أولاد جلال":{h:1300,d:900,r:400},"بني عباس":{h:1400,d:900,r:400},
...
function buildHTML(s, lang){
  // Localize: use T(lang, key) for any UI string. Falls back to the
  // baked-in DEFAULT_STRINGS or the key itself.
  const L = SUPPORTED_LANGS.includes(lang) ? lang : "ar";
  const t = (k, vars) => T(L, k, vars);
  const sn=s.storeName||t("site.title");
  const wa=s.whatsapp||"";
  const em=s.email||"";
  const ig=s.instagram||"";
  const adminDisc=Math.max(0,Math.min(90,parseInt(s.admin_discount||0)||0));
  const WILAYAS=["أدرار","الشلف","الأغواط","أم البواقي","باتنة","بجاية","بسكرة","بشار","بليدة","البويرة","تمنراست","تبسة","تلمسان","تيارت","تيزي وزو","الجزائر العاصمة","الجلفة","جيجل","سطيف","سعيدة","سكيكدة","سيدي بلعباس","عنابة","قالمة","قسنطينة","المدية","مستغانم","مسيلة","معسكر","ورقلة","وهران","البيض","إليزي","برج بوعريريج","بومرداس","الطارف","تندوف","تيسمسيلت","الواد","خنشلة","سوق أهراس","تيبازة","ميلة","عين الدفلى","النعامة","عين تموشنت","غرداية","غليزان","تيميمون","أولاد جلال","بني عباس","عين صالح","عين قزام","تقرت","جانت","المغير","المنيعة","وادي سوف"];
...
<title>${sn}</title>
<meta name="description" id="meta-desc" content="${sn}">
<meta property="og:title" id="og-title" content="${sn}">
<meta property="og:description" id="og-desc" content="${sn}">
<meta property="og:image" id="og-img" content="">
<meta name="twitter:card" content="summary_large_image">

<style>
*{margin:0;padding:0;box-sizing:border-box}
:root{--bg:#ffffff;--p1:rgba(0,0,0,.03);--b1:rgba(0,0,0,.1);--ac:#1a1a1a;--tx:rgba(0,0,0,.82);--dim:rgba(0,0,0,.5);--mu:rgba(0,0,0,.3);--r:16px;--rs:10px}
...
/* ══ HERO BACKGROUND ══ */
.hero-bg{position:relative;width:100%;overflow:hidden;z-index:3;margin-top:0;min-height:180px;max-height:75vh;background:#f0f0f0}
...
.hero-bg-fallback{position:absolute;inset:0;background:#f0f0f0;z-index:0}
.hero-bg-overlay{position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.04) 0%,rgba(0,0,0,.08) 100%);z-index:2;pointer-events:none}
.hero-tagline{position:absolute;bottom:22px;right:0;left:0;text-align:center;z-index:2;color:rgba(0,0,0,.68);font-size:12px;letter-spacing:4px;text-transform:uppercase}
...
/* ══ CCP PAYMENT OPTION ══ */
.pay-opt{display:flex;align-items:flex-start;gap:10px;background:rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.1);border-radius:11px;padding:10px 12px;cursor:pointer;transition:border-color .2s,background .2s}
.pay-opt:has(input:checked){background:rgba(0,0,0,.08);border-color:rgba(0,0,0,.35)}
.pay-opt input[type=radio]{margin-top:3px;accent-color:var(--ac);flex-shrink:0;width:14px;height:14px;cursor:pointer}
.pay-opt-body{flex:1}
.pay-opt-title{font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:2px}
.pay-opt-sub{font-size:10px;color:var(--mu);letter-spacing:.3px}

/* ══ SCROLL PROGRESS ══ */
#scroll-prog{position:fixed;top:0;right:0;left:0;height:2px;background:linear-gradient(90deg,#888,#444,#111);transform-origin:right;transform:scaleX(0);z-index:9999;transition:transform .1s linear}
#main-content{}

/* ══ HEADER ══ */
.hdr{position:sticky;top:0;z-index:200;background:rgba(255,255,255,.94);backdrop-filter:blur(28px);-webkit-backdrop-filter:blur(28px);border-bottom:1px solid rgba(0,0,0,.1);transition:border-color .3s,box-shadow .3s}
.hdr.scrolled{border-bottom-color:rgba(0,0,0,.15);box-shadow:0 4px 20px rgba(0,0,0,.1)}
...
.search-inp{width:100%;background:rgba(0,0,0,.05);border:1px solid rgba(0,0,0,.12);border-radius:var(--rs);color:var(--tx);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:12px;padding:8px 34px 8px 12px;outline:none;transition:.25s}
.search-inp::placeholder{color:var(--mu)}
.search-inp:focus{border-color:rgba(0,0,0,.5);background:rgba(0,0,0,.06);box-shadow:0 0 0 3px rgba(0,0,0,.1)}
...
.cart-btn{display:flex;align-items:center;gap:6px;background:rgba(0,0,0,.12);border:1px solid rgba(0,0,0,.25);border-radius:var(--rs);padding:8px 12px;cursor:pointer;color:rgba(0,0,0,.9);font-size:12px;font-weight:500;white-space:nowrap;transition:.2s}
.cart-btn:hover{background:rgba(0,0,0,.22);transform:translateY(-1px)}
...
.adm-btn{display:flex;align-items:center;gap:5px;background:rgba(0,0,0,.05);border:1px solid var(--b1);border-radius:8px;padding:7px 11px;cursor:pointer;color:var(--dim);font-size:11px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;transition:.18s;white-space:nowrap}
.adm-btn:hover{background:rgba(0,0,0,.1);border-color:rgba(0,0,0,.3);color:rgba(0,0,0,.85);transform:translateY(-1px)}
.adm-btn svg{width:12px;height:12px;flex-shrink:0}
.lang-sw{position:relative}
.lang-btn{display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.05);border:1px solid var(--b1);border-radius:8px;padding:7px 10px;cursor:pointer;color:var(--dim);font-size:11px;font-weight:700;transition:.18s}
.lang-btn:hover{background:rgba(0,0,0,.1);border-color:rgba(0,0,0,.3);color:rgba(0,0,0,.85)}
.lang-menu{display:none;position:absolute;top:calc(100% + 6px);inset-inline-start:0;background:#fff;border:1px solid var(--b1);border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.12);overflow:hidden;z-index:50;min-width:120px}
.lang-menu.open{display:block}
.lang-opt{padding:9px 14px;font-size:12px;color:var(--dim);cursor:pointer;white-space:nowrap;transition:.15s}
.lang-opt:hover{background:rgba(0,0,0,.05)}
.lang-opt.on{color:rgba(0,0,0,.9);font-weight:700;background:rgba(0,0,0,.04)}
.xbtn{background:rgba(0,0,0,.08);border:1px solid var(--b1);border-radius:8px;width:29px;height:29px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--dim);font-size:13px;transition:.18s}
.xbtn:hover{background:rgba(0,0,0,.12);color:#fff}

/* ══ CATEGORIES ══ */
.cats-bar{position:sticky;top:58px;z-index:150;background:rgba(255,255,255,.9);backdrop-filter:blur(16px);border-bottom:1px solid var(--b1);padding:9px 20px}
...
.pill:hover{border-color:rgba(0,0,0,.3);color:rgba(0,0,0,.8);transform:translateY(-1px)}
.pill.on{background:rgba(0,0,0,.15);border-color:rgba(0,0,0,.4);color:rgba(0,0,0,.95)}
...
/* ══ TRUST BAR ══ */
.trust-bar{background:rgba(0,0,0,.75);border-top:1px solid rgba(0,0,0,.14);border-bottom:1px solid rgba(0,0,0,.14);overflow:hidden;position:relative;z-index:5}
...
.trust-item{padding:11px 36px;font-size:9px;color:rgba(0,0,0,.52);letter-spacing:3px;text-transform:uppercase;white-space:nowrap;display:flex;align-items:center;gap:10px}
.trust-item::before{content:'✦';font-size:7px;color:rgba(0,0,0,.28);flex-shrink:0}
...
.embla__btn{position:fixed;top:50vh;z-index:15;background:rgba(248,248,248,.85);border:1px solid rgba(0,0,0,.3);border-radius:50%;width:36px;height:36px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(0,0,0,.9);font-size:18px;transition:.18s;pointer-events:auto}
.embla__btn:hover{background:rgba(0,0,0,.25);border-color:rgba(0,0,0,.6)}
...
.step-item:not(:last-child)::after{content:'';position:absolute;top:12px;left:calc(-50% + 14px);right:calc(50% + 14px);height:1px;background:rgba(0,0,0,.12)}
.step-item.done::after{background:rgba(0,0,0,.5)}
.step-dot{width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:700;border:1.5px solid rgba(0,0,0,.14);color:var(--mu);transition:.25s}
.step-item.active .step-dot{border-color:rgba(0,0,0,.8);color:rgba(0,0,0,.95);background:rgba(0,0,0,.15);box-shadow:0 0 10px rgba(0,0,0,.25)}
.step-item.done .step-dot{border-color:rgba(0,0,0,.6);color:rgba(0,0,0,.8);background:rgba(0,0,0,.1)}
.step-lbl{font-size:8px;color:var(--mu);letter-spacing:.5px;text-align:center;transition:.2s}
.step-item.active .step-lbl{color:rgba(0,0,0,.7)}
.chk-step{display:none}.chk-step.active{display:block}
.step-nav{display:flex;gap:9px;margin-top:14px}
.btn-back{background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.12);border-radius:11px;color:var(--dim);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:12px;font-weight:600;padding:11px;cursor:pointer;flex:1;transition:.2s}
.btn-back:hover{background:rgba(0,0,0,.04);border-color:rgba(0,0,0,.22)}

/* img zoom removed */

/* ══ HOVER LIFT + SCALE + DEPTH SHADOW ══ */
.card{background:rgba(252,252,252,.96);border:1px solid rgba(0,0,0,.1);border-radius:var(--r);overflow:hidden;display:flex;flex-direction:column;cursor:pointer;transition:transform .22s ease,box-shadow .22s ease,border-color .22s ease;position:relative}
.card:hover{transform:translateY(-4px);border-color:rgba(0,0,0,.25);box-shadow:0 14px 40px rgba(0,0,0,.5),0 0 18px rgba(0,0,0,.06)}
.card:active{transform:translateY(-1px)}
.card.hidden{display:none!important}

/* ══ PATTERN INTERRUPTION — every 5th card ══ */
.card:nth-child(5n+3){border-color:rgba(0,0,0,.11);background:rgba(0,0,0,.032)}

/* ══ VOID CARD CORNERS ══ */
.card::before{content:'';position:absolute;inset:0;border-radius:var(--r);pointer-events:none;z-index:3;opacity:0;transition:opacity .35s}
...
.card:hover .img-slider{box-shadow:0 10px 35px rgba(0,0,0,.5),inset 0 1px 0 rgba(0,0,0,.06)}
.img-slider{position:relative;overflow:hidden;aspect-ratio:3/4;background:#f0f0f0;transition:box-shadow .3s;border-radius:var(--r) var(--r) 0 0}
.img-slider::after{content:'';position:absolute;inset:0;z-index:4;pointer-events:none;border-radius:inherit}
.img-slider img{width:100%;height:100%;object-fit:cover;filter:brightness(1) saturate(1);transition:filter .4s,opacity .3s;position:absolute;top:0;left:0;opacity:0}
.img-slider img.active{opacity:1;position:relative}
.img-slider img.lazy-blur{filter:brightness(1) saturate(1) blur(10px);transform:scale(1.04)}
...
.slide-arr:hover{background:rgba(0,0,0,.6)}
.slide-dots{position:absolute;bottom:6px;left:50%;transform:translateX(-50%);display:flex;gap:4px;z-index:5}
.slide-dot{width:5px;height:5px;border-radius:50%;background:rgba(0,0,0,.32);cursor:pointer;transition:.18s}
.slide-dot.on{background:rgba(0,0,0,.9);width:12px;border-radius:3px}

.card-body{padding:11px;flex:1;display:flex;flex-direction:column;gap:5px;position:relative;z-index:2}
.card-cat{font-size:9px;color:rgba(0,0,0,.45);letter-spacing:2.5px;text-transform:uppercase}

.price-wrap{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:2px}
.card-price{font-family:Georgia,serif;font-size:16px;font-weight:700;color:rgba(0,0,0,.95);letter-spacing:.3px}
.card-name{font-size:12px;font-weight:400;color:rgba(0,0,0,.62);line-height:1.4;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;letter-spacing:.2px}
...
/* ══ SOCIAL PROOF PULSE ══ */
.social-proof{font-size:9px;color:rgba(0,0,0,.42);letter-spacing:.5px;display:flex;align-items:center;gap:4px}

/* ══ REAL SCARCITY INDICATOR ══ */
.scarcity-bar{height:3px;background:rgba(0,0,0,.08);border-radius:2px;overflow:hidden;margin-top:3px}
...
.fomo-txt{font-size:9px;color:rgba(0,0,0,.35);letter-spacing:.5px;font-style:italic;margin-top:auto}

/* ══ HOVER LIFT ADD BUTTON ══ */
.addbtn{background:rgba(0,0,0,.16);border:1px solid rgba(0,0,0,.25);border-radius:10px;color:rgba(0,0,0,.75);font-size:11px;font-weight:600;letter-spacing:.5px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;padding:10px 8px;cursor:pointer;transition:transform .22s cubic-bezier(.34,1.2,.64,1),background .22s,border-color .22s,box-shadow .22s;margin-top:8px;position:relative;overflow:hidden;text-align:center}
.addbtn:hover{background:rgba(0,0,0,.3);border-color:rgba(0,0,0,.52);transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,.25)}
...
/* ══ SKELETON ══ */
.skel-card{background:rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.06);border-radius:var(--r);overflow:hidden}
.skel{background:linear-gradient(90deg,rgba(0,0,0,.05) 25%,rgba(0,0,0,.055) 50%,rgba(0,0,0,.05) 75%);background-size:200% 100%;animation:skel-sh 1.6s ease-in-out infinite}
...
.ov.on{display:block}
.cart-sb{position:fixed;top:0;right:-105%;width:360px;max-width:100vw;height:100%;background:rgba(248,248,248,.98);border-right:1px solid var(--b1);z-index:400;display:flex;flex-direction:column;transition:right .32s cubic-bezier(.4,0,.2,1)}
.cart-sb.on{right:0}
...
.cart-title{font-family:Georgia,serif;font-size:15px;color:rgba(0,0,0,.9);letter-spacing:3px}
.cart-items{flex:1;overflow-y:auto;padding:13px;display:flex;flex-direction:column;gap:8px}
.cart-items::-webkit-scrollbar{width:3px}
.cart-items::-webkit-scrollbar-thumb{background:rgba(0,0,0,.3);border-radius:2px}
...
.c-item{display:grid;grid-template-columns:56px 1fr auto;gap:8px;align-items:center;background:rgba(0,0,0,.04);border:1px solid var(--b1);border-radius:11px;padding:8px}
.c-img{width:56px;height:70px;object-fit:cover;border-radius:5px;filter:brightness(.8)}
.c-name{font-size:11px;color:rgba(0,0,0,.5);margin-bottom:3px;line-height:1.4}
.c-price{font-family:Georgia,serif;font-size:12px;color:rgba(0,0,0,.85)}
.c-sz{font-size:10px;color:var(--mu);margin-top:2px}
.rmbtn{background:none;border:none;color:rgba(0,0,0,.25);font-size:14px;cursor:pointer;padding:3px;border-radius:5px;transition:.18s}
...
.cart-tot-v{font-family:Georgia,serif;font-size:18px;color:rgba(0,0,0,.9)}
.btn-main{background:linear-gradient(135deg,#111111,#1a1a1a);border:none;border-radius:11px;color:#fff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:12px;font-weight:600;padding:12px;cursor:pointer;transition:.22s;width:100%}
.btn-main:hover{box-shadow:0 6px 22px rgba(0,0,0,.35)}
...
.mod{background:rgba(248,248,248,.98);border:1px solid rgba(0,0,0,.18);border-radius:20px;padding:28px;width:100%;max-width:520px;animation:pop .3s cubic-bezier(.34,1.4,.64,1);position:relative;margin:auto;flex-shrink:0}
.mod::-webkit-scrollbar{width:3px}
.mod::-webkit-scrollbar-thumb{background:rgba(0,0,0,.3);border-radius:2px}
@keyframes pop{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}
.mod-title{font-family:Georgia,serif;font-size:16px;color:rgba(0,0,0,.9);letter-spacing:2px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between}
.fl{margin-bottom:12px}
.fl label{display:block;font-size:10px;font-weight:500;color:rgba(0,0,0,.75);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:5px}
.inp{width:100%;background:rgba(0,0,0,.05);border:1px solid rgba(0,0,0,.12);border-radius:9px;color:rgba(0,0,0,.82);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:12px;padding:9px 12px;outline:none;transition:.2s}
.inp::placeholder{color:rgba(0,0,0,.3)}
.inp:focus{border-color:rgba(0,0,0,.4);background:rgba(0,0,0,.08);box-shadow:0 0 0 3px rgba(0,0,0,.08)}
...
.gal-thumb.on{border-color:rgba(0,0,0,.7);filter:brightness(1)}
.sz-row{display:flex;gap:6px;flex-wrap:wrap}
.sz-btn{padding:6px 12px;background:rgba(0,0,0,.05);border:1px solid rgba(0,0,0,.12);border-radius:7px;color:var(--dim);font-size:12px;font-weight:600;cursor:pointer;transition:.18s;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif}
.sz-btn:hover{border-color:rgba(0,0,0,.3);color:rgba(0,0,0,.8)}
.sz-btn.on{background:rgba(0,0,0,.2);border-color:rgba(0,0,0,.5);color:rgba(0,0,0,.95)}
...
.or-sep::before,.or-sep::after{content:'';position:absolute;top:50%;width:38%;height:1px;background:rgba(0,0,0,.08)}
.or-sep::before{right:0}.or-sep::after{left:0}
.op{background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.1);border-radius:11px;padding:12px;margin-bottom:13px}
.op-row{display:flex;justify-content:space-between;padding:3px 0;font-size:11px}
.op-l{color:var(--mu)}.op-v{color:rgba(0,0,0,.65)}
.op-tot{display:flex;justify-content:space-between;align-items:center;padding-top:8px;margin-top:6px;border-top:1px solid rgba(0,0,0,.15)}
.op-tl{font-family:Georgia,serif;font-size:11px;color:rgba(0,0,0,.5);letter-spacing:1px}
.op-tv{font-family:Georgia,serif;font-size:17px;color:rgba(0,0,0,.95)}
...
.inv-main span:last-child{font-size:19px;font-weight:700;color:#111}
.inv-note{text-align:center;padding:10px;background:#f5f5f5;border:1px solid #ddd;border-radius:9px;font-size:12px;color:#333;font-weight:500;margin:10px 0}
...
.inv-btn-p{background:#1a1a1a;color:#fff}.inv-btn-d{background:#f0f0ee;color:#111;border:1px solid #ddd}
...
.mystery-mod{background:linear-gradient(145deg,rgba(250,250,250,.99),rgba(250,250,250,.98));border:1px solid rgba(0,0,0,.25);max-width:380px;text-align:center;padding:36px 28px;border-radius:20px;animation:pop .4s cubic-bezier(.34,1.56,.64,1)}
.mystery-brand{font-size:24px;font-weight:700;color:rgba(0,0,0,.6);letter-spacing:6px;margin-bottom:6px}
.mystery-title{font-size:10px;color:var(--mu);letter-spacing:3px;text-transform:uppercase;margin-bottom:20px}
.mystery-disc{font-size:48px;font-weight:700;color:rgba(0,0,0,.9);line-height:1;margin-bottom:6px}
.mystery-sub{font-size:11px;color:var(--dim);letter-spacing:1px;margin-bottom:22px}
.mystery-code{background:rgba(0,0,0,.04);border:1px dashed rgba(0,0,0,.2);border-radius:8px;padding:9px 14px;font-size:14px;color:rgba(0,0,0,.9);letter-spacing:3px;margin-bottom:18px}

/* ══ ADMIN PANEL ══ */
#adm{display:none;position:fixed;inset:0;background:#f5f5f5;z-index:2000;flex-direction:column}
#adm.on{display:flex}
.adm-hdr{background:rgba(255,255,255,.97);border-bottom:1px solid rgba(0,0,0,.18);
...
.adm-logo-txt{font-size:11px;color:rgba(0,0,0,.6);
  letter-spacing:3px;text-transform:uppercase}
.adm-body{display:flex;flex:1;overflow:hidden}
.adm-side{width:190px;background:rgba(0,0,0,.02);border-left:1px solid rgba(0,0,0,.08);
  display:flex;flex-direction:column;padding:16px 0;flex-shrink:0;overflow-y:auto;gap:2px}
.anav{padding:10px 18px;font-size:11px;font-weight:500;color:rgba(0,0,0,.38);
...
.anav::before{content:'';width:5px;height:5px;border-radius:50%;
  background:rgba(0,0,0,.25);flex-shrink:0;transition:.18s}
.anav:hover{color:rgba(0,0,0,.75);background:rgba(0,0,0,.04)}
.anav:hover::before{background:rgba(0,0,0,.5)}
.anav.on{color:rgba(0,0,0,.95);background:rgba(0,0,0,.08);
  border-right-color:rgba(0,0,0,.7)}
.anav.on::before{background:rgba(0,0,0,.9)}
.adm-c{flex:1;overflow-y:auto;padding:24px 28px}
.adm-c::-webkit-scrollbar{width:3px}
.adm-c::-webkit-scrollbar-thumb{background:rgba(0,0,0,.2);border-radius:2px}
.asec{display:none}.asec.on{display:block}
.adm-title{font-size:14px;color:rgba(0,0,0,.8);
...
.sc{background:rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.1);
...
.sc::before{content:'';position:absolute;inset:0;
  background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(0,0,0,.04),transparent 70%)}
.sc:hover{border-color:rgba(0,0,0,.22);transform:translateY(-2px)}
.sv{font-family:Georgia,serif;font-size:22px;color:rgba(0,0,0,.92);
  margin-bottom:3px;letter-spacing:.5px}
.sl{font-size:9px;color:rgba(0,0,0,.3);letter-spacing:2px;text-transform:uppercase}

/* Table upgrade */
.at{width:100%;border-collapse:collapse}
.at th{font-size:9px;color:rgba(0,0,0,.55);letter-spacing:2px;text-transform:uppercase;
  padding:10px 12px;text-align:right;border-bottom:1px solid rgba(0,0,0,.1);
  white-space:nowrap}
.at td{padding:10px 12px;font-size:11px;color:rgba(0,0,0,.6);
  border-bottom:1px solid rgba(0,0,0,.05);vertical-align:middle}
.at tr:hover td{background:rgba(0,0,0,.03);color:rgba(0,0,0,.75)}
.at tr:last-child td{border-bottom:none}

/* Bar charts */
.cw{margin-bottom:18px}
.cl{font-size:9px;color:rgba(0,0,0,.32);margin-bottom:8px;letter-spacing:1.5px;text-transform:uppercase}
.br{display:flex;align-items:center;gap:10px;margin-bottom:7px}
.brl{width:100px;color:rgba(0,0,0,.4);text-align:right;flex-shrink:0;
  font-size:10px;letter-spacing:.3px}
.brb{flex:1;height:4px;background:rgba(0,0,0,.06);border-radius:2px;overflow:hidden}
.brf{height:100%;background:linear-gradient(90deg,#111111,#1a1a1a,#555555);
  border-radius:2px;transition:width .6s cubic-bezier(.4,0,.2,1)}
.brv{width:50px;color:rgba(0,0,0,.75);font-weight:600;font-size:10px;
  font-family:Georgia,serif}
.at tr:hover td{background:rgba(0,0,0,.025)}
...
.aact.e:hover{border-color:rgba(0,0,0,.4);color:rgba(0,0,0,.9)}
...
.oc{background:rgba(0,0,0,.04);border:1px solid var(--b1);border-radius:11px;padding:13px;margin-bottom:8px}
.oc-h{display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;flex-wrap:wrap;gap:5px}
.oc-id{font-size:11px;color:rgba(0,0,0,.8);letter-spacing:1px}
...
.oc-if span{font-size:11px;color:rgba(0,0,0,.65)}
.oc-pl{border-top:1px solid rgba(0,0,0,.06);padding-top:7px;display:flex;flex-direction:column;gap:4px}
.oc-pi{display:flex;align-items:center;gap:7px;background:rgba(0,0,0,.025);border-radius:5px;padding:5px}
.oc-pimg{width:28px;height:35px;object-fit:cover;border-radius:4px;filter:brightness(.75)}
.oc-pn{flex:1;font-size:11px;color:rgba(0,0,0,.48)}.oc-pp{font-size:11px;color:rgba(0,0,0,.55)}
.oc-ft{display:flex;justify-content:space-between;align-items:center;margin-top:8px;padding-top:6px;border-top:1px solid rgba(0,0,0,.06);flex-wrap:wrap;gap:5px}
.status-sel{appearance:none;background:rgba(0,0,0,.05);border:1px solid var(--b1);border-radius:6px;color:var(--dim);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:11px;padding:4px 9px;outline:none;cursor:pointer}
.img-upload-area{border:2px dashed rgba(0,0,0,.28);border-radius:11px;padding:18px;text-align:center;cursor:pointer;transition:.2s;background:rgba(0,0,0,.03)}
.img-upload-area:hover,.img-upload-area.drag{border-color:rgba(0,0,0,.55);background:rgba(0,0,0,.08)}
.img-previews{display:flex;flex-wrap:wrap;gap:7px;margin-top:9px}
.img-prev-wrap{position:relative;width:68px;height:85px}
.img-prev-wrap img{width:100%;height:100%;object-fit:cover;border-radius:7px;border:1px solid rgba(0,0,0,.28)}
...
.up-prog{height:3px;background:rgba(0,0,0,.08);border-radius:2px;margin-top:7px;overflow:hidden;display:none}
.up-prog-bar{height:100%;background:linear-gradient(90deg,#555,#222);border-radius:2px;transition:width .3s}
.up-status{font-size:10px;color:var(--mu);margin-top:3px;text-align:center}
.vr{display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.06);border-radius:9px;margin-bottom:6px;font-size:11px}
.vr-id{color:var(--mu);font-family:monospace;font-size:10px}
.push-banner{background:rgba(0,0,0,.1);border:1px solid rgba(0,0,0,.2);border-radius:10px;padding:10px 13px;margin-bottom:13px;display:flex;align-items:center;justify-content:space-between;gap:9px;font-size:11px;color:rgba(0,0,0,.5)}
.push-banner button{background:rgba(0,0,0,.22);border:1px solid rgba(0,0,0,.35);border-radius:7px;color:rgba(0,0,0,.85);font-size:11px;padding:4px 10px;cursor:pointer;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif}

/* == CONFIRM-MOD m3 == */
.confirm-mod{background:rgba(250,250,250,.98);border:1px solid rgba(239,68,68,.3);border-radius:16px;padding:24px 20px;max-width:340px;width:90%;animation:pop .32s cubic-bezier(.34,1.56,.64,1)}
...
.confirm-mod-info{font-size:11px;color:rgba(0,0,0,.48);line-height:1.7;margin-bottom:14px;padding:9px 11px;background:rgba(239,68,68,.05);border:1px solid rgba(239,68,68,.1);border-radius:8px;white-space:pre-line}
.confirm-mod-label{font-size:10px;color:rgba(0,0,0,.35);letter-spacing:1px;margin-bottom:5px;display:block}
.confirm-mod-inp{width:100%;background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.12);border-radius:8px;color:rgba(0,0,0,.75);font-size:12px;padding:8px 10px;outline:none;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;margin-bottom:12px;box-sizing:border-box;transition:.18s}
...
.confirm-mod-cancel{background:rgba(0,0,0,.05);border:1px solid rgba(0,0,0,.12);border-radius:9px;padding:9px;font-size:11px;color:rgba(0,0,0,.44);cursor:pointer;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;transition:.18s}
.confirm-mod-cancel:hover,.confirm-mod-cancel:active{border-color:rgba(0,0,0,.25);color:rgba(0,0,0,.65)}
...
.confirm-mod-arch{background:rgba(0,0,0,.1);border:1px solid rgba(0,0,0,.25);border-radius:9px;padding:9px;font-size:11px;color:rgba(0,0,0,.8);cursor:pointer;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;transition:.18s;margin-bottom:8px;width:100%;display:block;text-align:center}
.confirm-mod-arch:hover,.confirm-mod-arch:active{background:rgba(0,0,0,.2);border-color:rgba(0,0,0,.45)}

/* ══ QUANTITY EDITOR IN ADMIN ══ */
.qty-inp{width:70px;background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.12);border-radius:6px;color:rgba(0,0,0,.75);font-size:11px;padding:4px 7px;text-align:center;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;outline:none}
.qty-inp:focus{border-color:rgba(0,0,0,.4)}
.qty-wrap{display:flex;align-items:center;gap:4px}

/* ══ BOTTOM NAV ══ */
.bot-nav{position:fixed;bottom:0;right:0;left:0;z-index:300;background:rgba(255,255,255,.95);backdrop-filter:blur(22px);border-top:1px solid var(--b1);display:flex;align-items:center;justify-content:space-around;padding:8px 0}
...
.bn-item:hover,.bn-item:active{color:rgba(0,0,0,.9);transform:translateY(-2px)}
.bn-item svg{width:18px;height:18px}
.bn-sep{width:1px;height:24px;background:var(--b1)}

/* ══ FOOTER ══ */
.footer{background:rgba(0,0,0,.7);border-top:1px solid rgba(0,0,0,.08);padding:32px 20px 110px;position:relative;z-index:5}
.footer-inner{max-width:1200px;margin:0 auto;display:grid;grid-template-columns:1fr 1fr 1fr;gap:28px}
.footer-brand{font-size:20px;color:rgba(0,0,0,.4);letter-spacing:5px;margin-bottom:8px}
.footer-tagline{font-size:10px;color:var(--mu);letter-spacing:2px;text-transform:uppercase;line-height:1.8}
.footer-h{font-size:9px;color:rgba(0,0,0,.5);letter-spacing:2px;text-transform:uppercase;margin-bottom:10px;border-bottom:1px solid rgba(0,0,0,.1);padding-bottom:5px}
.footer-link{display:block;font-size:11px;color:var(--mu);margin-bottom:7px;cursor:pointer;transition:.18s;letter-spacing:.5px}
.footer-link:hover{color:rgba(0,0,0,.7)}
.footer-contact{font-size:11px;color:rgba(0,0,0,.3);margin-bottom:6px;letter-spacing:.5px;line-height:1.7}
.footer-contact a{color:rgba(0,0,0,.45);text-decoration:none;transition:.18s}
.footer-contact a:hover{color:rgba(0,0,0,.7)}
.footer-copy{text-align:center;font-size:10px;color:rgba(0,0,0,.18);letter-spacing:2px;margin-top:22px;padding-top:16px;border-top:1px solid rgba(0,0,0,.06)}
.spin{display:inline-block;width:13px;height:13px;border:2px solid rgba(0,0,0,.3);border-top-color:rgba(0,0,0,.8);border-radius:50%;animation:sp .7s linear infinite;vertical-align:middle}
@keyframes sp{to{transform:rotate(360deg)}}
.toast{position:fixed;bottom:90px;left:50%;transform:translateX(-50%) translateY(20px);background:rgba(0,0,0,.85);backdrop-filter:blur(18px);border:1px solid rgba(0,0,0,.5);border-radius:9px;color:rgba(0,0,0,.78);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:12px;padding:8px 16px;z-index:9000;opacity:0;transition:.28s cubic-bezier(.34,1.56,.64,1);white-space:nowrap;pointer-events:none;max-width:90vw;text-align:center}
...
@media print{
  body::before,body::after,.ambient-bg,.vignette,.hdr,.cats-bar,.tb,.trust-bar,.embla,.embla-wrap,.grid,.ov,.cart-sb,.toast,.bot-nav,.mod-ov:not(#inv-mod),.footer,#adm,#scroll-prog,.hero-bg{display:none!important}
...
/* ══ KPI CARDS ══ */
.kpi-card{background:rgba(0,0,0,.03);border:1px solid rgba(0,0,0,.12);border-radius:12px;padding:14px;display:flex;flex-direction:column;gap:4px;position:relative;overflow:hidden}
.kpi-card::before{display:none}
.kpi-label{font-size:9px;color:rgba(0,0,0,.35);letter-spacing:2px;text-transform:uppercase}
.kpi-value{font-size:22px;font-weight:700;color:rgba(0,0,0,.95)}
...
.sales-bar{background:linear-gradient(180deg,rgba(0,0,0,.5),rgba(0,0,0,.25));border-radius:3px 3px 0 0;min-width:12px;flex:1;transition:opacity .2s;cursor:default;position:relative}
...
/* ══ COUPONS TABLE ══ */
.coup-row{display:grid;grid-template-columns:auto 1fr auto auto;gap:10px;align-items:center;padding:9px 12px;border:1px solid var(--b1);border-radius:9px;margin-bottom:6px;background:rgba(0,0,0,.025);font-size:11px}
.coup-code{color:rgba(0,0,0,.9);letter-spacing:2px;font-weight:700}
.coup-detail{color:var(--dim)}
.coup-used{color:var(--mu);font-size:10px}
.coup-expired{opacity:.4;text-decoration:line-through}

/* ══ ARCHIVE TABLE ══ */
.arch-row{display:grid;grid-template-columns:44px 1fr auto;gap:10px;align-items:center;padding:9px 12px;border:1px solid rgba(0,0,0,.08);border-radius:9px;margin-bottom:6px;background:rgba(0,0,0,.02);font-size:11px;opacity:.65}
.arch-row:hover{opacity:.9}

/* ══ STOCK HISTORY ══ */
.sh-row{display:grid;grid-template-columns:1fr auto auto auto;gap:8px;align-items:center;padding:8px 12px;border-bottom:1px solid rgba(0,0,0,.05);font-size:11px}
...
.tm-slide{flex:0 0 100%;scroll-snap-align:start;padding:18px 20px;background:rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.1);border-radius:12px;box-sizing:border-box}
.tm-stars{margin-bottom:8px;font-size:16px}
.tm-body{font-size:13px;color:rgba(0,0,0,.65);line-height:1.7;margin-bottom:10px;font-style:italic}
.tm-name{font-size:11px;color:rgba(0,0,0,.7);letter-spacing:1px}
...
.ord-hist{margin-top:10px;padding:8px 10px;background:rgba(0,0,0,.025);border-radius:7px;border:1px solid var(--b1);font-size:10px;display:flex;flex-direction:column;gap:4px}
...
.ord-note-inp{width:100%;background:rgba(0,0,0,.04);border:1px solid var(--b1);border-radius:8px;color:var(--tx);font-family:-apple-system,sans-serif;font-size:11px;padding:7px 10px;outline:none;resize:none;transition:.2s}
.ord-note-inp:focus{border-color:rgba(0,0,0,.4)}

/* ══ ACTIVITY LOG ══ */
.act-row{display:flex;gap:10px;align-items:flex-start;padding:8px 10px;border-bottom:1px solid rgba(0,0,0,.05);font-size:11px}
...
@media(prefers-reduced-motion:reduce){
  *{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important}
  .trust-scroll{animation:none!important}.skel{animation:none!important;background:rgba(0,0,0,.05)!important}
}
</style>
</head>
<body>
...
<header class="hdr">
  <div class="hdr-i">
    <a href="#" class="logo" id="store-name-hdr" aria-label="${t("aria.logo")}">
      <span id="store-logo-text" style="font-size:18px;font-weight:700;letter-spacing:2px;color:#1a1a1a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">${sn}</span>
    </a>
    <div class="search-wrap">
      <span class="search-ico">&#9906;</span>
      <input class="search-inp" id="search-inp" type="text" placeholder="${t("search.placeholder")}" oninput="STORE.liveSearch(this.value)">
    </div>
    <div class="hdr-r">
      ...
      <div class="lang-sw" id="lang-sw">
        <button class="lang-btn" id="lang-btn-hdr" title="${t("aria.lang_title")}">AR</button>
        <div class="lang-menu" id="lang-menu">
          <div class="lang-opt on" data-lang="ar">${t("lang.ar")}</div>
          <div class="lang-opt" data-lang="fr">${t("lang.fr")}</div>
          <div class="lang-opt" data-lang="en">${t("lang.en")}</div>
        </div>
      </div>
...
<div class="cats-bar">
  <div class="cats-i">
    <div class="pill on" id="pill-all">${t("cat.all")}</div>
    <div class="pill" id="pill-shirts">${t("cat.shirts")}</div>
    <div class="pill" id="pill-pants">${t("cat.pants")}</div>
    <div class="pill" id="pill-shorts">${t("cat.shorts")}</div>
    <div class="pill" id="pill-hats">${t("cat.hats")}</div>
    <div class="pill" id="pill-acc">${t("cat.accessories")}</div>
    <div class="pill" id="pill-other">${t("cat.other")}</div>
    <div class="pill-sep"></div>
    <div class="pill" id="pill-new">${t("cat.new")}</div>
    <div class="pill" id="pill-top">${t("cat.bestseller")}</div>
  </div>
</div>

<div class="tb">
  <div class="pc"><span id="pc">0</span> ${t("pc.pieces")}</div>
  <select class="ss" id="ss">
    <option value="d">${t("sort.default")}</option>
    <option value="l">${t("sort.price_asc")}</option>
    <option value="h">${t("sort.price_desc")}</option>
  </select>
</div>

<div class="trust-bar">
  <div class="trust-scroll">
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[0])||"توصيل لجميع الولايات")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[1])||"الدفع عند الاستلام")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[2])||"ضمان الاستبدال")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[3])||"خدمة عملاء متميزة")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[0])||"توصيل لجميع الولايات")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[1])||"الدفع عند الاستلام")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[2])||"ضمان الاستبدال")}</div>
    <div class="trust-item">${_escSrv((s.trustItems&&s.trustItems[3])||"خدمة عملاء متميزة")}</div>
...
<div id="main-content">
  <div class="embla-wrap">
    ...
    <button class="embla__btn embla__btn--prev" id="embla-prev" aria-label="${t("aria.prev")}">&#8249;</button>
    <button class="embla__btn embla__btn--next" id="embla-next" aria-label="${t("aria.next")}">&#8250;</button>
  </div>
</div>
...
<footer class="footer">
  <div class="footer-inner">
    <div>
      <div style="margin-bottom:12px">
        <span style="font-size:16px;font-weight:700;letter-spacing:2px;color:rgba(0,0,0,.4);display:block;margin-bottom:6px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">${sn}</span>
        <div class="footer-tagline"></div>
      </div>
    </div>
    <div>
      <div class="footer-h">${t("footer.links")}</div>
      <span class="footer-link" id="fl-track">${t("footer.track")}</span>
      <span class="footer-link" id="fl-faq">${t("footer.faq")}</span>
      <span class="footer-link" id="fl-policy">${t("footer.policy")}</span>
    </div>
    <div>
      <div class="footer-h">${t("footer.contact")}</div>
  ...
  <div class="footer-copy">${sn} — ${t("footer.copyright")}</div>
</footer>
  ...
  <div class="mod" style="max-width:330px">
    <div class="mod-title">
      <span style="display:flex;align-items:center;gap:7px">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(0,0,0,.8)" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
        Admin Access
      </span>
...
<div class="mod-ov" id="size-mod">
  <div class="mod">
    ...
    <div id="size-prod-name" style="font-size:11px;color:rgba(0,0,0,.35);margin-bottom:13px"></div>
...
<div class="mod-ov" id="prod-mod">
  <div class="mod" style="max-width:580px">
    ...
    <div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7;margin-bottom:13px" id="pm-desc"></div>
...
<div class="mod-ov" id="checkout-mod">
  <div class="mod">
    ...
    <div class="chk-step" id="chk-s3">
      <div class="fl"><label>طريقة الدفع *</label>
        <div id="pay-opts" style="display:flex;flex-direction:column;gap:7px;margin-top:4px">
          ...
          <div id="ccp-details" style="display:none;background:rgba(0,0,0,.07);border:1px solid rgba(0,0,0,.2);border-radius:10px;padding:11px 13px;font-size:11px;color:rgba(0,0,0,.6);line-height:1.8">
            <div style="font-size:10px;color:rgba(0,0,0,.7);letter-spacing:1.5px;text-transform:uppercase;margin-bottom:6px">تفاصيل الحساب البريدي</div>
            <div>رقم الحساب: <span style="color:rgba(0,0,0,.9);font-family:Georgia,serif" id="ccp-account-num">—</span></div>
            <div>الاسم: <span style="color:rgba(0,0,0,.75)" id="ccp-account-name">—</span></div>
    ...
    <div class="chk-step" id="chk-s4">
      <div id="chk-summary" style="margin-bottom:12px;background:rgba(0,0,0,.04);border:1px solid var(--b1);border-radius:9px;padding:10px;font-size:11px;max-height:110px;overflow-y:auto"></div>
      <div class="op">
        ...
        <div class="fl"><label>كود خصم (اختياري)</label>
          <div style="display:flex;gap:6px">
            ...
            <button class="aact e" style="font-size:11px;white-space:nowrap;touch-action:manipulation" onclick="STORE._applyCoupon()">تطبيق</button>
          </div>
...
<div class="mod-ov" id="review-mod">
  <div class="mod" style="max-width:420px">
    ...
    <button class="btn-main" onclick="STORE._submitReview()">إرسال التقييم</button>
  </div>
</div>

<div class="mod-ov" id="inv-mod">
  <div style="position:relative;width:100%;max-width:450px">
    <button id="inv-xbtn" style="position:absolute;top:-12px;left:-12px;z-index:10;background:rgba(248,248,248,.97);border:1px solid rgba(0,0,0,.2);border-radius:50%;width:30px;height:30px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:rgba(0,0,0,.48);font-size:13px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">&#10005;</button>
...
<div class="mod-ov" id="track-mod">
  <div class="mod" style="max-width:400px">
    ...
    <div class="fl"><label>رقم الطلبية او رقم الهاتف</label>
      <input class="inp" id="track-inp" type="text" placeholder="رقم الطلبية او 05XXXXXXXX">
    </div>
...
<div class="mod-ov" id="faq-mod">
  <div class="mod" style="max-width:500px">
    ...
    <div style="display:flex;flex-direction:column;gap:14px">
      <div style="border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:13px"><div style="font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:5px">كم تستغرق مدة التوصيل؟</div><div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7">تتراوح مدة التوصيل بين 2 الى 5 ايام عمل حسب الولاية.</div></div>
      <div style="border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:13px"><div style="font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:5px">هل يمكنني الاستبدال؟</div><div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7">نعم، نضمن الاستبدال في غضون 3 ايام من استلام المنتج شرط ان يكون بحالته الاصلية.</div></div>
      <div style="border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:13px"><div style="font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:5px">ما هي طرق الدفع المتاحة؟</div><div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7">الدفع نقداً عند الاستلام — او الدفع المسبق عبر CCP البريدي للحصول على خصم 50 دج على التوصيل.</div></div>
      <div style="border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:13px"><div style="font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:5px">كيف احدد مقاسي الصحيح؟</div><div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7">استخدم حاسبة المقاس داخل صفحة المنتج بادخال وزنك وطولك وجنسك.</div></div>
      <div style="border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:13px"><div style="font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:5px">هل التوصيل متوفر في ولايتي؟</div><div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7">نوصل لجميع الولايات الـ 58 في الجزائر.</div></div>
      <div style="border-bottom:1px solid rgba(0,0,0,.08);padding-bottom:13px"><div style="font-size:12px;font-weight:600;color:rgba(0,0,0,.75);margin-bottom:5px">كيف يعمل الدفع بـ CCP؟</div><div style="font-size:12px;color:rgba(0,0,0,.44);line-height:1.7">اختر الدفع المسبق بـ CCP عند الطلب، حوّل المبلغ لحسابنا البريدي، ثم ارسل صورة الإيصال عبر الواتساب لتأكيد الطلبية.</div></div>
    </div>
  </div>
</div>
...
<div class="mod-ov" id="policy-mod">
  <div class="mod" style="max-width:500px">
    ...
    <div style="display:flex;flex-direction:column;gap:12px;font-size:12px;color:rgba(0,0,0,.44);line-height:1.8">
      ...
      <ul style="list-style:none;display:flex;flex-direction:column;gap:7px">
        <li style="display:flex;gap:8px"><span style="color:rgba(0,0,0,.5)">—</span><span>مدة الاستبدال: 3 ايام من تاريخ الاستلام</span></li>
        <li style="display:flex;gap:8px"><span style="color:rgba(0,0,0,.5)">—</span><span>المنتج يجب ان يكون بحالته الاصلية غير ملبوس</span></li>
        <li style="display:flex;gap:8px"><span style="color:rgba(0,0,0,.5)">—</span><span>يجب التواصل معنا عبر الواتساب قبل الارسال</span></li>
        <li style="display:flex;gap:8px"><span style="color:rgba(0,0,0,.5)">—</span><span>رسوم الشحن العكسي على عاتق العميل في حالة تغيير المقاس</span></li>
      </ul>
    </div>
  </div>
</div>
...
<div class="mod-ov" id="mystery-mod">
  <div class="mystery-mod">
    <div class="mystery-brand">${sn}</div>
    ...
    <div style="margin-top:10px"><span class="footer-link" onclick="STORE._showFAQ()">الأسئلة الشائعة</span>
          <span class="footer-sep">·</span>
          <span class="footer-link" onclick="STORE._showRefundPolicy()">سياسة الإرجاع</span>
          <span class="footer-sep">·</span>
...
<div id="adm">
  <div class="adm-hdr">
    <div class="adm-logo">
      <span style="font-size:13px;font-weight:700;letter-spacing:2px;color:rgba(0,0,0,.5);font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif">${sn}</span>
    ...
    <div class="adm-c">
      <div class="asec on" id="as-analytics">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:7px">
          ...
          <button class="aact e" onclick="STORE._loadAnalytics()">&#8635; Refresh</button>
        </div>
        ...
        <div class="cw"><div class="cl" style="display:flex;justify-content:space-between"><span>المبيعات — آخر 14 يوم</span><span id="sales-chart-total" style="font-size:11px;color:rgba(0,0,0,.7)"></span></div><div id="sales-chart" style="margin-top:8px;overflow-x:auto"></div></div>
        ...
        <div class="cw" id="kv-stats-section">
          <div class="cl" style="display:flex;justify-content:space-between;align-items:center">
            ...
            <button onclick="STORE._loadKvStats()" style="background:rgba(0,0,0,.15);border:1px solid rgba(0,0,0,.3);color:rgba(0,0,0,.9);border-radius:6px;padding:3px 10px;font-size:10px;cursor:pointer">تحديث</button>
          </div>
      ...
      <div class="asec" id="as-addprod">
        ...
        <div style="display:flex;gap:7px">
          <!-- م26: Variants -->
        <div style="background:rgba(0,0,0,.04);border:1px solid rgba(0,0,0,.1);border-radius:10px;padding:12px;margin-bottom:12px">
          <div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:1px;margin-bottom:8px">المقاسات والألوان (م26)</div>
      ...
      <div class="asec" id="as-orders">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:7px">
          ...
          <div style="display:flex;gap:5px;flex-wrap:wrap">
            ...
            <button class="aact" onclick="STORE._exportCSV()" style="background:rgba(34,197,94,.12);border-color:rgba(34,197,94,.3);color:rgba(134,239,172,.9)">تصدير CSV</button>
          <button class="aact" onclick="STORE._showCsvCols()" style="font-size:9px">أعمدة</button>
            <button class="aact d" id="orders-clear-btn">Clear All</button>
          </div>
        </div>
        <div id="ord-f-bar" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;padding:9px;background:rgba(0,0,0,.025);border-radius:8px;border:1px solid var(--b1)">
          ...
          <button class="aact" onclick="STORE._groupOrders()">بالولاية</button>
        </div>
        <div id="orders-c"></div>
      </div>
      <div class="asec" id="as-visitors">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:7px">
          ...
          <button class="aact e" onclick="STORE._loadVisitors()">&#8635; Refresh</button>
        </div>
      ...
      <div class="asec" id="as-coupons">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadCoupons()">&#8635; Refresh</button>
        </div>
        <div style="background:rgba(0,0,0,.03);border:1px solid var(--b1);border-radius:10px;padding:14px;margin-bottom:14px">
          <div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:1px;margin-bottom:10px">إنشاء كوبون جديد</div>
          ...
          <button class="btn-main" style="width:auto;padding:8px 18px;font-size:11px" onclick="STORE._createCoupon()">+ إنشاء كوبون</button>
        </div>
      ...
      <div class="asec" id="as-archive">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadArchive()">مزامنة</button>
          <button class="aact" id="archive-restore-all-btn" style="display:none" onclick="STORE._restoreSelectedArchive()">استعادة المحدد</button>
          <label style="font-size:10px;color:var(--mu);display:flex;align-items:center;gap:4px">
            <input type="checkbox" id="archive-sel-all" style="accent-color:#1a1a1a"> تحديد الكل
          </label>
        </div>
      ...
      <div class="asec" id="as-stock">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadStockHistory()">&#8635; Refresh</button>
        </div>
        <div style="background:rgba(0,0,0,.03);border:1px solid var(--b1);border-radius:10px;padding:12px;margin-bottom:12px">
          <div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:1px;margin-bottom:8px">إضافة مخزون يدوياً</div>
          <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:8px;align-items:end">
            ...
            <button class="btn-main" style="width:auto;padding:8px 14px;font-size:11px;margin-bottom:0" onclick="STORE._addStock()">+ إضافة</button>
          </div>
        </div>
      ...
      <div class="asec" id="as-activity">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadActivity()">&#8635; Refresh</button>
        </div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin:24px 0 14px">
          <div class="adm-title" style="margin-bottom:0">سجل الأخطاء (Error Log)</div>
          <div style="display:flex;gap:8px">
            <button class="aact e" onclick="STORE._loadErrorLog()">&#8635; Refresh</button>
            <button class="aact d" onclick="STORE._clearErrorLog()">تفريغ</button>
          </div>
        </div>
        <div id="errlog-c"><div style="color:var(--mu);font-size:12px">اضغط Refresh لتحميل سجل الأخطاء</div></div>
        <div style="display:flex;justify-content:space-between;align-items:center;margin:24px 0 14px">
          <div>
            <div class="adm-title" style="margin-bottom:2px">ترجمة المنتجات (EN / FR)</div>
            <div style="font-size:11px;color:var(--mu)">يتطلب إعداد <code>ANTHROPIC_API_KEY</code> في Cloudflare Secrets</div>
          </div>
          <button class="aact e" id="translate-all-btn" onclick="STORE._translateAllProds()">&#127760; ترجمة الكل</button>
        </div>
      ...
      <div class="asec" id="as-flash">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadFlashSales()">&#8635; Refresh</button>
        </div>
        <div style="background:rgba(0,0,0,.03);border:1px solid var(--b1);border-radius:10px;padding:14px;margin-bottom:14px">
          ...
          <button class="btn-main" style="width:auto;padding:8px 18px;font-size:11px" onclick="STORE._createFlashSale()">إنشاء Flash Sale</button>
        </div>
        <div id="flash-c"></div>
      </div>
      ...
      <div class="asec" id="as-bundles">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadBundles()">&#8635; Refresh</button>
        </div>
        <div style="background:rgba(0,0,0,.03);border:1px solid var(--b1);border-radius:10px;padding:14px;margin-bottom:14px">
          ...
          <button class="btn-main" style="width:auto;padding:8px 18px;font-size:11px" onclick="STORE._createBundle()">إنشاء حزمة</button>
        </div>
      ...
      <div class="asec" id="as-waitlist">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadWaitlist()">&#8635; Refresh</button>
        </div>
      ...
      <div class="asec" id="as-loyalty">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadLoyalty()">&#8635; Refresh</button>
        </div>
        <div style="font-size:11px;color:var(--mu);margin-bottom:10px;padding:9px;background:rgba(0,0,0,.025);border-radius:8px;border:1px solid var(--b1)">
      ...
      <div class="asec" id="as-referrals">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadReferrals()">&#8635; Refresh</button>
        </div>
        <div style="font-size:11px;color:var(--mu);margin-bottom:10px;padding:9px;background:rgba(0,0,0,.025);border-radius:8px;border:1px solid var(--b1)">
          رابط الإحالة: <code style="color:rgba(0,0,0,.8)">/refer?p=PHONE</code> · الخصم: 5% لكل إحالة ناجحة
        </div>
      ...
      <div class="asec" id="as-reviews">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadReviews()">&#8635; Refresh</button>
        </div>
      ...
      <div class="asec" id="as-testimonials">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <button class="aact e" onclick="STORE._loadTestimonials()">&#8635; Refresh</button>
        </div>
        <div style="background:rgba(0,0,0,.03);border:1px solid var(--b1);border-radius:10px;padding:14px;margin-bottom:14px">
          ...
          <button class="btn-main" style="width:auto;padding:8px 18px;font-size:11px" onclick="STORE._createTestimonial()">+ إضافة</button>
        </div>
      ...
      <div class="asec" id="as-stories">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
          ...
          <div style="display:flex;gap:6px">
            ...
            <button class="aact e" onclick="STORE._loadStories()">&#8635; Refresh</button>
          </div>
        </div>
        <div style="background:rgba(0,0,0,.03);border:1px solid var(--b1);border-radius:10px;padding:14px;margin-bottom:14px">
          ...
          <button class="btn-main" style="width:auto;padding:8px 18px;font-size:11px" onclick="STORE._createStory()">+ نشر القصة</button>
        </div>
      ...
      <div class="asec" id="as-settings">
        ...
        <div class="fl"><label>Store Name</label><input class="inp" id="s-name" placeholder="STORE_NAME"></div>
        <div class="fl"><label>WhatsApp</label><input class="inp" id="s-wa" placeholder="+000000000000"></div>
        <div class="fl"><label>Email</label><input class="inp" id="s-em" placeholder="contact@example.com"></div>
        <div class="fl"><label>Instagram (username only)</label><input class="inp" id="s-ig" placeholder="your.handle"></div>
        <div class="fl">
          <label>تخفيض عام على المنتجات % <span style="font-size:9px;color:rgba(0,0,0,.5)">(0 = بدون تخفيض — الحد الأقصى 90%)</span></label>
        ...
        <div style="margin-bottom:10px">
          <label style="font-size:10px;color:rgba(0,0,0,.6);display:block;margin-bottom:6px">أو اختر من المعرض مباشرة:</label>
          <label id="hero-pick-lbl" style="display:flex;align-items:center;gap:8px;background:rgba(0,0,0,.08);border:1px dashed rgba(0,0,0,.3);border-radius:10px;padding:10px 12px;cursor:pointer;transition:.2s">
            <span style="font-size:18px; display:none"></span>
            <span id="hero-pick-txt" style="font-size:11px;color:rgba(0,0,0,.7)">اضغط لاختيار صورة أو فيديو من المعرض</span>
        ...
        <div style="font-size:10px;color:rgba(0,0,0,.4);margin-bottom:10px;line-height:1.6">الصورة أو الفيديو من المعرض يُحوَّل إلى Base64 ويُحفظ مباشرة — لا حاجة لرابط خارجي.</div>
        <!-- Trust Bar (م38) -->
        <div style="border-top:1px solid var(--b1);margin:14px 0;padding-top:14px">
          <div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:1px;margin-bottom:8px">Trust Bar (م38)</div>
        ...
        <div style="border-top:1px solid var(--b1);margin:14px 0;padding-top:14px">
          <div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:1px;margin-bottom:8px">الأسئلة الشائعة FAQ (م39)</div>
        ...
        <!-- Trust Badges (م40) -->
        <div style="border-top:1px solid var(--b1);margin:14px 0;padding-top:14px">
          <div style="font-size:10px;color:rgba(0,0,0,.6);letter-spacing:1px;margin-bottom:8px">شارات الثقة (م40)</div>
...
<div id="kb-hint" style="display:none;transition:opacity .28s;position:fixed;bottom:70px;left:50%;transform:translateX(-50%);background:rgba(10,0,22,.97);border:1px solid rgba(0,0,0,.25);border-radius:10px;padding:10px 16px;z-index:9000;font-size:10px;color:var(--mu);white-space:nowrap;letter-spacing:.5px;direction:ltr">Ctrl+N: منتج &nbsp;·&nbsp; Ctrl+O: طلبيات &nbsp;·&nbsp; Ctrl+A: تحليلات &nbsp;·&nbsp; Ctrl+F: بحث &nbsp;·&nbsp; Ctrl+S: حفظ &nbsp;·&nbsp; Ctrl+P: طباعة &nbsp;·&nbsp; Ctrl+D: ولايات &nbsp;·&nbsp; F11: ملء الشاشة &nbsp;·&nbsp; 1–9: تبويبات &nbsp;·&nbsp; Esc: إغلاق</div>

<script>
/* ══════════════════════════════════════════════════════════════
   STORE — Client JS v11.0 — Stable Production Build
   ══════════════════════════════════════════════════════════════ */
...
var STORE = (function(){
  ...
  var _stName="STORE_NAME"; // يُحدَّث عند _loadSettings
  var SESSION_KEY="store_session",REMEMBER_KEY="store_remember";
  ...
  /* ── CART UPDATE ── */
  /* ── CART PERSISTENCE ── */
  function _saveCart(){try{localStorage.setItem("store_cart",JSON.stringify(_cart));}catch(e){}}
  function _loadCart(){try{var c=localStorage.getItem("store_cart");if(c)_cart=JSON.parse(c)||[];}catch(e){}}
  ...
  function _updCart(){
    try{
      ...
      if(!cont)return;
      if(!_cart.length){cont.innerHTML="<div class='c-empty'><span style='font-size:30px;opacity:.2'>&#8711;</span><span>"+_t("cartEmpty")+"</span></div>";return;}
      var html="";
  ...
  function _renderGridData(fp){
    try{
      ...
      var g=document.getElementById("grid");if(!g)return;
      if(!fp.length){g.innerHTML="<div class='empty'>"+_t("noProds")+"</div>";return;}
      var html="";
      fp.forEach(function(p,idx){
        ...
        var spHtml="<div class='social-proof'><svg width='8' height='8' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2'><path d='M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2'/><circle cx='9' cy='7' r='4'/><path d='M23 21v-2a4 4 0 00-3-3.87'/><path d='M16 3.13a4 4 0 010 7.75'/></svg>"+views+_t("viewing")+"</div>";
        var scarHtml="";
        if(p.quantity!==null&&p.quantity!==undefined&&p.quantity<=20){
          ...
          scarHtml="<div class='scarcity-bar'><div class='scarcity-fill "+cls+"' style='width:"+pct+"%'></div></div>"
                  +"<div class='scarcity-txt'>"+_t("scarce1")+p.quantity+_t("scarce2")+"</div>";
        }
        var badgesHtml=(p.salesCount>0?"<div class='sales-counter'>"+p.salesCount+" "+_t("sold")+"</div>":"")
        ...
        var actionHtml=(!p.stock||(p.quantity!==null&&p.quantity!==undefined&&p.quantity===0))
          ?"<button class='waitlist-btn' data-wl-pid='"+p.id+"' data-wl-name='"+_esc(p.name||"")+"'>"+_t("waitlistBtn")+"</button>"
          :"<button class='addbtn' data-pid='"+p.id+"'>"+_t("addCartShort")+"</button>";
        html+="<div class='embla__slide'><div class='card' data-pid='"+p.id+"' data-name='"+_esc(p.name)+"' data-cat='"+_esc(p.cat)+"'>"
             +_makeSlider(imgs,p.id)
             +"<div class='card-body'><div class='card-cat'>"+_esc(_t(p.cat||"other"))+"</div>"
             +"<div class='card-name'>"+_esc(_pname(p))+"</div>"+ph
             +spHtml+scarHtml
             +"<div class='fomo-txt'>"+_t("fomoCard")+"</div>"
      ...
      g.querySelectorAll(".card").forEach(function(card){
        ...
        if(wlbtn){
          wlbtn.addEventListener("click",function(e){
            ...
            var phone=prompt(_t("waitlistPrompt"));
            if(!phone)return;
            _api("/api/waitlist",{method:"POST",body:JSON.stringify({productId:pid,phone:phone})})
              .then(function(r){return r.json();}).then(function(d){_toast(d.msg||d.error||_t("waitlistOk"));})
              .catch(function(){_toast(_t("waitlistErr"));});
          });
        }
      ...
      _initFlashTimers();
      _updateMeta(_stName+" — "+fp.length+" منتج","");
    }catch(e){}
  }
  ...
  function _openProd(id){
    try{
      ...
      var imgs=p.images&&p.images.length?p.images:(p.img?[p.img]:[]);
      var ep=_effPrice(p);
      var pmName=document.getElementById("pm-name");if(pmName)pmName.textContent=_pname(p);
      var pw=document.getElementById("pm-price-wrap");
      if(pw){
        if(p.discount&&p.discount>0){
          var _dpv=Math.min(p.discount,90);
          pw.innerHTML="<div class='price-wrap'><span style='font-family:Georgia,serif;font-size:20px;color:rgba(0,0,0,.9)'>"+_fmt(ep)+"</span><span style='font-size:13px;color:var(--mu);text-decoration:line-through'>"+_fmt(p.price)+"</span><span class='disc-badge'>-"+_dpv+"%</span></div>";
        }else{
          pw.innerHTML="<span style='font-family:Georgia,serif;font-size:20px;color:rgba(0,0,0,.9)'>"+_fmt(ep)+"</span>";
        }
      }
      var fomo=document.getElementById("pm-fomo");if(fomo)fomo.textContent=_t("fomo");
      var desc=document.getElementById("pm-desc");if(desc)desc.textContent=_pdesc(p)||"";
      ...
      _updateMeta(p.name+" — "+_stName,p.desc||"",imgs[0]||"");
      _openMod("prod-mod");
    }catch(e){}
  }
  ...
  function _confirmAdd(){
    ...
    _toast(_t("toastPickSize"));return;}
  ...
  function _spawnParticles(){
    try{
      ...
      var colors=["#1a1a1a","#555555","#333333","#f0abfc","#e879f9"];
      for(var i=0;i<10;i++){
        (function(idx){
  ...
  function _openCheckout(){
    if(!_cart.length){_toast(_t("cartEmpty"));return;}
    _closeCart();
    // ملء ملخص الخطوة 4
    ...
    var sh="";
    _cart.forEach(function(c){sh+="<div style='display:flex;justify-content:space-between;color:rgba(0,0,0,.44);padding:3px 0;border-bottom:1px solid rgba(0,0,0,.06)'><span>"+_esc(c.name.substring(0,18))+" ["+_esc(c.size)+"] x"+c.qty+"</span><span style='color:rgba(0,0,0,.7)'>"+_fmt(c.price*c.qty)+"</span></div>";});
  ...
  function _submitOrder(){
    ...
    if(!name||!p1||!p2||!wilaya||!commune){_toast(_t("toastFillFields"));return;}
    if(p1===p2){_toast(_t("toastPhonesDiffer"));return;}
    if(!_cart.length){_toast(_t("cartEmpty"));return;}
    var fee=_getShipFee(wilaya,dt);
    ...
    var _mystExp=0;try{var _me=localStorage.getItem("store_disc_exp");if(_me)_mystExp=parseInt(_me)||0;}catch(e){}
    ...
    .then(function(data){
      if(btn){btn.disabled=false;btn.innerHTML=_t("confirmOrder");}
      if(!data.ok){_toast(_t("toastOrderErr")+(data.error||_t("toastTryAgain")));return;}
      ...
      var date=new Date().toLocaleDateString(_dateLocale(),{day:"2-digit",month:"2-digit",year:"numeric"});
      var ih="<div style='text-align:center;margin-bottom:18px;padding-bottom:14px;border-bottom:2px solid #111'><div class='inv-brand'>"+_esc(_stName)+"</div><div class='inv-sub'>Invoice / فاتورة</div></div>"
        +"<div class='inv-grid'>"
        +"<div class='inv-f'><small>"+_t("invOrderNo")+"</small><span>"+_esc(oid)+"</span></div>"
        +"<div class='inv-f'><small>"+_t("invDate")+"</small><span>"+date+"</span></div>"
        +"<div class='inv-f'><small>"+_t("invName")+"</small><span>"+_esc(name)+"</span></div>"
        +"<div class='inv-f'><small>"+_t("invPhone1")+"</small><span>"+_esc(p1)+"</span></div>"
        +"<div class='inv-f'><small>"+_t("invPhone2")+"</small><span>"+_esc(p2)+"</span></div>"
        +"<div class='inv-f'><small>"+_t("invEmail")+"</small><span>"+(em?_esc(em):"—")+"</span></div>"
        +"<div class='inv-f' style='grid-column:1/-1'><small>"+_t("invWilayaCommune")+"</small><span>"+_esc(wilaya)+" — "+_esc(commune)+"</span></div>"
        +"<div class='inv-f' style='grid-column:1/-1'><small>"+_t("invDeliveryLbl")+"</small><span>"+_esc(dlbl)+"</span></div>"
        +"<div class='inv-f' style='grid-column:1/-1'><small>"+_t("invPaymentLbl")+"</small><span>"+(payMethod==="ccp"?_t("payCcp")+(ccpRef?" — "+_esc(ccpRef):""):_t("payCod"))+"</span></div>"
        +"</div>";
      ...
      ih+="<div class='inv-tots'><div class='inv-row'><span>"+_t("opProducts")+"</span><span>"+_fmt(rawSub)+"</span></div>"
        +(srvDiscAmt>0?"<div class='inv-row'><span style='color:#16a34a'>"+_t("invDiscPrefix")+srvGlobalDisc+"%</span><span style='color:#16a34a'>-"+_fmt(srvDiscAmt)+"</span></div>":"")
        +"<div class='inv-row'><span>"+_t("invDeliveryLbl")+"</span><span style='color:#111111'>"+_fmt(srvFee)+"</span></div>"
        +(srvCcpDisc>0?"<div class='inv-row'><span style='color:#16a34a'>"+_t("invCcpDiscLbl")+"</span><span style='color:#16a34a'>-"+_fmt(srvCcpDisc)+"</span></div>":"")
        +"<div class='inv-main'><span>TOTAL</span><span>"+_fmt(srvTotal)+"</span></div></div>"
        +"<div class='inv-note'>"+_t("invNote")+"</div>"
      ...
      if(invDone)invDone.onclick=function(){
        ...
        try{
          var mystExp=localStorage.getItem("store_disc_exp");
          if(mystExp){localStorage.removeItem("store_disc_val");localStorage.removeItem("store_disc_exp");_globalDiscount=(_adminDiscountCache&&_adminDiscountCache>0)?_adminDiscountCache:0;}
        }catch(e){}
      };
      _closeMod("checkout-mod");_openMod("inv-mod");
    })
    .catch(function(){if(btn){btn.disabled=false;btn.innerHTML=_t("confirmOrder");}  _toast(_t("toastConnErr"));});
  }
  ...
  function _doTrack(){
    ...
    if(!inp){_toast(_t("toastEnterOrderOrPhone"));return;}
    if(btn){btn.disabled=true;btn.innerHTML="<span class='spin'></span>";}
    var body=/^[A-Z]+-\d+$/.test(inp)?{orderId:inp}:{phone:inp};
    fetch("/api/track",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body)})
    .then(function(r){return r.json();})
    .then(function(d){
      if(btn){btn.disabled=false;btn.innerHTML=_t("trackBtn");}
      if(!res)return;
      if(!d.ok){res.innerHTML="<div class='track-status processing'><div class='track-label'>"+_t("trackResultLbl")+"</div><div class='track-val'>"+_esc(d.msg)+"</div></div>";return;}
      var cls=d.status==="delivered"?"delivered":d.status==="shipped"?"shipped":"processing";
      res.innerHTML="<div class='track-status "+cls+"'><div class='track-label'>"+_esc(d.status?_t(d.status):"")+"</div>"
        +"<div class='track-val' style='font-size:12px;margin-top:5px'>"+_esc(d.id)+" — "+_esc(d.wilaya||"")+"</div>"
        +"<div style='font-size:10px;margin-top:4px;opacity:.7'>"+new Date(d.date).toLocaleDateString(_dateLocale())+"</div></div>";
    })
    .catch(function(){if(btn){btn.disabled=false;btn.innerHTML=_t("trackBtn");}  _toast(_t("toastConnErr"));});
  }
  ...
  function _doLogin(){
    ...
    var rmCheck0=document.getElementById("rm-check");
    fetch("/api/auth",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({password:pass,remember:!!(rmCheck0&&rmCheck0.checked)})})
    .then(function(r){return r.json();})
    .then(function(data){
  ...
  function _showAdm(){
    ...
    _startClock();_loadAnalytics();_loadAdmProds();_checkPushStatus();_loadSalesCounter();
  }
  function _closeAdm(){
    if(_adminToken){
  ...
  function _requestPush(){
    ...
    Notification.requestPermission().then(function(p){
      ...
      navigator.serviceWorker.register("/sw.js").then(function(reg){
        if(reg.showNotification)reg.showNotification(_stName,{body:"الإشعارات المحلية مفعلة"});
  ...
  function _aTab(name,el){
    ...
    if(name==="activity"){_loadActivity();_loadErrorLog();}
  ...
  function _loadAnalytics(){
    _api("/api/analytics").then(function(r){return r.json();}).then(function(d){
      ...
      if(kpiRow){
        ...
        var shipCost=0;
        try{var _sc=parseFloat(localStorage.getItem("store_ship_cost")||"0");shipCost=_sc||0;}catch(e){}
      ...
      (function(){
        ...
        for(var yi=0;yi<=4;yi++){
          ...
          yLabels+="<text x='"+(PL-4)+"' y='"+(yp+4)+"' text-anchor='end' font-size='8' fill='rgba(0,0,0,.28)'>"+
            (yv>=1000?Math.round(yv/1000)+"k":yv)+"</text>"+
            "<line x1='"+PL+"' y1='"+yp+"' x2='"+(PL+IW)+"' y2='"+yp+"' stroke='rgba(0,0,0,.05)' stroke-width='1'/>";
        }
        // x labels (every 2nd)
        var xLabels="";
        pts.forEach(function(p,i){
          if(i%2===0)xLabels+="<text x='"+p.cx+"' y='"+(H-8)+"' text-anchor='middle' font-size='8' fill='rgba(0,0,0,.32)'>"+p.lbl+"</text>";
        });
        // dots + tooltips
        var dots="";
        pts.forEach(function(p){
          dots+="<circle cx='"+p.cx+"' cy='"+p.cy+"' r='3.5' fill='#1a1a1a' stroke='rgba(10,0,30,.8)' stroke-width='1.5'>"+
        ...
        var svg="<svg xmlns='http://www.w3.org/2000/svg' width='"+W+"' height='"+H+"' style='min-width:"+W+"px'>"+
          ...
          "<stop offset='100%' stop-color='rgba(0,0,0,.03)'/></linearGradient></defs>"+
          yLabels+xLabels+
          "<path d='"+areaD+"' fill='url(#sg)'/>"+
          "<path d='"+lineD+"' fill='none' stroke='rgba(0,0,0,.8)' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/>"+
          dots+"</svg>";
      ...
      if(bc&&d.brandMap){
        ...
        bc.innerHTML=brands.map(function(e){
          ...
          return "<div style='display:flex;align-items:center;gap:8px;margin-bottom:6px'>"
            ...
            +"<div style='flex:1;background:rgba(0,0,0,.08);border-radius:3px;height:8px'>"
            +"<div style='width:"+pct+"%;height:100%;background:linear-gradient(90deg,rgba(0,0,0,.7),rgba(0,0,0,.5));border-radius:3px'></div></div>"
      ...
      if(dc&&d.devMap){
        ...
        dc.innerHTML=devs.map(function(e){
          ...
          return "<div style='display:flex;align-items:center;gap:8px;margin-bottom:6px'>"
            ...
            +"<div style='flex:1;background:rgba(0,0,0,.08);border-radius:3px;height:7px'>"
            +"<div style='width:"+pct+"%;height:100%;background:linear-gradient(90deg,rgba(0,0,0,.7),rgba(0,0,0,.4));border-radius:3px'></div></div>"
      ...
      if(hc&&d.hourMap){
        ...
        for(var h=0;h<24;h++){
          ...
          bars+="<div title='"+h+":00 - "+v+" \u0632\u064a\u0627\u0631\u0629' style='display:inline-flex;flex-direction:column;align-items:center;gap:2px;width:3.8%'>"
            +"<div style='height:"+(pct*0.5)+"px;background:rgba(0,0,0,.5);border-radius:2px 2px 0 0;width:100%;min-height:2px'></div>"
          ...
          wc.innerHTML="<div style='margin-bottom:4px;font-size:9px;color:var(--mu)'>\u0637\u0644\u0628\u064a\u0627\u062a \u0645\u0624\u0643\u062f\u0629 \u062d\u0633\u0628 \u0627\u0644\u0648\u0644\u0627\u064a\u0629</div>"+
            sortedW.map(function(e){
              ...
              return "<div style='display:flex;align-items:center;gap:7px;margin-bottom:5px'>"
                ...
                +"<div style='flex:1;background:rgba(0,0,0,.06);border-radius:3px;height:10px;position:relative'>"
                +"<div style='width:"+pct+"%;height:100%;background:linear-gradient(90deg,rgba(0,0,0,.65),rgba(0,0,0,.4));border-radius:3px;transition:width .4s'></div></div>"
                +"<div style='font-size:9px;color:var(--mu);width:40px;text-align:left'>"+e[1].cnt+" \u0637</div></div>";
            }).join("")+
            (d.bestProd?"<div style='margin-top:8px;font-size:10px;color:var(--dim)'>\u0623\u0643\u062b\u0631 \u0645\u0646\u062a\u062c \u0645\u0628\u064a\u0639\u0627: <span style='color:rgba(0,0,0,.9)'>"+_esc(d.bestProd.name)+"</span> ("+d.bestProd.qty+" \u0642\u0637\u0639\u0629)</div>":"");
        } else {
      ...
      (function(){
        ...
        WILAYAS.forEach(function(w){
          ...
          var strokeColor=cnt>0?"rgba(0,0,0,.35)":"rgba(0,0,0,.12)";
          circles+="<circle cx='"+w.x+"' cy='"+w.y+"' r='"+r+"' fill='"+fillColor+"' stroke='"+strokeColor+"' stroke-width='1'>"
            +"<title>"+w.n+" — "+cnt+" \u0637\u0644\u0628\u064a\u0629</title></circle>";
          if(cnt>0){
            labels+="<text x='"+(w.x+r+2)+"' y='"+(w.y+4)+"' font-size='7' fill='rgba(0,0,0,.75)'>"+_esc(w.n)+"</text>";
          }
        });
        // legend
        legend="<g transform='translate(10,295)'>"
          +"<text x='0' y='0' font-size='8' fill='rgba(0,0,0,.32)'>\u0627\u0644\u062d\u062c\u0645 \u064a\u0639\u0643\u0633 \u0639\u062f\u062f \u0627\u0644\u0637\u0644\u0628\u064a\u0627\u062a</text>"
          +"<circle cx='5' cy='12' r='3' fill='rgba(0,0,0,.15)' stroke='rgba(0,0,0,.3)' stroke-width='1'/>"
          +"<text x='12' y='16' font-size='8' fill='rgba(0,0,0,.28)'>\u0644\u0627 \u0637\u0644\u0628\u064a\u0627\u062a</text>"
          +"<circle cx='65' cy='12' r='8' fill='rgba(239,108,148,.7)' stroke='rgba(0,0,0,.6)' stroke-width='1'/>"
          +"<text x='77' y='16' font-size='8' fill='rgba(0,0,0,.28)'>\u0623\u0643\u062b\u0631 \u0637\u0644\u0628\u064a\u0627\u062a</text></g>";
        var svg="<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 "+W+" "+H+"' width='"+W+"' height='"+H+"' style='min-width:"+W+"px'>"
          +"<rect width='"+W+"' height='"+H+"' fill='rgba(0,0,0,.02)' rx='8'/>"
  ...
  function _loadKvStats(){
    ...
    _api("/api/kv-stats").then(function(r){return r.json();}).then(function(d){
      ...
      if(c)c.innerHTML="<div style='margin-bottom:8px'>"+_fmt(Math.round(d.usedMB||0))+" MB / "+_fmt(Math.round(d.totalMB||1024))+" MB — "+pct+"%</div>"
        +"<div style='height:7px;background:rgba(0,0,0,.08);border-radius:999px;overflow:hidden;margin-bottom:8px'><div style='height:100%;width:"+Math.min(100,pct)+"%;background:linear-gradient(90deg,#22c55e,#1a1a1a)'></div></div>"+keys;
    ...
    _api("/api/products").then(function(r){return r.json();}).then(function(data){
      ...
      tb.innerHTML=data.map(function(p){
        ...
        return "<tr class='aprd-row' draggable='true' data-pid='"+p.id+"'><td>"+(img?"<img class='ath' src='"+_esc(img)+"' loading='lazy'>":"—")+"</td>"
             +"<td>"+_esc(p.name)+"</td>"
             +"<td style='font-family:Georgia,serif;color:rgba(0,0,0,.8)'>"+_fmt(_effPrice(p))+"</td>"
    ...
    function _flushPendingNotes(){
      try{
        ...
        for(var i=0;i<localStorage.length;i++){
          ...
          if(k&&k.indexOf("store_note_")===0)keys.push(k);
        }
        if(!keys.length)return;
        var synced=0,failed=0,total=keys.length;
        keys.forEach(function(k){
          var oid=k.replace("store_note_","");
  ...
  function _loadOrders(){
    ...
    _api("/api/orders").then(function(r){return r.json();}).then(function(orders){
        ...
        return "<div class='oc'><div class='oc-h'><span class='oc-id'>"+_esc(o.id)+"</span>"
              ...
              +(o.auditLog&&o.auditLog.length
                ...
                +"<div>"+o.auditLog.map(function(en){
                  return "<div style='font-size:9px;color:var(--mu);padding:3px 6px;border-bottom:1px solid rgba(0,0,0,.04)'>"
                    +new Date(en.t).toLocaleString("ar-DZ")+" — "
                    +en.changes.map(function(ch){return _esc(String(ch.f||""))+(ch.from!==undefined?": "+_esc(String(ch.from))+" → "+_esc(String(ch.to)):"");}).join(" · ")+"</div>";
                }).join("")+"</div></details>":"")
              +histHtml
              +"<div class='oc-ft'><span style='font-family:Georgia,serif;color:rgba(0,0,0,.9)'>"+_fmt(o.total)+"</span>"
              ...
              +"<button class='aact' data-invoice='"+_esc(o.id)+"' title='INV' style='background:rgba(0,0,0,.12);border-color:rgba(0,0,0,.3)'></button>"
      ...
      c.querySelectorAll(".ord-note-inp").forEach(function(ta){
        ...
        try{
          var draft=localStorage.getItem("store_note_"+oid);
        ...
        function _syncNote(val){
          if(!navigator.onLine){
            ...
            try{localStorage.setItem("store_note_"+oid,val);}catch(e){}
            ta.style.borderColor="rgba(251,191,36,.5)";
          ...
          _api("/api/orders",{method:"PATCH",body:JSON.stringify({id:oid,note:val})})
            .then(function(){
              try{localStorage.removeItem("store_note_"+oid);}catch(e){}
        ...
        ta.addEventListener("input",function(){
          ta.style.borderColor="rgba(0,0,0,.35)";
          ta.title="";
          /* حفظ draft في localStorage فوراً لمنع فقدان البيانات */
          try{localStorage.setItem("store_note_"+oid,ta.value);}catch(e){}
      ...
      c.querySelectorAll("[data-invoice]").forEach(function(btn){
        btn.addEventListener("click",function(){
          var _iid=btn.getAttribute("data-invoice");
          _api("/api/url-token",{method:"POST"}).then(function(r){return r.json();}).then(function(d){if(d.token)window.open("/invoice?id="+encodeURIComponent(_iid)+"&key="+encodeURIComponent(d.token),"_blank");}).catch(function(){_toast("خطأ");});
        });
      });
      c.querySelectorAll("[data-shiplbl]").forEach(function(btn){
        btn.addEventListener("click",function(){
          var _lid=btn.getAttribute("data-shiplbl");
          _api("/api/url-token",{method:"POST"}).then(function(r){return r.json();}).then(function(d){if(d.token)window.open("/shipping-label?id="+encodeURIComponent(_lid)+"&key="+encodeURIComponent(d.token),"_blank");}).catch(function(){_toast("خطأ");});
        });
      });
  ...
  function _renderOrders(orders){
    ...
    var toolbar="<div id='bulk-toolbar' style='display:flex;align-items:center;gap:8px;padding:7px 10px;margin-bottom:8px;background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.1);border-radius:9px;flex-wrap:wrap;position:sticky;top:0;z-index:2;backdrop-filter:blur(8px)'>"
      +"<label style='display:flex;align-items:center;gap:5px;font-size:11px;color:var(--mu);cursor:pointer'>"
      +"<input type='checkbox' id='bulk-sel-all' style='accent-color:#1a1a1a;cursor:pointer'> تحديد الكل</label>"
      +"<span id='bulk-count' style='font-size:10px;color:rgba(0,0,0,.7)'>0 محدد</span>"
    ...
    c.innerHTML=toolbar+orders.map(function(o){
      ...
      return "<div class='oc'><div class='oc-h'>"
        ...
        +" style='accent-color:#1a1a1a;cursor:pointer;flex-shrink:0'></label>"
        ...
        +"<div class='oc-pl'>"+ih+"</div>"
        +"<div class='oc-ft'>"
        +"<span style='font-family:Georgia,serif;color:rgba(0,0,0,.9)'>"+_fmt(o.total||0)+"</span>"
        +"<select class='status-sel' data-oid='"+_esc(o.id)+"'>"+stOpts+"</select>"
        +(o.confirmed
        ...
        +(o.auditLog&&o.auditLog.length
          ?"<details class='audit-trail'><summary style='font-size:9px;color:var(--mu);cursor:pointer;padding:5px 10px;list-style:none'>سجل التغييرات ("+o.auditLog.length+")</summary>"
            ...
            +o.auditLog.map(function(ae){
              ...
              return "<div style='font-size:9px;color:var(--mu);padding:2px 0;border-bottom:1px solid rgba(0,0,0,.04)'>"
                +"<span style='color:rgba(0,0,0,.5)'>"+dt+"</span>"
                +(chg?"<span style='margin-right:6px;color:rgba(0,0,0,.4)'>"+chg+"</span>":"")
                +"</div>";
            }).join("")
            +"</div></details>"
          :"")
    ...
    if(pbtn){
      pbtn.addEventListener("click",function(){
        ...
        w.document.write(
          ...
          +'</style></head><body>'
          +'<div class="ctrl">'
          +'<button onclick="window.print()" style="background:#111;color:#fff;border-color:#111">طباعة الكل ('+ids.length+' فاتورة)</button>'
          +'<button onclick="window.close()" style="background:#eee;color:#333">إغلاق</button>'
          +'</div>'
          ...
          +'d.innerHTML=m?m[1].replace(/<scr'+'ipt[\\s\\S]*?<\\/scr'+'ipt>/gi,""):h;'
    ...
    c.querySelectorAll("[data-invoice]").forEach(function(btn){
      btn.addEventListener("click",function(){
        var _iid2=btn.getAttribute("data-invoice");
        _api("/api/url-token",{method:"POST"}).then(function(r){return r.json();}).then(function(d){if(d.token)window.open("/invoice?id="+encodeURIComponent(_iid2)+"&key="+encodeURIComponent(d.token),"_blank");}).catch(function(){_toast("خطأ");});
      });
    });
    c.querySelectorAll("[data-delord]").forEach(function(btn){
      btn.addEventListener("click",function(){
  ...
  function _groupOrders(){
    ...
    var html="<div style='margin-bottom:9px;padding:8px 11px;background:rgba(0,0,0,.06);border:1px solid rgba(0,0,0,.12);border-radius:9px;display:flex;justify-content:space-between;flex-wrap:wrap;gap:6px;font-size:10px;color:var(--mu)'>"
      +"<span>"+sorted.length+" ولاية — "+source.length+" طلبية</span>"
      +"<span style='color:rgba(0,0,0,.8)'>"+_fmt(totalRev)+" إيراد مؤكد</span>"
    ...
    html+=sorted.map(function(entry){
      ...
      var smry="<span style='font-size:12px;color:rgba(0,0,0,.9);font-weight:600'>"+_esc(wn)+"</span>"
        +"<span style='background:rgba(0,0,0,.12);border:1px solid rgba(0,0,0,.2);border-radius:12px;padding:1px 8px;font-size:10px;color:rgba(0,0,0,.7)'>"+d.total+" طلبية</span>"
        ...
        +"<div style='flex:1;min-width:50px;height:3px;background:rgba(0,0,0,.08);border-radius:2px;overflow:hidden'>"
        +"<div style='width:"+pct+"%;height:100%;background:"+pc+";border-radius:2px'></div></div>";
      var rows=d.orders.map(function(o){
        var sc={processing:"rgba(251,191,36,.75)",shipped:"rgba(59,130,246,.75)",delivered:"rgba(74,222,128,.75)",returned:"rgba(239,68,68,.75)"}[o.status]||"rgba(0,0,0,.6)";
        return "<div style='display:grid;grid-template-columns:auto 1fr auto auto;gap:6px;align-items:center;padding:6px 10px;border-bottom:1px solid rgba(0,0,0,.04);font-size:10px'>"
          +"<span style='font-family:monospace;color:rgba(0,0,0,.5);font-size:9px'>"+_esc((o.id||"").slice(-6))+"</span>"
          +"<div><div style='color:rgba(0,0,0,.6)'>"+_esc(o.name||"")+"</div>"
          +"<div style='color:var(--mu);font-size:9px'>"+_esc(o.phone1||"")+" · "+_esc(o.commune||"")+"</div></div>"
          +"<span style='font-family:Georgia,serif;color:rgba(0,0,0,.8);white-space:nowrap'>"+_fmt(o.total||0)+"</span>"
      ...
      return "<details class='wly-acc' style='margin-bottom:5px;background:rgba(0,0,0,.025);border:1px solid rgba(0,0,0,.09);border-radius:10px;overflow:hidden'>"
        ...
        +smry+"</summary>"
        +"<div style='border-top:1px solid rgba(0,0,0,.07)'>"+rows+"</div>"
        +"</details>";
    }).join("");
    var c=document.getElementById("orders-c");if(!c)return;
    c.innerHTML=html;
  ...
  function _showCsvCols(){
    ...
    var html="<div style='position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:10000;display:flex;align-items:center;justify-content:center' id='csv-col-ov'>"
      +"<div style='background:#f5f5f5;border:1px solid rgba(0,0,0,.3);border-radius:12px;padding:18px 20px;max-width:320px;width:90%'>"
      +"<div style='font-size:12px;color:rgba(0,0,0,.9);margin-bottom:12px'>اختر الأعمدة</div>"
      +"<div style='display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:12px'>"
      +cols.map(function(k){
        return "<label style='display:flex;align-items:center;gap:5px;font-size:11px;color:var(--mu);cursor:pointer'>"
          +"<input type='checkbox' class='csv-col-chk' data-k='"+k+"' checked style='accent-color:#1a1a1a'> "+(labels[k]||k)+"</label>";
      }).join("")+"</div>"
      +"<div style='display:flex;gap:7px'>"
  ...
  function _exportCSV(cols){
    ...
    _api("/api/analytics").then(function(r){return r.json();}).then(function(d){
      ...
      var brHTML="<div style='margin-bottom:12px'><div style='font-size:10px;color:var(--mu);margin-bottom:6px'>توزيع المتصفحات</div>"
        +browsers.slice(0,6).map(function(e){
          ...
          return "<div style='display:flex;align-items:center;gap:7px;margin-bottom:5px'>"
            ...
            +"<div style='flex:1;background:rgba(0,0,0,.06);border-radius:3px;height:7px'><div style='width:"+pct+"%;height:100%;background:rgba(0,0,0,.5);border-radius:3px'></div></div>"
      ...
      var visHTML=vs.slice(0,50).map(function(v){
        var tc=TIER_C[v.tier]||"rgba(0,0,0,.6)";
        return "<div class='vr' style='display:grid;grid-template-columns:auto 1fr auto auto;gap:8px;align-items:center;padding:6px 0;border-bottom:1px solid rgba(0,0,0,.05)'>"
          +"<span style='font-family:monospace;font-size:9px;color:rgba(0,0,0,.5)'>"+_esc((v.vid||"").slice(-6))+"</span>"
          +"<div><div style='font-size:10px;color:rgba(0,0,0,.6)'>"+_esc(v.dev||"")+" · "+_esc(v.browser||"")+"</div>"
          ...
          +"<span style='font-size:11px;font-family:Georgia,serif;color:rgba(0,0,0,.8)'>"+v.count+"</span></div>";
      }).join("");
  ...
  function _loadSettings(onDone){
    _api("/api/settings").then(function(r){return r.json();}).then(function(s){
      ...
      _stName=s.storeName||"STORE_NAME";
      _updateMeta(_stName,"");
  ...
  function _saveSettings(){
    ...
    _api("/api/settings",{method:"POST",body:JSON.stringify(body)}).then(function(){
      ...
      _stName=body.storeName||"STORE_NAME";
      _updateMeta(_stName,"");
      ...
      if(body.admin_discount>0){
        ...
        if(_globalDiscount>0){
          var mystExp=null;try{mystExp=localStorage.getItem("store_disc_exp");}catch(e){}
  ...
  /* رسوم الشحن محقونة من SF (مستوى الوحدة) — عدّل SF أعلاه فقط */
  var SHIP_FEES=${JSON.stringify(SF)};
  ...
  function _showMystery(){
    try{
      ...
      if(sessionStorage.getItem("store_s_shown")==="1")return;
      // تحقق من آخر مرة عُرض/رُفض (20 يوماً)
      var last=localStorage.getItem("store_myst_ts");
      if(last){
    ...
    var codes=["FIRST"+d,"STYLE"+d,"DEAL"+d,"SAVE"+d];
    ...
    if(acceptBtn){
      acceptBtn.onclick=function(){
        try{
          localStorage.setItem("store_disc_val",String(d));
          localStorage.setItem("store_disc_exp",String(Date.now()+4*60*60*1000));
          localStorage.setItem("store_myst_ts",Date.now().toString());
        }catch(e){}
        _globalDiscount=d;
    ...
    if(skipBtn){
      skipBtn.onclick=function(){
        try{localStorage.setItem("store_myst_ts",Date.now().toString());}catch(e){}
        _closeMod("mystery-mod");
      };
    }
    _openMod("mystery-mod");
    try{sessionStorage.setItem("store_s_shown","1");}catch(e){}
  }
  function _restoreDiscount(){
    try{
      var val=localStorage.getItem("store_disc_val");
      var exp=localStorage.getItem("store_disc_exp");
      if(val&&exp&&Date.now()<parseInt(exp)){
        _globalDiscount=parseInt(val)||0;
      } else {
        localStorage.removeItem("store_disc_val");
        localStorage.removeItem("store_disc_exp");
    ...
    _api("/api/coupons").then(function(r){return r.json();}).then(function(coupons){
      ...
      c.innerHTML=coupons.map(function(cp){
        ...
        return "<div class='coup-row"+(inactive?" coup-expired":"")+"'>"
          ...
          +(cp.wilayaList&&cp.wilayaList.length?"<br><span style='font-size:9px;color:rgba(0,0,0,.6)'>ولايات: "+cp.wilayaList.slice(0,3).join(",")+( cp.wilayaList.length>3?"...":"")+"</span>":"")
    ...
    _api("/api/products/archive").then(function(r){return r.json();}).then(function(prods){
      ...
      c.innerHTML=prods.map(function(p){
        ...
        return "<div class='arch-row'>"
          ...
          +"<input type='checkbox' class='arch-chk' data-pid='"+p.id+"' style='accent-color:#1a1a1a;cursor:pointer'></label>"
          +(img?"<img src='"+_esc(img)+"' style='width:44px;height:54px;object-fit:cover;border-radius:5px'>":"<div style='width:44px;height:54px;background:rgba(0,0,0,.06);border-radius:5px'></div>")
  ...
  function _loadStockHistory(prodId){
    ...
    _api(url).then(function(r){return r.json();}).then(function(hist){
      ...
      c.innerHTML="<div style='background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:9px;overflow:hidden'>"
        +hist.map(function(h){
          ...
          return "<div class='sh-row'>"
            ...
            +"<div style='font-size:9px;background:rgba(0,0,0,.05);padding:2px 7px;border-radius:4px;color:var(--mu)'>"+_esc(h.type||"")+"</div>"
            +"</div>";
        }).join("")+"</div>";
  ...
  function _loadActivity(){
    ...
    _api("/api/activity-log").then(function(r){return r.json();}).then(function(log){
      ...
      var colors={order_status:"rgba(0,0,0,.8)",coupon_create:"rgba(74,222,128,.8)",product_archive:"rgba(251,191,36,.8)",product_restore:"rgba(34,197,94,.8)",stock_add:"rgba(56,189,248,.8)",visits_cleanup:"rgba(239,68,68,.8)",product_delete:"rgba(239,68,68,.8)"};
      c.innerHTML="<div style='background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:9px;overflow:hidden'>"
        +log.map(function(a){
          var col=colors[a.type]||"rgba(0,0,0,.7)";
  ...
  function _loadErrorLog(){
    var c=document.getElementById("errlog-c");if(!c)return;
    c.innerHTML="<div style='color:var(--mu);font-size:12px;padding:10px'><span class='spin'></span> Loading...</div>";
    _api("/api/error-log").then(function(r){return r.json();}).then(function(log){
      if(!log.length){c.innerHTML="<div style='color:var(--mu);font-size:12px;padding:10px'>لا توجد أخطاء مسجلة 👍</div>";return;}
      c.innerHTML="<div style='background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:9px;overflow:hidden'>"
        +log.map(function(a){
          return "<div class='act-row' title='"+_esc(a.stack||"")+"'>"
            +"<div class='act-t'>"+_esc((a.t||"").slice(0,16).replace("T"," "))+"</div>"
            +"<div class='act-type' style='background:rgba(239,68,68,.15);color:rgba(239,68,68,.9)'>"+_esc(a.method||"")+" "+_esc(a.path||"")+"</div>"
            +"<div class='act-details'>"+_esc(a.msg||"")+"</div>"
            +"</div>";
        }).join("")+"</div>";
    }).catch(function(){c.innerHTML="<div style='color:rgba(239,68,68,.7)'>خطأ</div>";});
  }
  function _clearErrorLog(){
    if(!confirm("سيتم حذف كل سجلات الأخطاء نهائياً. متابعة؟"))return;
    _api("/api/error-log",{method:"DELETE"}).then(function(){_loadErrorLog();}).catch(function(){});
  }

  function _translateAllProds(){
    var btn=document.getElementById("translate-all-btn");
    var st=document.getElementById("translate-status");
    if(btn){btn.disabled=true;btn.innerHTML="<span class='spin'></span> جارٍ الترجمة...";}
    if(st)st.innerHTML="<div style='color:var(--mu);font-size:12px;padding:8px 0'>يتم إرسال المنتجات إلى Claude Haiku... قد يستغرق 10-20 ثانية</div>";
    _api("/api/translate-products",{method:"POST"}).then(function(r){return r.json();}).then(function(d){
      if(btn){btn.disabled=false;btn.innerHTML="&#127760; ترجمة الكل";}
      if(st){
        if(d.ok)st.innerHTML="<div style='color:rgba(34,197,94,.8);font-size:12px;padding:8px 0'>✓ تمت ترجمة "+d.count+" منتج بنجاح</div>";
        else st.innerHTML="<div style='color:rgba(239,68,68,.8);font-size:12px;padding:8px 0'>✗ خطأ: "+_esc(d.error||"")+"</div>";
      }
    }).catch(function(){
      if(btn){btn.disabled=false;btn.innerHTML="&#127760; ترجمة الكل";}
      if(st)st.innerHTML="<div style='color:rgba(239,68,68,.8);font-size:12px;padding:8px 0'>✗ خطأ في الاتصال</div>";
    });
  }
  ...
  function _loadBundlesStorefront(){
    _api("/api/bundles").then(function(r){return r.json();}).then(function(bundles){
      ...
      grid.innerHTML=active.map(function(b){
        ...
        return "<div class='bundle-card' onclick='STORE._openBundle("+JSON.stringify(b)+")'>"
  ...
  function _loadWaitlist(){
    ...
    _api("/api/waitlist").then(function(r){return r.json();}).then(function(list){
      ...
      c.innerHTML=Object.entries(byProd).map(function(e){
        return "<details style='margin-bottom:7px;background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:9px'>"
          +"<summary style='padding:10px 13px;cursor:pointer;font-size:12px;color:rgba(0,0,0,.9);font-weight:600;list-style:none'>⏳ "+_esc(e[0])+" <span style='color:var(--mu)'>("+e[1].length+")</span></summary>"
          +"<div style='padding:8px'>"+e[1].map(function(w){return "<div style='font-size:11px;color:var(--dim);padding:3px 8px;border-bottom:1px solid rgba(0,0,0,.05)'> "+_esc(w.phone)+" <span style='color:var(--mu);font-size:9px'>"+w.t.slice(0,10)+"</span></div>";}).join("")+"</div>"
          +"</details>";
      }).join("");
    }).catch(function(){});
  }
    ...
    _api("/api/loyalty").then(function(r){return r.json();}).then(function(list){
      ...
      c.innerHTML="<div style='background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:9px;overflow:hidden'>"
        +list.slice(0,100).map(function(u){
          return "<div style='display:flex;justify-content:space-between;align-items:center;padding:8px 13px;border-bottom:1px solid rgba(0,0,0,.05);font-size:11px'>"
            +"<div><div style='color:var(--dim)'>"+_esc(u.name||"")+"</div><div style='color:var(--mu);font-size:10px'>"+_esc(u.phone||"")+"</div></div>"
            +"<div class='loyalty-pts-badge' onclick='STORE._viewLoyaltyDetail(\\""+_esc(u.phone)+"\\")' style='cursor:pointer'>⭐ نقاط</div>"
            +"</div>";
        }).join("")+"</div>";
    }).catch(function(){});
  }
  ...
  function _loadReferrals(){
    ...
    _api("/api/referrals").then(function(r){return r.json();}).then(function(list){
      ...
      c.innerHTML="<div style='background:rgba(0,0,0,.025);border:1px solid var(--b1);border-radius:9px;overflow:hidden'>"
        +list.map(function(r){
          return "<div style='display:grid;grid-template-columns:1fr auto auto;gap:10px;align-items:center;padding:8px 13px;border-bottom:1px solid rgba(0,0,0,.05);font-size:11px'>"
            ...
            +"<code style='font-size:10px;color:rgba(0,0,0,.7)'>"+_esc(r.hash||"")+"</code>"
            +"</div>";
        }).join("")+"</div>";
    }).catch(function(){});
  }
    ...
    _api("/api/reviews").then(function(r){return r.json();}).then(function(list){
      ...
      c.innerHTML=list.slice(0,100).map(function(rv){
        ...
        return "<div class='rv-card"+(rv.approved?"":" rv-pending")+"'>"
          ...
          +"<button class='aact' style='font-size:9px;color:rgba(0,0,0,.8)' data-rv-to-tm='"+rv.id+"'>شهادة</button>"
    ...
    _api("/api/testimonials").then(function(r){return r.json();}).then(function(list){
      ...
      c.innerHTML=list.map(function(t){
        ...
        return "<div class='rv-card'>"
          ...
          +"<div style='display:flex;gap:5px'>"
          +(t.fromReview?"<span style='font-size:8px;color:rgba(0,0,0,.6)'>من تقييم</span>":"")
      ...
      if(sec&&sl&&list.length){
        ...
        sl.innerHTML=list.slice(0,8).map(function(t){
          var sh2="";for(var i=1;i<=5;i++)sh2+="<span style='color:"+(i<=(t.rating||5)?"rgba(251,191,36,.9)":"rgba(0,0,0,.22)")+"'>&#9733;</span>";
  ...
  function _initExitIntent(){
    ...
    try{if(localStorage.getItem("store_exit_shown_"+new Date().toLocaleDateString()))return;}catch{}
    // Desktop: mouseleave
    document.addEventListener("mouseleave",function(e){
  ...
  function _showExitIntent(){
    if(_exitShown)return;
    _exitShown=true;
    try{localStorage.setItem("store_exit_shown_"+new Date().toLocaleDateString(),"1");}catch{}
    // Show best product
    ...
    if(prev&&bestProd){
      prev.innerHTML="<div style='display:flex;align-items:center;gap:10px;padding:8px;background:rgba(0,0,0,.05);border-radius:8px;border:1px solid rgba(0,0,0,.1)'>"
        ...
        +"<div style='font-family:Georgia,serif;color:rgba(0,0,0,.9);font-size:13px'>"+_fmt(bestProd.price||0)+" دج</div></div></div>";
    }
  ...
  function _showUpsell(addedProd){
    ...
    cont.innerHTML="<div style='display:flex;gap:9px;align-items:center'>"
      ...
      +"<div style='font-family:Georgia,serif;color:rgba(0,0,0,.9);font-size:13px;margin-bottom:7px'>"+_fmt(upsellProd.price||0)+" دج</div>"
  ...
  function _loadSalesCounter(){
    ...
    _api("/api/analytics").then(function(r){return r.json();}).then(function(d){
      if(!d.ordersThisMonth)return;
      var el=document.getElementById("sales-counter-bar");
      if(el)el.textContent=d.ordersThisMonth+" طلبية مؤكدة هذا الشهر";
    }).catch(function(){});
  }
  ...
  function _showQR(prod){
    ...
    w.document.write(
      ...
      +'<style>'
      +'*{margin:0;padding:0;box-sizing:border-box}'
      +'body{font-family:-apple-system,sans-serif;text-align:center;padding:24px 16px;background:#f5f5f5;color:#fff}'
      +'.brand{font-family:Georgia,serif;font-size:22px;font-weight:900;letter-spacing:5px;color:#555555;margin-bottom:4px}'
      +'.nm{font-size:14px;color:rgba(0,0,0,.65);margin-bottom:3px}'
      +'.price{font-family:Georgia,serif;font-size:17px;color:rgba(0,0,0,.9);margin-bottom:16px}'
      +'.qw{background:#fff;padding:12px;border-radius:12px;display:inline-block;margin-bottom:12px}'
      +'.lnk{font-size:9px;color:rgba(0,0,0,.32);word-break:break-all;margin-bottom:14px;padding:0 8px}'
      ...
      +'.b1{background:#111111;color:#fff}.b2{background:rgba(0,0,0,.1);color:rgba(0,0,0,.68)}'
      +'@media print{.btns,.lnk{display:none}body{background:#fff;padding:8px}}'
      +'</style></head><body>'
      +'<div class="brand">'+_esc(_stName)+'</div>'
  ...
  function _initDragReorder(container){
    ...
    rows.forEach(function(row){
      row.addEventListener("touchstart",function(e){
        ...
        _tp.style.cssText="height:"+row.offsetHeight+"px;background:rgba(0,0,0,.06);border:1px dashed rgba(0,0,0,.25);border-radius:8px";
  ...
  function _initFullscreen(){
    ...
    try{
      if(localStorage.getItem("store_fullscreen")==="1"){
        var el=document.documentElement;
    ...
    function _onFsChange(){
      ...
      btns.forEach(function(b){if(b)b.textContent=on?"▥":"▣";});
      try{if(on)localStorage.setItem("store_fullscreen","1");else localStorage.removeItem("store_fullscreen");}catch(e){}
    }
  ...
  // ══ MULTI-LANGUAGE (م44) ══════════════════════════════════════════
  var _lang="ar";
  // ══ TRANSLATION DICTIONARY — القاموس الكامل ══════════════════════
  var _T={
    ar:{
      myCart:"CART",search:"ابحث عن منتج...",
      processing:"قيد المعالجة",shipped:"تم الشحن",delivered:"تم التوصيل",returned:"مُرتجعة",
      catAll:"الكل",catShirts:"القمصان",catPants:"البناطيل",catShorts:"الشورتات",catHats:"القبعات",catAcc:"الاكسسوارات",catOther:"اخرى",catNew:"الجديد",catTop:"الاكثر مبيعا",
      piecesCount:"قطعة",sortDef:"ترتيب افتراضي",sortLow:"السعر: الاقل اولا",sortHigh:"السعر: الاعلى اولا",
      navHome:"الرئيسية",navCart:"السلة",navTrack:"تتبع",navHelp:"مساعدة",
      footerLinks:"روابط",footerContact:"تواصل معنا",footerTrack:"تتبع طلبيتك",footerFaq:"الاسئلة الشائعة",footerPolicy:"سياسة الاستبدال",
      cartEmpty:"السلة فارغة",cartTotal:"المجموع",checkoutBtn:"اتمام الشراء &#8594;",
      noProds:"لا توجد منتجات في هذا القسم",chooseSizeAdd:"اختر المقاس واضف للسلة",addToCart:"اضف للسلة",addCartShort:"+ اضف للسلة",
      chooseSize:"اختر المقاس",directSize:"المقاس المباشر",weight:"الوزن (kg)",height:"الطول (cm)",gender:"الجنس",male:"ذكر",female:"انثى",orMeasure:"او ادخل مقاياسك",
      fomo:"متوفر قطع محدودة جداً من هذا التصميم هذا الاسبوع",fomoCard:"قطع محدودة جداً من هذا التصميم هذا الاسبوع",
      waitlistBtn:"⏳ نبهني حين يتوفر",viewing:" شخص يتصفح الآن",sold:"مباع",scarce1:"تبقى ",scarce2:" قطعة فقط",
      reviewTitle:"أضف تقييمك",reviewName:"اسمك",reviewPhone:"رقم هاتفك (للتحقق)",reviewRating:"التقييم",reviewBody:"رأيك في المنتج",reviewSubmit:"إرسال التقييم",reviewBodyPh:"شاركنا تجربتك...",reviewNamePh:"أحمد م.",
      trackTitle:"تتبع الطلبية",trackLabel:"رقم الطلبية او رقم الهاتف",trackPh:"رقم الطلبية او 05XXXXXXXX",trackBtn:"تتبع",
      step1:"المعلومات",step2:"العنوان",step3:"الدفع",step4:"المراجعة",
      lbFullName:"الاسم الكامل *",lbPhone1:"رقم الهاتف 1 *",lbPhone2:"رقم الهاتف 2 *",lbEmail:"البريد الالكتروني",
      lbWilaya:"الولاية *",lbCommune:"البلدية *",lbDelivery:"نوع التوصيل",delOffice:"للمكتب / Stop Desk",delHome:"للمنزل",
      lbPayment:"طريقة الدفع *",payCod:"الدفع عند الاستلام",payCodSub:"ادفع نقداً حين وصول طلبيتك — مجاني",
      lbCcpRef:"رقم الإيصال (اختياري)",ccpRefPh:"رقم وصل الدفع...",ccpDetailsTitle:"تفاصيل الحساب البريدي",
      ccpAccountNum:"رقم الحساب:",ccpAccountName:"الاسم:",ccpWhatsappNote:"ارسل صورة الإيصال عبر الواتساب بعد التحويل",
      payCcp:"الدفع المسبق بـ CCP",payCcpSub:"تحويل بريدي مسبق — خصم 50 دج على التوصيل",
      opProducts:"المنتجات",opDelivery:"التوصيل",opDiscount:"خصم العرض",opCouponLbl:"كوبون",opCouponLabel:"كود خصم (اختياري)",opCouponPh:"مثال: SAVE10",opApply:"تطبيق",
      confirmOrder:"تاكيد الطلبية &#8594;",nextBtn:"التالي &#8592;",prevBtn:"&#8594; السابق",fullNamePh:"اكتب اسمك...",communePh:"اكتب بلديتك...",wilayaOpt0:"اختر الولاية...",
      shirts:"القمصان",pants:"البناطيل",shorts:"الشورتات",hats:"القبعات",accessories:"الاكسسوارات",other:"اخرى",
      toastPickSize:"اختر مقاساً او ادخل الوزن والطول والجنس",toastAdded:"تمت الاضافة — مقاس ",
      toastFillFields:"يرجى إكمال جميع الحقول الإلزامية",toastPhonesDiffer:"يجب ان يختلف رقما الهاتف",
      toastOrderErr:"خطا: ",toastTryAgain:"حاول مجددا",toastConnErr:"خطا في الاتصال",
      toastEnterOrderOrPhone:"ادخل رقم الطلبية او الهاتف",
      waitlistPrompt:"أدخل رقم هاتفك للتنبيه عند توفر المنتج",waitlistOk:"تم التسجيل",waitlistErr:"خطأ في التسجيل",
      invOrderNo:"رقم الطلبية",invDate:"التاريخ",invName:"الاسم",invPhone1:"هاتف 1",invPhone2:"هاتف 2",invEmail:"الايميل",
      invWilayaCommune:"الولاية / البلدية",invDeliveryLbl:"التوصيل",invPaymentLbl:"طريقة الدفع",
      invDiscPrefix:"خصم ",invCcpDiscLbl:"خصم CCP",invNote:"سوف نتصل بك لتاكيد الطلبية",
      trackResultLbl:"النتيجة"
    },
    fr:{
      myCart:"Mon panier",search:"Rechercher...",
      processing:"En traitement",shipped:"Expédié",delivered:"Livré",returned:"Retourné",
      catAll:"Tout",catShirts:"T-shirts",catPants:"Pantalons",catShorts:"Shorts",catHats:"Casquettes",catAcc:"Accessoires",catOther:"Autres",catNew:"Nouveautés",catTop:"Best-sellers",
      piecesCount:"article(s)",sortDef:"Par défaut",sortLow:"Prix croissant",sortHigh:"Prix décroissant",
      navHome:"Accueil",navCart:"Panier",navTrack:"Suivi",navHelp:"Aide",
      footerLinks:"Liens",footerContact:"Contact",footerTrack:"Suivi commande",footerFaq:"FAQ",footerPolicy:"Politique retour",
      cartEmpty:"Panier vide",cartTotal:"Total",checkoutBtn:"Commander &#8594;",
      noProds:"Aucun produit dans cette catégorie",chooseSizeAdd:"Choisir taille et ajouter",addToCart:"Ajouter au panier",addCartShort:"+ Ajouter",
      chooseSize:"Choisir la taille",directSize:"Taille directe",weight:"Poids (kg)",height:"Taille (cm)",gender:"Genre",male:"Homme",female:"Femme",orMeasure:"Ou entrez vos mesures",
      fomo:"Stock très limité cette semaine",fomoCard:"Stock très limité cette semaine",
      waitlistBtn:"⏳ M'alerter",viewing:" personnes regardent",sold:"vendu",scarce1:"Plus que ",scarce2:" en stock",
      reviewTitle:"Votre avis",reviewName:"Votre nom",reviewPhone:"Tél. (vérification)",reviewRating:"Évaluation",reviewBody:"Votre avis",reviewSubmit:"Envoyer",reviewBodyPh:"Partagez votre expérience...",reviewNamePh:"Ahmed M.",
      trackTitle:"Suivi commande",trackLabel:"N° commande ou téléphone",trackPh:"N° commande ou 05XXXXXXXX",trackBtn:"Suivre",
      step1:"Infos",step2:"Adresse",step3:"Paiement",step4:"Révision",
      lbFullName:"Nom complet *",lbPhone1:"Tél. 1 *",lbPhone2:"Tél. 2 *",lbEmail:"Email",
      lbWilaya:"Wilaya *",lbCommune:"Commune *",lbDelivery:"Livraison",delOffice:"Bureau / Stop Desk",delHome:"À domicile",
      lbPayment:"Mode de paiement *",payCod:"Paiement à la livraison",payCodSub:"Payez en espèces à la livraison — gratuit",
      lbCcpRef:"N° de reçu (facultatif)",ccpRefPh:"N° du reçu de paiement...",ccpDetailsTitle:"Détails du compte postal",
      ccpAccountNum:"N° de compte :",ccpAccountName:"Nom :",ccpWhatsappNote:"Envoyez la photo du reçu via WhatsApp après le virement",
      payCcp:"Prépaiement CCP",payCcpSub:"Virement postal — remise 50 DA",
      opProducts:"Produits",opDelivery:"Livraison",opDiscount:"Remise",opCouponLbl:"Coupon",opCouponLabel:"Code promo (facultatif)",opCouponPh:"Ex: SAVE10",opApply:"Appliquer",
      confirmOrder:"Confirmer &#8594;",nextBtn:"Suivant &#8594;",prevBtn:"&#8592; Retour",fullNamePh:"Votre nom...",communePh:"Votre commune...",wilayaOpt0:"Choisir wilaya...",
      shirts:"T-shirts",pants:"Pantalons",shorts:"Shorts",hats:"Casquettes",accessories:"Accessoires",other:"Autres",
      toastPickSize:"Choisissez une taille ou entrez poids/taille/genre",toastAdded:"Ajouté — taille ",
      toastFillFields:"Veuillez remplir tous les champs obligatoires",toastPhonesDiffer:"Les deux numéros doivent différer",
      toastOrderErr:"Erreur : ",toastTryAgain:"Réessayez",toastConnErr:"Erreur de connexion",
      toastEnterOrderOrPhone:"Entrez le n° de commande ou le téléphone",
      waitlistPrompt:"Entrez votre numéro pour être alerté",waitlistOk:"Inscription confirmée",waitlistErr:"Erreur d'inscription",
      invOrderNo:"N° commande",invDate:"Date",invName:"Nom",invPhone1:"Tél. 1",invPhone2:"Tél. 2",invEmail:"Email",
      invWilayaCommune:"Wilaya / Commune",invDeliveryLbl:"Livraison",invPaymentLbl:"Mode de paiement",
      invDiscPrefix:"Remise ",invCcpDiscLbl:"Remise CCP",invNote:"Nous vous contacterons pour confirmer votre commande",
      trackResultLbl:"Résultat"
    },
    en:{
      myCart:"My Cart",search:"Search...",
      processing:"Processing",shipped:"Shipped",delivered:"Delivered",returned:"Returned",
      catAll:"All",catShirts:"T-Shirts",catPants:"Pants",catShorts:"Shorts",catHats:"Hats",catAcc:"Accessories",catOther:"Other",catNew:"New",catTop:"Best Sellers",
      piecesCount:"item(s)",sortDef:"Default",sortLow:"Price: Low to High",sortHigh:"Price: High to Low",
      navHome:"Home",navCart:"Cart",navTrack:"Track",navHelp:"Help",
      footerLinks:"Links",footerContact:"Contact Us",footerTrack:"Track Order",footerFaq:"FAQ",footerPolicy:"Return Policy",
      cartEmpty:"Cart is empty",cartTotal:"Total",checkoutBtn:"Checkout &#8594;",
      noProds:"No products in this category",chooseSizeAdd:"Choose Size & Add to Cart",addToCart:"Add to Cart",addCartShort:"+ Add to Cart",
      chooseSize:"Choose Size",directSize:"Direct Size",weight:"Weight (kg)",height:"Height (cm)",gender:"Gender",male:"Male",female:"Female",orMeasure:"Or enter your measurements",
      fomo:"Very limited stock this week",fomoCard:"Very limited stock this week",
      waitlistBtn:"⏳ Notify Me",viewing:" viewing now",sold:"sold",scarce1:"Only ",scarce2:" left",
      reviewTitle:"Add Your Review",reviewName:"Your Name",reviewPhone:"Phone (verification)",reviewRating:"Rating",reviewBody:"Your Opinion",reviewSubmit:"Submit Review",reviewBodyPh:"Share your experience...",reviewNamePh:"Ahmed M.",
      trackTitle:"Track Order",trackLabel:"Order ID or phone number",trackPh:"Order ID or 05XXXXXXXX",trackBtn:"Track",
      step1:"Info",step2:"Address",step3:"Payment",step4:"Review",
      lbFullName:"Full Name *",lbPhone1:"Phone 1 *",lbPhone2:"Phone 2 *",lbEmail:"Email",
      lbWilaya:"Wilaya *",lbCommune:"Commune *",lbDelivery:"Delivery Type",delOffice:"Office / Stop Desk",delHome:"Home Delivery",
      lbPayment:"Payment Method *",payCod:"Cash on Delivery",payCodSub:"Pay cash on arrival — free",
      lbCcpRef:"Receipt No. (optional)",ccpRefPh:"Payment receipt number...",ccpDetailsTitle:"Postal Account Details",
      ccpAccountNum:"Account No.:",ccpAccountName:"Name:",ccpWhatsappNote:"Send the receipt photo via WhatsApp after the transfer",
      payCcp:"CCP Prepayment",payCcpSub:"Postal transfer — 50 DA discount",
      opProducts:"Products",opDelivery:"Delivery",opDiscount:"Discount",opCouponLbl:"Coupon",opCouponLabel:"Coupon code (optional)",opCouponPh:"e.g. SAVE10",opApply:"Apply",
      confirmOrder:"Confirm Order &#8594;",nextBtn:"Next &#8594;",prevBtn:"&#8592; Back",fullNamePh:"Your name...",communePh:"Your commune...",wilayaOpt0:"Select wilaya...",
      shirts:"T-Shirts",pants:"Pants",shorts:"Shorts",hats:"Hats",accessories:"Accessories",other:"Other",
      toastPickSize:"Choose a size or enter weight/height/gender",toastAdded:"Added — size ",
      toastFillFields:"Please fill in all required fields",toastPhonesDiffer:"The two phone numbers must differ",
      toastOrderErr:"Error: ",toastTryAgain:"Try again",toastConnErr:"Connection error",
      toastEnterOrderOrPhone:"Enter order ID or phone number",
      waitlistPrompt:"Enter your phone number to be notified",waitlistOk:"You're on the list",waitlistErr:"Registration error",
      invOrderNo:"Order No.",invDate:"Date",invName:"Name",invPhone1:"Phone 1",invPhone2:"Phone 2",invEmail:"Email",
      invWilayaCommune:"Wilaya / Commune",invDeliveryLbl:"Delivery",invPaymentLbl:"Payment Method",
      invDiscPrefix:"Discount ",invCcpDiscLbl:"CCP Discount",invNote:"We'll call you to confirm your order",
      trackResultLbl:"Result"
    }
  };
  function _t(key){return(_T[_lang]&&_T[_lang][key])||_T["ar"][key]||key;}
  // helpers: localized product name/desc (falls back to Arabic if translation not stored)
  function _pname(p){return _lang==="fr"?(p.nameFr||p.name):_lang==="en"?(p.nameEn||p.name):p.name;}
  function _pdesc(p){return _lang==="fr"?(p.descFr||p.desc):_lang==="en"?(p.descEn||p.desc):p.desc;}
  function _dateLocale(){return _lang==="fr"?"fr-DZ":_lang==="en"?"en-GB":"ar-DZ";}

  // ══ APPLY i18n — يُطبَّق على كل عناصر الواجهة الأمامية ══
  function _applyI18n(){
    function st(id,key){var e=document.getElementById(id);if(e)e.textContent=_t(key);}
    function si(id,key){var e=document.getElementById(id);if(e)e.innerHTML=_t(key);}
    function sph(id,key){var e=document.getElementById(id);if(e)e.placeholder=_t(key);}
    function sq(sel,key){var e=document.querySelector(sel);if(e)e.textContent=_t(key);}
    function sqh(sel,key){var e=document.querySelector(sel);if(e)e.innerHTML=_t(key);}
    function stn(el,key){if(!el)return;for(var i=0;i<el.childNodes.length;i++){if(el.childNodes[i].nodeType===3){el.childNodes[i].textContent=_t(key);return;}}}
    // ── فئات ──
    st("pill-all","catAll");st("pill-shirts","catShirts");st("pill-pants","catPants");
    st("pill-shorts","catShorts");st("pill-hats","catHats");st("pill-acc","catAcc");
    st("pill-other","catOther");st("pill-new","catNew");st("pill-top","catTop");
    // ── عداد القطع ──
    var pcDiv=document.querySelector(".pc");
    if(pcDiv){for(var _tn=0;_tn<pcDiv.childNodes.length;_tn++){if(pcDiv.childNodes[_tn].nodeType===3){pcDiv.childNodes[_tn].textContent=" "+_t("piecesCount");break;}}}
    // ── الفرز ──
    var ss=document.getElementById("ss");
    if(ss&&ss.options.length>=3){ss.options[0].text=_t("sortDef");ss.options[1].text=_t("sortLow");ss.options[2].text=_t("sortHigh");}
    // ── التنقل السفلي ──
    sq("#bn-home span","navHome");sq("#bn-cart span","navCart");
    sq("#bn-track span","navTrack");sq("#bn-help span","navHelp");
    // ── الفوتر ──
    var fhs=document.querySelectorAll(".footer-h");
    if(fhs[0])fhs[0].textContent=_t("footerLinks");
    if(fhs[1])fhs[1].textContent=_t("footerContact");
    st("fl-track","footerTrack");st("fl-faq","footerFaq");st("fl-policy","footerPolicy");
    // ── الشريط الجانبي (سلة) ──
    sq(".cart-title","myCart");sq(".cart-tot-l","cartTotal");
    si("checkout-btn","checkoutBtn");sph("search-inp","search");
    var emptySpan=document.querySelector("#cart-items .c-empty span:last-child");
    if(emptySpan)emptySpan.textContent=_t("cartEmpty");
    // ── نافذة المقاس ──
    var szMod=document.getElementById("size-mod");
    if(szMod){
      stn(szMod.querySelector(".mod-title"),"chooseSize");
      var sl=szMod.querySelectorAll(".fl label");
      if(sl[0])sl[0].textContent=_t("directSize");if(sl[1])sl[1].textContent=_t("weight");if(sl[2])sl[2].textContent=_t("height");if(sl[3])sl[3].textContent=_t("gender");
      sq("#size-mod .or-sep","orMeasure");
      var mgSel=document.getElementById("mg");
      if(mgSel&&mgSel.options.length>=3){mgSel.options[1].text=_t("male");mgSel.options[2].text=_t("female");}
      st("confirm-add-btn","addToCart");
    }
    st("pm-add-btn","chooseSizeAdd");
    // ── نافذة التقييم ──
    var rvMod=document.getElementById("review-mod");
    if(rvMod){
      stn(rvMod.querySelector(".mod-title"),"reviewTitle");
      var rl=rvMod.querySelectorAll(".fl label");
      if(rl[0])rl[0].textContent=_t("reviewName");if(rl[1])rl[1].textContent=_t("reviewPhone");
      if(rl[2])rl[2].textContent=_t("reviewRating");if(rl[3])rl[3].textContent=_t("reviewBody");
      sph("rv-body","reviewBodyPh");sph("rv-name","reviewNamePh");
      sqh("#review-mod .btn-main","reviewSubmit");
    }
    // ── نافذة التتبع ──
    var trkMod=document.getElementById("track-mod");
    if(trkMod){
      stn(trkMod.querySelector(".mod-title"),"trackTitle");
      sq("#track-mod .fl label","trackLabel");
      sph("track-inp","trackPh");st("track-btn","trackBtn");
    }
    // ── خطوات الدفع ──
    sq("#si-1 .step-lbl","step1");sq("#si-2 .step-lbl","step2");sq("#si-3 .step-lbl","step3");sq("#si-4 .step-lbl","step4");
    var s1=document.getElementById("chk-s1");
    if(s1){var l1=s1.querySelectorAll(".fl label");if(l1[0])l1[0].textContent=_t("lbFullName");if(l1[1])l1[1].textContent=_t("lbPhone1");if(l1[2])l1[2].textContent=_t("lbPhone2");if(l1[3])l1[3].textContent=_t("lbEmail");sph("o-name","fullNamePh");si("chk-next-1","nextBtn");}
    var s2=document.getElementById("chk-s2");
    if(s2){var l2=s2.querySelectorAll(".fl label");if(l2[0])l2[0].textContent=_t("lbWilaya");if(l2[1])l2[1].textContent=_t("lbCommune");if(l2[2])l2[2].textContent=_t("lbDelivery");var wOpt=document.querySelector("#o-wilaya option[value='']");if(wOpt)wOpt.text=_t("wilayaOpt0");var dlSel=document.getElementById("o-del");if(dlSel&&dlSel.options.length>=2){dlSel.options[0].text=_t("delOffice");dlSel.options[1].text=_t("delHome");}sph("o-commune","communePh");si("chk-prev-2","prevBtn");si("chk-next-2","nextBtn");}
    var s3=document.getElementById("chk-s3");
    if(s3){var l3=s3.querySelectorAll(".fl label");if(l3[0])l3[0].textContent=_t("lbPayment");sq("#pay-cod-lbl .pay-opt-title","payCod");sq("#pay-cod-lbl .pay-opt-sub","payCodSub");sq("#pay-ccp-lbl .pay-opt-title","payCcp");sq("#pay-ccp-lbl .pay-opt-sub","payCcpSub");
      var ccpDet=document.getElementById("ccp-details");
      if(ccpDet){var ccpDivs=ccpDet.querySelectorAll(":scope>div");if(ccpDivs[0])ccpDivs[0].textContent=_t("ccpDetailsTitle");if(ccpDivs[1])stn(ccpDivs[1],"ccpAccountNum");if(ccpDivs[2])stn(ccpDivs[2],"ccpAccountName");if(ccpDivs[3])ccpDivs[3].textContent=_t("ccpWhatsappNote");var ccpRefLbl=ccpDet.querySelector(".fl label");if(ccpRefLbl)ccpRefLbl.textContent=_t("lbCcpRef");sph("o-ccp-ref","ccpRefPh");}
      si("chk-prev-3","prevBtn");si("chk-next-3","nextBtn");}
    var s4=document.getElementById("chk-s4");
    if(s4){var niRows=s4.querySelectorAll(".op .op-row:not([id])");if(niRows[0]){var ol=niRows[0].querySelector(".op-l");if(ol)ol.textContent=_t("opProducts");}if(niRows[1]){var ol2=niRows[1].querySelector(".op-l");if(ol2)ol2.textContent=_t("opDelivery");}sq("#op-disc-row .op-l","opDiscount");st("op-coupon-lbl","opCouponLbl");var cLbl=s4.querySelector(".op .fl label");if(cLbl)cLbl.textContent=_t("opCouponLabel");sph("o-coupon","opCouponPh");var apB=s4.querySelector(".aact");if(apB)apB.textContent=_t("opApply");si("chk-prev-4","prevBtn");si("chk-btn","confirmOrder");}
    // ── إعادة رسم بطاقات المنتجات بالاسم والفئة المترجمة ──
    if(_prods&&_prods.length)_renderGrid();
  }

  function _setLang(lang){
    _lang=lang||"ar";
    try{localStorage.setItem("store_lang",_lang);}catch{}
    document.documentElement.lang=_lang;
    document.documentElement.dir=_lang==="ar"?"rtl":"ltr";
    var sb=document.getElementById("cart-sb");if(sb)sb.setAttribute("dir",_lang==="ar"?"rtl":"ltr");
    var lb=document.getElementById("lang-btn-hdr");if(lb)lb.textContent=_lang.toUpperCase();
    var opts=document.querySelectorAll(".lang-opt");
    for(var oi=0;oi<opts.length;oi++)opts[oi].classList.toggle("on",opts[oi].getAttribute("data-lang")===_lang);
    _applyI18n();
  }
  function _toggleLangMenu(force){
    var m=document.getElementById("lang-menu");if(!m)return;
    var show=force!==undefined?force:!m.classList.contains("open");
    m.classList.toggle("open",show);
  }
  function _initLang(){
    try{var saved=localStorage.getItem("store_lang");if(saved)_lang=saved;}catch{}
    _setLang(_lang);
  }
  ...
  document.addEventListener("DOMContentLoaded",function(){
    try{
      ...
      // ── LANGUAGE SWITCHER (touch-optimised) ──
      _fastTap(document.getElementById("lang-btn-hdr"),function(e){e.preventDefault();e.stopPropagation();_toggleLangMenu();});
      var _langOpts=document.querySelectorAll(".lang-opt");
      for(var _lo=0;_lo<_langOpts.length;_lo++){
        (function(opt){
          _fastTap(opt,function(e){e.preventDefault();_setLang(opt.getAttribute("data-lang"));_toggleLangMenu(false);});
        })(_langOpts[_lo]);
      }
      document.addEventListener("click",function(e){
        var sw=document.getElementById("lang-sw");
        if(sw&&!sw.contains(e.target))_toggleLangMenu(false);
      });
      ...
      (function(){
        ...
        // hover style
        if(lbl){
          lbl.addEventListener("mouseenter",function(){lbl.style.borderColor="rgba(0,0,0,.6)";lbl.style.background="rgba(0,0,0,.13)";});
          lbl.addEventListener("mouseleave",function(){lbl.style.borderColor="rgba(0,0,0,.3)";lbl.style.background="rgba(0,0,0,.08)";});
        }
      })();
      ...
      document.querySelectorAll("[data-delvis]").forEach(function(btn){
        btn.addEventListener("click",function(){_deleteVisitors(btn.getAttribute("data-delvis"));});
      });
  ...
  return {
    ...
    _loadErrorLog:_loadErrorLog,
    _clearErrorLog:_clearErrorLog,
    _translateAllProds:_translateAllProds,
