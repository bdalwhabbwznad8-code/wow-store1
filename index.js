...
.lang-switch{display:flex;align-items:center;gap:6px;background:linear-gradient(135deg,rgba(168,85,247,.14),rgba(59,7,100,.18));border:1px solid rgba(168,85,247,.34);border-radius:12px;padding:6px 8px;color:rgba(233,213,255,.9);box-shadow:0 8px 26px rgba(109,40,217,.14);position:relative}
.lang-switch::before{content:'🌐';font-size:13px;line-height:1}
.lang-label{font-size:10px;font-weight:800;letter-spacing:.7px;color:rgba(216,180,254,.96);white-space:nowrap}
.lang-select{appearance:none;background:rgba(3,0,8,.8);border:1px solid rgba(168,85,247,.25);border-radius:8px;color:#fff;font-size:11px;font-weight:700;padding:5px 24px 5px 8px;cursor:pointer;outline:none;min-width:78px}
.lang-switch::after{content:'⌄';position:absolute;left:12px;top:50%;transform:translateY(-55%);color:rgba(216,180,254,.75);font-size:12px;pointer-events:none}
html[dir="ltr"] .lang-switch::after{left:auto;right:12px}
.cart-btn{display:flex;align-items:center;gap:6px;background:rgba(168,85,247,.12);border:1px solid rgba(168,85,247,.25);border-radius:var(--rs);padding:8px 12px;cursor:pointer;color:rgba(192,132,252,.9);font-size:12px;font-weight:700;white-space:nowrap;transition:transform .18s ease,background .18s ease,border-color .18s ease,box-shadow .18s ease;min-height:38px;touch-action:manipulation}
.cart-btn:hover{background:rgba(168,85,247,.22);border-color:rgba(168,85,247,.5);transform:translateY(-1px);box-shadow:0 10px 24px rgba(109,40,217,.18)}
...
.addbtn{background:linear-gradient(135deg,rgba(109,40,217,.34),rgba(168,85,247,.2));border:1px solid rgba(168,85,247,.38);border-radius:12px;color:rgba(245,240,255,.96);font-size:12px;font-weight:800;letter-spacing:.3px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;padding:11px 10px;cursor:pointer;transition:transform .18s cubic-bezier(.34,1.2,.64,1),background .18s,border-color .18s,box-shadow .18s;margin-top:9px;position:relative;overflow:hidden;text-align:center;min-height:40px;touch-action:manipulation}
...
.btn-main{background:linear-gradient(135deg,#6d28d9,#9333ea);border:1px solid rgba(216,180,254,.18);border-radius:12px;color:#fff;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;font-size:12px;font-weight:800;padding:12px;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease,filter .18s ease;width:100%;min-height:42px;touch-action:manipulation}
.btn-main:hover{box-shadow:0 8px 26px rgba(109,40,217,.38);filter:brightness(1.05);transform:translateY(-1px)}
...
.aact{background:rgba(255,255,255,.035);border:1px solid var(--b1);border-radius:8px;padding:6px 10px;font-size:11px;font-weight:700;cursor:pointer;color:rgba(255,255,255,.58);transition:transform .15s ease,background .15s ease,border-color .15s ease,color .15s ease;min-height:30px;touch-action:manipulation;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif}
.aact:hover{transform:translateY(-1px);background:rgba(168,85,247,.09);border-color:rgba(168,85,247,.28);color:rgba(255,255,255,.82)}
.aact:active{transform:scale(.97)}
.aact.e:hover{border-color:rgba(168,85,247,.5);color:rgba(192,132,252,.96)}
...
.oc{background:rgba(255,255,255,.03);border:1px solid var(--b1);border-radius:13px;padding:13px;margin-bottom:10px;transition:border-color .18s ease,background .18s ease,transform .18s ease;cursor:pointer}
.oc:hover{background:rgba(168,85,247,.035);border-color:rgba(168,85,247,.2);transform:translateY(-1px)}
.oc.open{border-color:rgba(168,85,247,.38);background:rgba(168,85,247,.045)}
...
.oc-link{display:inline-flex;align-items:center;gap:4px;color:rgba(192,132,252,.88);text-decoration:none;font-size:10px;font-weight:700;border:1px solid rgba(168,85,247,.25);background:rgba(168,85,247,.08);border-radius:7px;padding:3px 7px;margin-inline-start:6px;white-space:nowrap}
.oc-link:hover{background:rgba(168,85,247,.18);border-color:rgba(168,85,247,.45);color:#fff}
.oc-more{display:none;margin-top:10px;padding:10px;border:1px solid rgba(168,85,247,.13);background:rgba(0,0,0,.18);border-radius:10px}
.oc.open .oc-more{display:block}
.oc-more-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:8px}
.oc-more-field{background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.055);border-radius:8px;padding:8px}
.oc-more-field small{display:block;color:var(--mu);font-size:9px;letter-spacing:1px;text-transform:uppercase;margin-bottom:3px}
.oc-more-field span{font-size:11px;color:rgba(255,255,255,.72);word-break:break-word}
.oc-total-breakdown{margin-top:8px;display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:6px;font-size:10px;color:var(--dim)}
...
@media(max-width:600px){
  ...
  .lang-select{min-width:66px;padding-inline-start:6px;padding-inline-end:20px;font-size:10px}
  .lang-switch{padding:5px 6px}
}
...
<header class="hdr">
  <div class="hdr-i">
    ...
    <div class="hdr-r">
      ...
      <div class="lang-switch" title="تغيير لغة المتجر" aria-label="تغيير لغة المتجر">
        <span class="lang-label" id="lang-label">اللغة</span>
        <select class="lang-select" id="lang-select" aria-label="اختيار اللغة">
          <option value="ar">AR عربي</option>
          <option value="fr">FR Français</option>
          <option value="en">EN English</option>
        </select>
      </div>
      <button class="cart-btn" id="cart-btn-hdr" aria-label="السلة">
...
var WOW = (function(){
    ...
    try{
      ...
      fp.forEach(function(p,idx){
        ...
        var actionHtml=(!p.stock||(p.quantity!==null&&p.quantity!==undefined&&p.quantity===0))
          ?"<button class='waitlist-btn' data-wl-pid='"+p.id+"' data-wl-name='"+_esc(p.name||"")+"'>"+_t("waitlist")+"</button>"
          :"<button class='addbtn' data-pid='"+p.id+"'>"+_t("addCart")+"</button>";
  ...
  /* ── ORDERS ── */
  function _orderProductLink(it){
    if(!it||it.id===undefined||it.id===null||it.id==="")return "";
    return "<a class='oc-link' href='/p/"+encodeURIComponent(it.id)+"' target='_blank' rel='noopener' title='فتح صفحة المنتج' data-no-toggle='1'>🔗 المنتج</a>";
  }
  function _renderOrderItem(it,withPrice){
    var qty=Math.max(1,parseInt(it.qty)||1);
    var price=(parseFloat(it.price)||0)*qty;
    return "<div class='oc-pi'>"
      +(it.img?"<img class='oc-pimg' src='"+_esc(it.img)+"' loading='lazy' alt=''>":"")
      +"<span class='oc-pn'>"+_esc(it.name||"")+(it.size?" ["+_esc(it.size)+"]":"")+" x"+qty+_orderProductLink(it)+"</span>"
      +(withPrice?"<span class='oc-pp'>"+_fmt(price)+"</span>":"")
      +"</div>";
  }
  function _renderOrderExtra(o){
    var pay=o.payMethod==="ccp"?"CCP / دفع مسبق":"الدفع عند الاستلام";
    var items=(o.items||[]).map(function(it){return _renderOrderItem(it,true);}).join("");
    return "<div class='oc-more'>"
      +"<div class='oc-more-grid'>"
      +"<div class='oc-more-field'><small>البريد</small><span>"+_esc(o.email||"—")+"</span></div>"
      +"<div class='oc-more-field'><small>طريقة الدفع</small><span>"+_esc(pay)+(o.ccpRef?" — مرجع: "+_esc(o.ccpRef):"")+"</span></div>"
      +"<div class='oc-more-field'><small>التوصيل</small><span>"+_esc(o.dlbl||"Stop Desk")+"</span></div>"
      +"<div class='oc-more-field'><small>العنوان الكامل</small><span>"+_esc((o.wilaya||"")+" / "+(o.commune||""))+"</span></div>"
      +"<div class='oc-more-field'><small>الخصم</small><span>"+_fmt(o.discAmt||0)+" دج"+(o.couponCode?" — كوبون "+_esc(o.couponCode):"")+"</span></div>"
      +"<div class='oc-more-field'><small>رسوم التوصيل</small><span>"+_fmt(o.fee||0)+" دج</span></div>"
      +"</div>"
      +"<div style='margin-top:10px;font-size:10px;color:rgba(192,132,252,.75);font-weight:800'>كل المنتجات مع روابطها:</div>"
      +"<div class='oc-pl' style='margin-top:6px'>"+items+"</div>"
      +"<div class='oc-total-breakdown'>"
      +"<span>المنتجات قبل الخصم: "+_fmt(o.originalSub||o.finalSub||0)+" دج</span>"
      +"<span>بعد الخصم: "+_fmt(o.finalSub||0)+" دج</span>"
      +"<span>الإجمالي النهائي: "+_fmt(o.total||0)+" دج</span>"
      +"</div>"
      +"</div>";
  }
  function _bindOrderCardToggles(container){
    container.querySelectorAll(".oc[data-order-card]").forEach(function(card){
      card.addEventListener("click",function(e){
        if(e.target.closest("button,select,textarea,input,a,[data-no-toggle]"))return;
        card.classList.toggle("open");
      });
    });
    container.querySelectorAll("[data-toggle-order]").forEach(function(btn){
      btn.addEventListener("click",function(e){
        e.preventDefault();e.stopPropagation();
        var card=btn.closest(".oc");if(!card)return;
        card.classList.toggle("open");
        btn.textContent=card.classList.contains("open")?"إخفاء التفاصيل":"تفاصيل كاملة";
      });
    });
  }
  function _loadOrders(){
    ...
    _api("/api/orders").then(function(r){return r.json();}).then(function(orders){
      ...
      c.innerHTML=orders.map(function(o){
        var ih=(o.items||[]).map(function(it){return _renderOrderItem(it,true);}).join("");
        ...
        return "<div class='oc' data-order-card='"+_esc(o.id)+"'><div class='oc-h'><span class='oc-id'>"+_esc(o.id)+"</span>"
              ...
              +"<button class='aact e' data-toggle-order='"+_esc(o.id)+"'>تفاصيل كاملة</button>"
  ...
  var _lang="ar";
  var _T={
    ar:{langLabel:"اللغة",addCart:"+ أضف للسلة",waitlist:"⏳ نبهني حين يتوفر",outOfStock:"نفذ المخزون",checkout:"إتمام الشراء →",cart:"السلة",search:"ابحث عن منتج...",all:"الكل",shirts:"القمصان",pants:"البناطيل",shorts:"الشورتات",hats:"القبعات",acc:"الاكسسوارات",other:"اخرى",new:"الجديد",top:"الاكثر مبيعا",defaultSort:"ترتيب افتراضي",lowSort:"السعر: الاقل اولا",highSort:"السعر: الاعلى اولا",piece:"قطعة",home:"الرئيسية",track:"تتبع",help:"مساعدة",processing:"قيد المعالجة",shipped:"تم الشحن",delivered:"تم التوصيل",returned:"مُرتجعة"},
    fr:{langLabel:"Langue",addCart:"+ Ajouter au panier",waitlist:"⏳ Me prévenir",outOfStock:"Rupture de stock",checkout:"Commander →",cart:"Panier",search:"Rechercher un produit...",all:"Tous",shirts:"Chemises",pants:"Pantalons",shorts:"Shorts",hats:"Casquettes",acc:"Accessoires",other:"Autres",new:"Nouveautés",top:"Meilleures ventes",defaultSort:"Tri par défaut",lowSort:"Prix: croissant",highSort:"Prix: décroissant",piece:"articles",home:"Accueil",track:"Suivi",help:"Aide",processing:"En traitement",shipped:"Expédié",delivered:"Livré",returned:"Retourné"},
    en:{langLabel:"Language",addCart:"+ Add to cart",waitlist:"⏳ Notify me",outOfStock:"Out of stock",checkout:"Checkout →",cart:"Cart",search:"Search products...",all:"All",shirts:"Shirts",pants:"Pants",shorts:"Shorts",hats:"Hats",acc:"Accessories",other:"Other",new:"New",top:"Best sellers",defaultSort:"Default sort",lowSort:"Price: low to high",highSort:"Price: high to low",piece:"items",home:"Home",track:"Track",help:"Help",processing:"Processing",shipped:"Shipped",delivered:"Delivered",returned:"Returned"}
  };
  function _t(key){return(_T[_lang]&&_T[_lang][key])||_T["ar"][key]||key;}
  function _txt(id,key){var el=document.getElementById(id);if(el)el.textContent=_t(key);}
  function _setLang(lang){
    _lang=(_T[lang]?lang:"ar");
    try{localStorage.setItem("wow_lang",_lang);}catch{}
    document.documentElement.lang=_lang;
    document.documentElement.dir=_lang==="ar"?"rtl":"ltr";
    var sel=document.getElementById("lang-select");if(sel)sel.value=_lang;
    _txt("lang-label","langLabel");
    var search=document.getElementById("search-inp");if(search)search.placeholder=_t("search");
    _txt("pill-all","all");_txt("pill-shirts","shirts");_txt("pill-pants","pants");_txt("pill-shorts","shorts");_txt("pill-hats","hats");_txt("pill-acc","acc");_txt("pill-other","other");_txt("pill-new","new");_txt("pill-top","top");
    var ss=document.getElementById("ss");if(ss&&ss.options.length>=3){ss.options[0].textContent=_t("defaultSort");ss.options[1].textContent=_t("lowSort");ss.options[2].textContent=_t("highSort");}
    var pc=document.querySelector(".pc");if(pc){var n=document.getElementById("pc");pc.innerHTML="<span id='pc'>"+(n?n.textContent:"0")+"</span> "+_t("piece");}
    var checkout=document.getElementById("checkout-btn");if(checkout)checkout.textContent=_t("checkout");
    var cartTitle=document.querySelector(".cart-title");if(cartTitle)cartTitle.textContent=_t("cart");
    var cartBtn=document.getElementById("cart-btn-hdr");if(cartBtn)cartBtn.setAttribute("aria-label",_t("cart"));
    var navs=[ ["bn-home","home"],["bn-cart","cart"],["bn-track","track"],["bn-help","help"] ];
    navs.forEach(function(x){var el=document.querySelector("#"+x[0]+" span");if(el)el.textContent=_t(x[1]);});
    document.querySelectorAll(".addbtn").forEach(function(b){b.textContent=_t("addCart");});
    document.querySelectorAll(".waitlist-btn").forEach(function(b){b.textContent=_t("waitlist");});
  }
  function _initLang(){
    var saved="ar";try{saved=localStorage.getItem("wow_lang")||"ar";}catch{}
    var sel=document.getElementById("lang-select");
    if(sel){sel.value=_T[saved]?saved:"ar";sel.addEventListener("change",function(){_setLang(sel.value);_renderGrid();_toast("🌐 "+sel.options[sel.selectedIndex].text);});}
    _setLang(_T[saved]?saved:"ar");
  }
  ...
  function _loadSettingsExtra(s){
    ...
    try{if(s.lang&&!localStorage.getItem("wow_lang"))_setLang(s.lang);}catch(e){if(s.lang)_setLang(s.lang);}
  }
